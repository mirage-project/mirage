...
def draw_edge(G, from_node, to_node, graph_type, label=None):
    G.edge(
        from_node,
        to_node,
        color=colors_map[graph_type]["edge"],
        penwidth="6",
        label=label,
        fontname="sans-serif",
        fontsize=edge_font_size,
        fontcolor=colors_map[graph_type]["edge_label"],
    )
...
class block_node(node):
    ...
    def draw(self, G):
        ...
        if self.is_input_node():
            ...
            G.node(
                tensor_node_name,
                label=self.original_tensor.name
                + "'\n"
                ...
                shape="box",
                penwidth="0",
                fontsize=tensor_node_font_size,
                fontcolor="white",
                fontname="sans-serif",
            )
            G.node(
    ...
    def draw(self, G):
        if self.next_nodes:
            for next_node in self.next_nodes:
                if (
                    self.last_node.is_customized_node()
                    or next_node.is_customized_node()
                ):
                    G.node(
                        self.name,
                        label=self.name + "\n" + str(self.shape),
...
class block_tensor(tensor):
    ...
    def draw(self, G):
        for next_node in self.next_nodes:
            draw_edge(
                G, self.last_node.name, next_node.name, "block", label=str(self.shape)
            )
...
class kernel_graph(graph):
    ...
    def __init__(self, label):
        super().__init__(
            label, colors_map["kernel"]["bg"], colors_map["kernel"]["edge"]
        )
        self.block_graphs = []
    ...
    def read_nodes(self, graph_data):
        ...
        for node_data in graph_data:
            ...
            new_node = kernel_node(
                node_name, node_op_type, node_id, op_nodelabel_mapping[node_op_type]
            )
            for input_tensor in node_data["input_tensors"]:
                ...
                if tensor_guid in guid_tensors_map:
                    ...
                    tensor = kernel_tensor(
                        tensor_guid, input_tensor["dim"][: input_tensor["num_dims"]]
                    )
            ...
            for output_tensor in node_data["output_tensors"]:
                ...
                if tensor_guid in guid_tensors_map:
                    ...
                    tensor = kernel_tensor(
                        tensor_guid, output_tensor["dim"][: output_tensor["num_dims"]]
                    )
                if new_node.is_customized_node() and not tensor.name:
                    tensor.name = next(self.letter_sequence)
            ...
            if "bgraph" in node_data:
                ...
                block_graph_datas_to_handle.append(
                    (new_block_graph, node_data["bgraph"]["operators"])
                )
    ...
    def read_nodes(self, graph_data):
        ...
        for node_data in graph_data:
            ...
            if "dtensor" in node_data:
                new_node.original_tensor = guid_tensors_map[
                    node_data["dtensor"]["guid"]
                ]
    ...
    def draw_graph(self, G):
        with G.subgraph(name="cluster" + self.label) as sub:
            sub.attr(
                ...
                bgcolor=self.bg_color,
                fontname="sans-serif",
                label=self.label + self.get_grid_size_and_forloop(),
                labelloc="t",
                labeljust="l",
                labeldistance="1.5",
                fontsize=graph_label_font_size,
    ...
    if isinstance(data, list):
        if all(isinstance(item, list) for item in data):
            for idx, graph_list in enumerate(data):
                handle_graph_data(
                    graph_list,
                    graph_title=f"Combined graph {idx+1}",
                    output_filename=f"reframe_outcome/reframe_combined_graph_{idx+1}",
                )
        elif all(is_graph_data(item) for item in data):
            handle_graph_data(
                data,
                graph_title="Combined graph",
                output_filename="reframe_outcome/reframe_combined_graph",
            )
        else:
