...
class PersistentKernel:
    ...
    def ep_moe_routing_layer(
        ...
        # output: (routing_indices [batch_size, topk], routing_weights [batch_size, topk],
        ...
        assert routing_indices.num_dims == 2   # (batch_size, topk)
        assert routing_weights.num_dims == 2   # (batch_size, topk)
        assert dispatch_counts.num_dims == 1   # (world_size,)
        tb_graph = TBGraph(CyTBGraph(grid_dim, block_dim, 1, 64))
        # input: partition on batch dim; outputs: per-token or shared
        tb_graph.new_input(input,           (0, -1, -1), -1, True)
        tb_graph.new_input(routing_indices, (0, -1, -1), -1, True)
        tb_graph.new_input(routing_weights, (0, -1, -1), -1, True)
        tb_graph.new_input(dispatch_counts, (-1, -1, -1), -1, True)  # shared atomic
        self.kn_graph.customized(
