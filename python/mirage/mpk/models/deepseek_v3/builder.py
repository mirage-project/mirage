...
class DeepSeekV3Builder(GraphBuilder):
    ...
    def _fp8_mbt_buffers_for_reduction_f32scale(self, reduction_size: int):
        ...
        return self._fp8_quant_buffers(
            ...
            f"fp8_input_{reduction_size}_shared",
            f"fp8_scale_{reduction_size}_shared")
    ...
    def _new_intermediate_tensors(self):
        ...
        if self._use_prefill:
            ...
            # `kpe_sep_bridged` receives the identity-copy "phantom bridge"
            ...
            # case-3 rationale).
            self._kpe_sep_bridged_torch = _torch.zeros(
                self._prefill_kv_rows, QK_ROPE_HEAD_DIM,
                dtype=_torch.bfloat16, device="cuda")
            self.kpe_sep_bridged = self.mpk.attach_input(
                self._kpe_sep_bridged_torch, name="kpe_sep_bridged")
    ...
    def _build_mla_attention_layer(self, layer_idx: int, state_dict: dict):
        ...
        if self._use_prefill:
            ...
            # `kpe_sep → kpe_sep_bridged` (chunked_prefill reads kpe_sep_bridged)
            ...
            self.mpk.identity_layer(
                input=kpe_sep,
                output=self.kpe_sep_bridged,
                grid_dim=(8, 1, 1),
                block_dim=(128, 1, 1),
                # Decode iters (Q_LEN<=8) skip the copy body: kpe_sep_bridged
            ...
            self.mpk.mla_prefill_tp8_chunked_layer(
                ...
                # k_rope comes from `kpe_sep_bridged`, the phantom-bridged copy
                # of the kv_buf rope view produced by the identity_layer
                # above (case-3, see comment there).
                k_rope=self.kpe_sep_bridged,
                v=self.prefill_v,
