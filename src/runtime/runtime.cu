...
__global__ RuntimeConfig init_kernel(void* work_space) {
  assert(gridDim.x == 1);
  assert(gridDim.y == 1);
  assert(gridDim.z == 1);
  ...
  if (threadIdx.x == 0) {
    for (int i = 0; i < config.num_workers; i++) {
      config.work_queue_last_task[i] = 0;
    }
    for (int i = 0; i < config.num_schedulers; i++) {
      config.sched_queue_last_task[i] = 0;
    }
  }
  return config;
}
...
bool is_termination_event(size_t event_loc, EventDesc e) {
  return (event_loc == 0);
}
...
__global__ void persistent_kernel(RuntimeConfig config) {
  ...
  if (blockIdx.x < config.num_workers) {
    ...
    TaskId* task_queue = config.work_queues[worker_id];
    while (true) {
      ...
      if (threadIdx.x == 0) {
        ...
        assert(cur_task_id + config.per_work_queue_len > last_task_id);
        cur_task_loc = task_queue[cur_task_id % config.work_queue_len];
      }
      __syncthreads();
      TaskDesc task_desc = config.all_tasks[cur_task_loc];
      // Successfully fetched a new task
      switch (task_desc.task_type) {
        case type::TASK_TERMINATE: {
          return;
          break;
        }
        case type::RMS_NORM_LINEAR: {
          break;
        }
        default: {
      ...
      // Trigger event
      if (threadIdx.x == 0) {
        EventId event_id = task_desc.event_id;
        ...
        if (count == 1) {
          ...
          atomicAdd(config.all_event_counters[event_id], event_desc.num_triggers);
          ...
          config.sched_queues[sched_id][last_task_id % config.per_sched_queue_len] = event_id;
        }
      }
      cur_task_id += 1;
    }
    ...
    if (warp_id < 4 && thread_id == 0) {
      ...
      int next_worker = rand() % config.num_workers;
      while (true) {
        ...
        // Launch new tasks
        EventId event_loc = sched_queue[cur_event_id % config.sched_queue_len];
        EventDesc e = config.event_descs[event_loc];
        if (is_termination_event(event_loc, e) {
          return;
        }
        for (TaskId i = e.first_task_id; i < e.last_task_id; i++) {
          size_t last_task_id =
              atomicAdd(&(config.worker_queue_last_task_id[next_worker]), 1);
          config.worker_queue[next_worker][last_task_id % config.per_worker_queue_len] = i;
