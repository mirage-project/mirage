    ...
    for(uint32_t n = 0; n < num_n / 4; n++){
      for(uint32_t m = 0; m < num_m; m++){
        ...
        for(uint32_t k = 0; k < num_k; k++){
          ...
          //  printf("%f, %f, %f, %f, ", __bfloat162float(input_weight_smem.at(n_row, n_col)),
          ...
          //  if(for_idx == 0){
            // printf("threadid %d, k %d, n_row %d, n_col %d, value %f, %f, %f, %f\n", threadIdx.x, k, n_row, n_col, 
            //   __bfloat162float(input_weight_smem.at(n_row, n_col)), __bfloat162float(input_weight_smem.at(n_row, n_col + 1)), 
            //   __bfloat162float(input_weight_smem.at(n_row, n_col + 2)), __bfloat162float(input_weight_smem.at(n_row, n_col + 3)));
          // }
           
          mma_m16n16k16_bf16bf16bf32(s_frag[m][n], a_frag, b_frag, s_frag[m][n]);

          //     if(m == 0 && n == 0 && threadIdx.x == 0 ){
          //   printf("for_idx = %d, s_frag[m][n] %f\n", for_idx, s_frag[0][0][0]);
          //  }
