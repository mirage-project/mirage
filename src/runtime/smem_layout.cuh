...
template <typename T, int B, int M, int S, size_t ROW_, size_t COL_, size_t STRIDE>
struct smem_row {
    ...
    __device__ __forceinline__
    void set_ptr(T* ptr) { base_ptr = ptr; }
    ...
    T* operator() (size_t logical_idx_row, size_t logical_idx_col) {
        ...
        size_t phy_offset = block_idx * (Pow2_M * Pow2_S * Pow2_B) + irow * (Pow2_M * Pow2_S)
        + icol * (Pow2_M) + offset_in_bank;

        // printf("tid is %d, offset is %d, %d, %d, blockidx %d logical_idx %d\n", threadIdx.x, (int)logical_idx_row, (int)logical_idx_col, (int)phy_offset, (int)block_idx, (int)logical_idx);
    ...
    T& at(size_t logical_idx) {
        ...
        // printf("smem at %d, %d, %f\n", (int)logical_idx, (int)phy_offset, __bfloat162float(base_ptr[phy_offset]));
...
// template <typename T, int B, int M, int S, size_t ROW_, size_t COL_, size_t STRIDE>
// struct smem_row_const {
//     const T* __restrict__ base_ptr;
...
//     __device__ __forceinline__
//     smem_row(const T* ptr) : base_ptr(ptr) {}
    
//     __device__ __forceinline__
//     void set_ptr(const T* ptr) { base_ptr = ptr; }
...
//     // 1D access
//     __device__ __forceinline__
//     const T* operator[] (size_t logical_idx) const {
//         // assert(logical_idx % Pow2_M == 0);
...
template <typename T, int B, int M, int S, size_t ROW_, size_t COL_, size_t STRIDE,  typename Ptr = T*>
struct smem_col {
    Ptr base_ptr;
    ...
    __device__ __forceinline__
    smem_col(Ptr ptr) : base_ptr(ptr) {}

    __device__ __forceinline__
    void set_ptr(Ptr ptr) { base_ptr = ptr; }
