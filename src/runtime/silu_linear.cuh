...
namespace mirage {
 ...
 __device__ __forceinline__ void silu_linear_kernel(){
    constexpr int chunk_size = 128 / sizeof(T);
    constexpr int num_chunks = HIDDEN_SIZE / chunk_size;

    //using SM80_16x8x16_F16F16F16F16_TN -> 16X16X16
  ...
  smem_row<T> mm_output_smem(mm_output);
  smem_row<T> reduction_output_smem(reduction_output);

  smem_row<T> output_smem(shared_output);
    ...
    for(uint32_t n = 0; n < num_n / 4; n++){
      ...
      for(uint32_t m = 0; m < num_m; m++){
        ...
        for(uint32_t k = 0; k < num_k; k++){
           //load A matrix
           uint32_t m_offset = k * (16 * 16)+ lane_idx;
           //load B matrix, 
           uint32_t n_offset = warp_id * (64*8) + k * (16*8) + + lane_idx;
           ldsm(input_smem(m_offset), a_frag);
           ldsm(input_weight_smem(n_offset), b_frag);
           mma16816_bf16bf16bf32(s_frag[m][n], a_frag, b_frag, s_frag[m][n]);
        }
      }
    }
    //MNK = 1, 64, 64
