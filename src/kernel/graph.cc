...
void Graph::register_task(char const *task_type, std::vector<int> params) {
  ...
  } else if (name == "linear_fp8_mpk_sm100") {
    int variant_id = task_register->register_linear_fp8_mpk_sm100_task(
        customized->bgraph, params, false);
    task_config[op] =
        std::make_tuple(4, 1, TASK_LINEAR_FP8_MPK_SM100, variant_id);
  } else if (name == "linear_fp8_mpk_with_residual_sm100") {
    int variant_id = task_register->register_linear_fp8_mpk_sm100_task(
        customized->bgraph, params, true);
    task_config[op] = std::make_tuple(
        5, 1, TASK_LINEAR_FP8_WITH_RESIDUAL_MPK_SM100, variant_id);
  }
  // MLA KV gather
