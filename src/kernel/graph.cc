...
void Graph::register_task(char const *task_type, std::vector<int> params) {
  ...
  // SM100 tasks
    ...
    int variant_id = task_register->register_mla_prefill_tp8_chunked_sm100_task(
        customized->bgraph, params);
    // Per-head unabsorbed: 5 inputs (Qn, Qp, K_nope, K_rope, V) + 1 output (O).
    task_config[op] =
    ...
    int variant_id =
        task_register->register_mla_prefill_tp8_chunked_splitk_sm100_task(
            customized->bgraph, params);
    // 5 inputs (Qn, Qp, K_nope, K_rope, V), 1 output (partial). Partial as
    // output so MPK dep tracker links SPLITK → REDUCE through it.
    task_config[op] = std::make_tuple(
  ...
  // FP8 tasks
    ...
    int variant_id =
        task_register->register_fp8_group_gemm_smallm_sm100_task(
            customized->bgraph, params);
    task_config[op] = std::make_tuple(
        5, 1, TASK_FP8_GROUP_GEMM_SMALLM_SM100, variant_id);
  } else if (name == "fp8_group_gemm_largem_sm100") {
    int variant_id =
        task_register->register_fp8_group_gemm_largem_sm100_task(
            customized->bgraph, params);
    task_config[op] = std::make_tuple(
        5, 1, TASK_FP8_GROUP_GEMM_LARGEM_SM100, variant_id);
  }
  // MLA KV gather
