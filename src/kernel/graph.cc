...
void Graph::register_task(char const *task_type, std::vector<int> params) {
  ...
  } else if (name == "sigmoid_gate_mul_add_sm100") {
    int variant_id = task_register->register_sigmoid_gate_mul_add_sm100_task(
        customized->bgraph, params);
    task_config[op] =
        std::make_tuple(4, 1, TASK_SIGMOID_GATE_MUL_ADD_SM100, variant_id);
  ...
  } else if (name == "linear_fp8_blockscale_sm100") {
    int variant_id = task_register->register_linear_fp8_blockscale_sm100_task(
        customized->bgraph, params, false);
    task_config[op] =
        std::make_tuple(4, 1, TASK_LINEAR_FP8_BLOCKSCALE_SM100, variant_id);
  } else if (name == "linear_fp8_blockscale_with_residual_sm100") {
    int variant_id = task_register->register_linear_fp8_blockscale_sm100_task(
        customized->bgraph, params, true);
    task_config[op] =
  ...
  // Gated-DeltaNet recurrence + fused gated norm: 7 inputs (qkv_c, ba,
  // A_log|dt_bias, state pool, z, norm weight, decode split scratch) + 1
  // output.  The state pool is read AND written in place through
  // input_ptrs[3]; the split scratch (fp32 `o` partials + the arrival counter
  // that elects the epilogue task) likewise through input_ptrs[6].
