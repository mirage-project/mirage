...
void Graph::register_task(char const *task_type, std::vector<int> params) {
  ...
  // SM100 tasks
    ...
    int variant_id = task_register->register_mhc_post_pre_k1_sm100_task(
        customized->bgraph, params);
    task_config[op] =
        std::make_tuple(5, 3, TASK_MHC_POST_PRE_K1_SM100, variant_id);
  } else if (name == "mhc_post_pre_k2_sm100") {
    int variant_id = task_register->register_mhc_post_pre_k2_sm100_task(
        customized->bgraph, params);
    task_config[op] =
        std::make_tuple(5, 3, TASK_MHC_POST_PRE_K2_SM100, variant_id);
