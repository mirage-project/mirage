...
void Transpiler::get_threadblock_swizzle_plan(tb::Graph const &tb_graph,
	...
	{
		for (const tb::STensor &stensor : all_stensors) {
		...
		for (const TBSchedNode &node : Combine(Combine(sched.pre_loop_nodes, sched.loop_nodes), sched.post_loop_nodes)) {
			...
			if (op->op_type == type::TB_INPUT_OP) {
				if (op_meta.is_chunked_input) {
					// For chunked input ops, every num_16B_elems (8 when T is half) elements must be contiguous
					const tb::STensor &output_stensor = last_op->output_tensors.at(0);
				...
				if (op_meta.is_chunked_output) {
					// For chunked output ops, every num_16B_elems (8 when T is half) elements must be contiguous
					const tb::STensor &input_stensor = op->input_tensors.at(0);
				...
				const tb::STensor &input_stensor0 = op->input_tensors.at(0);
				const tb::STensor &input_stensor1 = op->input_tensors.at(1);
				const tb::STensor &output_stensor = last_op->output_tensors.at(0);
	...
	for (const tb::STensor &stensor : all_stensors) {
		...
		if (stensor.dim[swizzled_dim] == 1 || stensor.dim[meta.innermost_dim] == 1) {
			// No need to swizzle
			continue;
		}
    ...
    assert(chunk_size_bytes <= 16); // The chunk size should be less than 16B (8 halfs)
    ...
    int num_chunks_in_inner_dim = meta.strides[swizzled_dim] / chunk_size_num_elems;
    int num_chunks_in_128B = 128 / chunk_size_bytes;	// 128B is the size of all banks
      ...
      if (num_chunks_in_inner_dim > num_chunks_in_128B) {
        ...
        meta.xor_swizzle_b = log2(num_chunks_in_128B / (num_chunks_in_128B / num_chunks_in_inner_dim));  // Actually log2(num_chunks_in_inner_dim)
        meta.xor_swizzle_m = log2(chunk_size_num_elems);
