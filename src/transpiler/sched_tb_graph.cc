...
std::vector<TBSchedNode> ops2sched(vector<std::pair<tb::TBOperator const*, OpMeta>> &ops, const std::function<bool(const tb::TBOperator*)>& filter) {
	std::sort(ops.begin(), ops.end(), [&](auto const& a, auto const& b) {
		const OpMeta &meta_a = a.second;
		const OpMeta &meta_b = b.second;
		return meta_a.level != meta_b.level ? meta_a.level < meta_b.level : meta_a.fuse_chain_idx != meta_b.fuse_chain_idx ? meta_a.fuse_chain_idx < meta_b.fuse_chain_idx : meta_a.pos_in_fuse_chain < meta_b.pos_in_fuse_chain;
	});
	...
	for (int cur_op_idx = 0; cur_op_idx < num_ops; ) {
		...
		if (!filter(op)) {
			// Skip this operator
			assert(cur_op_idx+1 == num_ops || ops[cur_op_idx+1].second.fuse_chain_idx != meta.fuse_chain_idx);
			cur_op_idx++;
			continue;
		}
		...
		while (nxt_op_idx < num_ops && ops[nxt_op_idx].second.fuse_chain_idx == meta.fuse_chain_idx) {
			nxt_op_idx++;
		}
...
TBSched Transpiler::get_threadblock_schedule(const tb::Graph& tb_graph) {
	// Currently in Mirage, the output tensor of a threadblock level input op must have
	// after_accum = False. We first check against this condition
	...
	std::unordered_map<tb::TBOperator const*, bool> is_fused_with_next;
	{
		...
		for (tb::TBOperator* const op : tb_graph.operators) {
			...
			for (tb::STensor const& stensor : Combine(op->input_tensors, op->output_tensors)) {
		...
		// the previous operator, when the previous operator has only one consumer
		// for (tb::TBOperator* const op : tb_graph.operators) {
		// 	if ((op->op_type == type::TB_EXP_OP || op->op_type == type::TB_FORLOOP_ACCUM_OP)) {
		// 		const tb::STensor &input0 = op->input_tensors.at(0);
		// 		tb::TBOperator* prev_op = input0.owner_op;
		// 		// Don't fuse with an input op with forloop_dim = -1
		// 		if (prev_op->op_type == type::TB_INPUT_OP && dynamic_cast<tb::TBInputOp*>(prev_op)->forloop_dim == -1) {
		// 			continue;
		// 		}
	...
	// Generate `loop_nodes`
	{
		...
		for (tb::TBOperator* const op : tb_graph.operators) {
			...
			if (!op->input_tensors.at(0).after_accum) {
				if (is_fused_with_prev[op]) {
					...
					op2meta[op].pos_in_fuse_chain = op2meta[prev_op].pos_in_fuse_chain + 1;
				} else {
					int res = 0;
					for (tb::STensor const& input : op->input_tensors) {
		...
		vector<std::pair<tb::TBOperator const*, OpMeta>> all_ops(op2meta.begin(), op2meta.end());
...

	{
		...
		// Calculate the level of each operator
		for (tb::TBOperator* const op : tb_graph.operators) {
			if (op->op_type == type::TB_INPUT_OP || op->op_type == type::TB_FORLOOP_ACCUM_OP) {
				op2meta[op] = {0, next_fuse_chain_idx++, 0};
			}
		}
		for (tb::TBOperator* const op : tb_graph.operators) {
	...
	// Some sanity checks
	auto count_num_operators = [](const std::vector<TBSchedNode>& nodes) {
		size_t res = 0;
	...
	assert(count_num_operators(sched.pre_loop_nodes) + count_num_operators(sched.loop_nodes) + count_num_operators(sched.post_loop_nodes) == tb_graph.operators.size());
