...
// Let's partition the time axis into intervals, and each interval is a time unit.
// A tensor with (alloc_time, free_time) = (l, r) means that, the tensor is allocated
// at the beginning of the l-th time unit, and freed at the beginning of the r-th time unit,
// which means that within the time interval [l, r), the tensor is allocated and can be used.
struct TensorDecl {
	sguid_t sguid;
	size_t phy_size;
	int alloc_time;
...
public:
	...
	virtual void declare_tensor(const TensorDecl& tensor_decl) = 0;
...
public:
	void declare_tensor(const TensorDecl& t) override {
		assert(t.phy_size != 0 && "Trying to allocate memory for a STensor with a size of 0");
	...
	AllocResult get_allocation() override {
		...
		for (const Event& event : events) {
			if (event.type == event_t::ALLOC) {
				// Check whether there is enough free space for the new tensor. If there is not, allocate more memory
				bool is_avail_space = std::any_of(free_ranges.begin(), free_ranges.end(), [&](const Range& range) {
					return range.second - range.first >= event.phy_size;
				});
				if (!is_avail_space) {
					auto last = free_ranges.rbegin();
					size_t last_chunk_size = !free_ranges.empty() && last->second == cur_peak_usage ? last->second - last->first : (size_t)0;
					size_t new_peak_usage = cur_peak_usage + (event.phy_size - last_chunk_size);
					rise_peak_usage(new_peak_usage);
				}
				Range selected_range = select_range(event.phy_size);
				assert(addrs.count(event.stensor_guid) == 0 && "The STensor has been allocated before");
				assert(selected_range.second - selected_range.first == event.phy_size && "Logic error: the size of the selected range is not equal to the size of the tensor");
...
protected:
	Range select_range(size_t size) override {
		auto it = std::find_if(free_ranges.begin(), free_ranges.end(), [&](const Range& range) {
			return range.second - range.first >= size;
		});
		assert(it != free_ranges.end() && "No enough free space for the new tensor");
...
protected:
	Range select_range(size_t size) override {
		...
		for (const Range& range : free_ranges) {
		...
		assert(best_fit_size != std::numeric_limits<size_t>::max() && "No enough free space for the new tensor");
...
AllocResult plan_memory(vector<TensorDecl> const &tensor_decls) {
	...
	final_result.peak_memory_usage = std::numeric_limits<decltype(final_result.peak_memory_usage)>::max();
	...
	for (std::shared_ptr<AbstractMemoryPlanner> &planner : planners) {
		for (const TensorDecl& tensor_decl : tensor_decls) {
...
TBMemoryPlan Transpiler::get_threadblock_memory_plan(tb::Graph const& tb_graph, const TBSched& tb_sched) {
	...
	auto get_phy_size = [&](const tb::STensor &stensor) {
		STensorMeta const& stensor_meta = stensor_metas.at(stensor.guid);
		return stensor_meta.num_phy_elems * type::get_datatype_size(stensor.data_type);
	};
	auto find_first_used_time = [](sguid_t sguid, vector<TBSchedNode> const &nodes, int time_delta) -> int {
		...
		for (size_t i = 0; i < nodes.size(); ++i) {
			...
			for (tb::STensor const& input_tensor : node.ops.front().first->input_tensors) {
	...
	auto find_last_used_time = [](sguid_t sguid, vector<TBSchedNode> const &nodes, int time_delta) -> int {
	...
	auto find_earlist_free_time = [&find_last_used_time](sguid_t sguid, vector<TBSchedNode> const &nodes, int time_delta) -> int {
		...
		while (last_used_time < (int)nodes.size() && nodes[last_used_time].type != tb_sched_node_t::SYNCTHREADS) {
			++last_used_time;
		}
	...
	for (int i = 0; i < (int)tb_sched.pre_loop_nodes.size(); ++i) {
		...
		int earlist_free_time = find_earlist_free_time(stensor.guid, tb_sched.post_loop_nodes, 2*T);
	...
	for (int i = 0; i < (int)tb_sched.loop_nodes.size(); ++i) {
		...
		if (last_op->op_type == type::TB_FORLOOP_ACCUM_NO_RED_OP) {
			...
			int earlist_free_time = find_earlist_free_time(accum.guid, tb_sched.post_loop_nodes, 2*T);
			if (last_op_meta.is_accum_in_reg) {
				int first_used_time = find_first_used_time(accum.guid, tb_sched.post_loop_nodes, 2*T);
				assert (first_used_time != -1 && "An accumulator is not used after the for loop");
				tensor_decls.push_back({accum.guid, phy_size, first_used_time, earlist_free_time});
			} else {
				tensor_decls.push_back({accum.guid, phy_size, T-1, earlist_free_time});	// Use T-1 here since we should clear the accum
			}
		}
	}
	...
	for (int i = 0; i < (int)tb_sched.loop_nodes.size(); ++i) {
		...
		if (last_op->op_type != type::TB_FORLOOP_ACCUM_NO_RED_OP) {
			for (tb::STensor const& output_tensor : last_op->output_tensors) {
				...
				int earlist_free_time = find_earlist_free_time(output_tensor.guid, tb_sched.loop_nodes, T);
				assert (earlist_free_time != -1 && "An intermediate tensor produced in the for loop is never used");
				tensor_decls.push_back({output_tensor.guid, phy_size, i+T, earlist_free_time});
			}
		}
	}
	...
	for (int i = 0; i < (int)tb_sched.post_loop_nodes.size(); ++i) {
		...
		for (tb::STensor const& output_tensor : last_op->output_tensors) {
			size_t phy_size = get_phy_size(output_tensor);
			int earlist_free_time = find_earlist_free_time(output_tensor.guid, tb_sched.post_loop_nodes, 2*T);
			assert (earlist_free_time != -1 && "An intermediate tensor produced after the for loop is never used");
