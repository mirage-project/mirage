...
static int find_innermost_dim(const std::vector<size_t> &strides) {
...
void Transpiler::resolve_dtensor_meta() {
	...
	auto process_dtensor = [&](kn::DTensor const *dtensor) {
		if (processed_guids.count(dtensor->guid)) return;
		processed_guids.insert(dtensor->guid);
		all_dtensors.push_back(dtensor);
	};
	for (kn::KNOperator *const op : this->g->operators) {
		for (const kn::DTensor &dtensor : op->input_tensors)
			process_dtensor(&dtensor);
		for (const kn::DTensor &dtensor : op->output_tensors)
			process_dtensor(&dtensor);
	}

	// Get the max guid
	guid_t max_guid = std::numeric_limits<guid_t>::min();
	for (const kn::DTensor *dtensor : all_dtensors)
	...
	for (kernel::KNOperator *const op : this->g->operators) {
		if (op->op_type == type::KN_INPUT_OP) {
			const vector<size_t> &cur_stride = this->input_strides[cur_input_idx];
			const kernel::DTensor &tensor = op->output_tensors.at(0);
			if (tensor.num_dims != (int)cur_stride.size()) {
				printf("The number of dimensions of the stride of the %dth tensor (%ld) does not match the tensor's num_dims (%d)\n",
					cur_input_idx, cur_stride.size(), tensor.num_dims);
			...
			assert(innermost_dim == (int)cur_stride.size()-1 && "Currently only row-major layout is supported for input tensors");
	...
	for (const kn::DTensor *dtensor : all_dtensors) {
		if (dtensor->guid == max_guid) {
			const vector<size_t> &cur_stride = this->output_stride;
			if (dtensor->num_dims != (int)cur_stride.size()) {
				printf("The number of dimensions of the stride of the output tensor (%ld) does not match the tensor's num_dims (%d)\n",
					cur_stride.size(), dtensor->num_dims);
				throw std::runtime_error("Stride size mismatch");
			}
			...
			assert(innermost_dim == (int)cur_stride.size()-1 && "Currently only row-major layout is supported for output tensors");
	...
	for (const kn::DTensor *dtensor : all_dtensors) {
		...
		if (meta.is_input || meta.is_output) continue;
...
void Transpiler::resolve_stensor_meta() {
	...
	for (kernel::KNOperator *const kn_op : this->g->operators) {
		if (kn_op->op_type != type::KN_CUSTOMIZED_OP) continue;
		kernel::KNCustomizedOp *const cur_kn_op = dynamic_cast<kernel::KNCustomizedOp *>(kn_op);
    ...
    auto process_stensor = [&](tb::STensor const *stensor) {
      if (processed_guids.count(stensor->guid)) return;
      processed_guids.insert(stensor->guid);
      all_stensors.push_back(stensor);
    };
    for (tb::TBOperator *const op : cur_tb_g->operators) {
      for (const tb::STensor &stensor : op->input_tensors)
        process_stensor(&stensor);
      for (const tb::STensor &stensor : op->output_tensors)
        process_stensor(&stensor);
    }
    ...
    // tensors, we may use deep first search to find the best layout for stensors
    // TODO(intlsy): Support non-row-major layout for global tensors

    // Swizzle some dimensions if needed
    auto swizzle_second_last_dim = [&](const tb::STensor &stensor) {
      ...
      cur_metas[stensor.guid].swizzled_dims.push_back(num_dims - 2);
		...
		for (tb::TBOperator *const op : cur_tb_g->operators) {
          ...
          if (this->config.target_cc >= GPU_CC::T4) {
            ...
            // Use UniversalCopy<uint32_t>, still need to swizzle the second last dim
            swizzle_second_last_dim(cur_op->input_tensors.at(0));
            swizzle_second_last_dim(cur_op->input_tensors.at(1));
          }
          ...
          const tb::STensor& input = op->input_tensors.at(0);
          const tb::STensor& output = op->output_tensors.at(0);
          ...
          assert(false && "Not supported now");
				...
				default:
					assert(false && ("TB op not supported: " + std::to_string(op->op_type)).c_str());
			}
		}
    ...
    for (tb::STensor const *stensor_ptr : all_stensors) {
      ...
      const tb::STensor &stensor = *stensor_ptr;
      size_t cur_stride = 1;
      for (int i = stensor.num_dims - 1; i >= 0; --i) {
		...
		for (const auto &[guid, meta] : cur_metas) {
