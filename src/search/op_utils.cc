...
std::shared_ptr<AbstractExpr> get_abstract_expr(type::TBOperatorType op,
  ...
  switch (op) {
    ...
    case type::TBOperatorType::TB_RMS_NORM_OP: {
      return abstract_expr_make_div(opd, abstract_expr_make_rms(tensor.dim[tensor.num_dims - 1], opd));
    }
...
std::shared_ptr<AbstractExpr>
  ...
  switch (op) {
    ...
    case type::TBOperatorType::TB_FORLOOP_ACCUM_RED_LD_RMS_OP: {
      assert(opds.size() == 1);
      std::shared_ptr<TensorDimExpr> reduction_size_expr = dim_expr_make_mul(g.forloop_range.dim_expr, tensors[0].dims[tensors[0].dims.size() - 1].dim_expr);
    ...
    case type::TBOperatorType::TB_FORLOOP_ACCUM_REDTOX_LD_SUM_OP: {
      assert(opds.size() == 1);
      std::shared_ptr<TensorDimExpr> reduction_size_expr = dim_expr_make_mul(g.forloop_range.dim_expr, tensors[0].dims[tensors[0].dims.size() - 1].dim_expr);
      reduction_size_expr = dim_expr_make_div(reduction_size_expr, dim_expr_make_const(g.reduction_dimx));
      int reduction_size = get_dimension_size_lower_bound(reduction_size_expr);
      return abstract_expr_make_red(reduction_size, opds[0]);
    }
    default: {
