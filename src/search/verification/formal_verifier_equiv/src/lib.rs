...
pub fn rules() -> Vec<Rewrite> {
   ...
   let mut rules5 = vec![
        ...
        rw!("bc-div-combine";
            "(combine (bc_div (partition ?t0 ?d0 ?d1 ?i0) (partition ?t1 ?d2 ?d3 ?i1)) ?d4 ?d5)" 
            => "(bc_div ?t0 ?t1)" 
            if is_equal(&["?d0", "?d2", "?d4"]) 
            if is_equal(&["?d1", "?d3", "?d5"]) 
            if is_equal(&["?i0", "?i1"])
...
fn check_equiv(expr1: *const c_char, expr2: *const c_char) -> bool {
    ...
    egraph.dot().to_svg("egraph.svg").unwrap();
