...
OutputMatch FormalVerifier::verify(kernel::Graph const &graph) {
  ...
  auto verify_with_match = [&](OutputMatch const &match) {
    for (size_t i = 0; i < match.size(); i++) {
      ...
      bool is_equiv = check_equiv(
          input_exprs[i].c_str(), graph_exprs[match[i]].c_str(), false);
      if (!is_equiv) {
        return false;
      }
    }
...
OutputMatch
    FormalVerifier::verify_symbolic_graph(SymbolicKNGraph const &graph) {
  ...
  auto is_shape_match = [&](std::vector<int> const &shape_std,
                            SymbolicDTensor const &symbolic_tensor) {
    std::vector<int> shape_symbolic;
    for (size_t i = 0; i < symbolic_tensor.dims.size(); i++) {
      shape_symbolic.push_back(
          get_value_with_bool_vars_zero_others_random(symbolic_tensor.dims[i]));
    }
  ...
  auto verify_with_match = [&](OutputMatch const &match) {
    for (size_t i = 0; i < match.size(); i++) {
      ...
      bool is_equiv = check_equiv(
          input_exprs[i].c_str(), graph_exprs[match[i]].c_str(), true);
      if (!is_equiv) {
        return false;
      }
    }
...
std::vector<std::string> get_concrete_exprs(SymbolicKNGraph const &graph,
  ...
  auto calc_stensor_exprs = [&](SymbolicTBGraph const &graph,
    ...
    bool is_forloop_greater_than_one = [&] {
      for (auto const &op : graph.operators) {
        if (op.op_type == type::TB_INPUT_OP) {
          if (std::static_pointer_cast<TBInputOpArgs const>(op.args)
                  ->input_map.to_legacy_forloop_dim() >= 0) {
            return true;
          }
        }
      }
    ...
    std::vector<std::string> parallel_dim =
        vector_map(graph.grid_dim, [&](SymbolicTensorDim const &dim) {
          assert(dim->is_var());
          return dim->to_string();
        });
    std::vector<int> dummy_dim_size =
        vector_map(graph.grid_dim, [&](SymbolicTensorDim const &dim) {
          assert(dim->is_var());
    ...
    int dummy_fsize =
        std::static_pointer_cast<TensorDimVar const>(graph.forloop_range)
            ->index +
        2;
    for (size_t i = 0; i < graph.operators.size(); ++i) {
      switch (graph.operators[i].op_type) {
        case type::TBOperatorType::TB_INPUT_OP: {
          auto args = std::static_pointer_cast<TBInputOpArgs const>(
              graph.operators[i].args);
          ...
          for (size_t j = 0; j < legacy_imap.size(); ++j) {
            if (legacy_imap[j] >= 0) {
              ...
              a = "(partition " + a + " " + data_dim[axis] + " " +
                  parallel_dim[j] + " " + std::to_string(dummy_dim_size[j]) +
                  ")";
            } else {
              a = "(replicate " + a + " " + parallel_dim[j] + " " +
                  std::to_string(dummy_dim_size[j]) + ")";
            }
          }
          if (is_forloop_greater_than_one) {
            if (legacy_forloop >= 0) {
              ...
              a = "(partition " + a + " " + data_dim[axis] + " " +
                  forloop_range + " " + std::to_string(dummy_fsize) + ")";
            } else {
              a = "(replicate " + a + " " + forloop_range + " " +
                  std::to_string(dummy_fsize) + ")";
            }
          }
        ...
        case type::TBOperatorType::TB_OUTPUT_OP: {
          auto args = std::static_pointer_cast<TBOutputOpArgs const>(
              graph.operators[i].args);
        ...
        case type::TBOperatorType::TB_FORLOOP_ACCUM_REDTOX_LD_SUM_OP: {
          ...
          int reduce_degree = (int)std::static_pointer_cast<TensorDimVar const>(
                                  graph.reduction_degree)
                                  ->index;
          a = "(partition " + a + " " + data_dim[0] + " " + reddim + " " +
              std::to_string(reduce_degree) + ")";
          a = "(reduce " + a + " " + reddim + ")";
        ...
        default: {
          std::cerr << "Unsupported operator type: "
                    << json(graph.operators[i].op_type) << std::endl;
          assert(false && "Unsupported operator type");
        }
      }
    }
  ...
  auto calc_dtensor_exprs = [&](SymbolicKNGraph const &graph) {
    ...
    for (size_t i = 0; i < graph.operators.size(); ++i) {
      switch (graph.operators[i].op_type) {
        ...
        case type::KNOperatorType::KN_CUSTOMIZED_OP: {
          std::vector<std::string> input_exprs = vector_map(
              graph.input_indices[i], [&](int i) { return tensor_exprs[i]; });
          std::vector<std::string> output_exprs = calc_stensor_exprs(
              std::static_pointer_cast<KNCustomizedOpArgs const>(
                  graph.operators[i].args)
                  ->tb_graph_template,
              input_exprs);
          tensor_exprs.insert(
              tensor_exprs.end(), output_exprs.begin(), output_exprs.end());
          break;
        }
        default: {
          assert(false && "Unsupported operator type");
...
static std::optional<SymbolicTBGraph> rebuild_tb_with_concrete_maps(
  ...
  result.grid_dim.assign(tb.grid_dim.begin(),
                         tb.grid_dim.begin() + std::min((int)tb.grid_dim.size(),
                                                        actual_num_grid_dims));
  ...
  for (size_t i = 0; i < tb.operators.size(); ++i) {
    ...
    if (op.op_type == type::TBOperatorType::TB_INPUT_OP) {
      ...
      if (!result.add_input(
              args->dtensor, input_maps[input_idx], forloop_dims[input_idx])) {
        return std::nullopt;
      }
      ++input_idx;
      ...
      if (!result.add_output(
              tb_input_index, output_maps[output_idx], args->epilogue)) {
        return std::nullopt;
      }
      ++output_idx;
    } else {
...
static SymbolicKNGraph rebuild_kn_with_tb(SymbolicKNGraph const &kn,
  ...
  for (size_t i = 0; i < kn.operators.size(); ++i) {
    ...
    if (op.op_type == type::KNOperatorType::KN_CUSTOMIZED_OP &&
      ...
      result.add_input(args->input_dims,
                       args->input_strides,
                       args->data_type,
                       args->layout,
                       args->input_map);
...
// Generate maps where each grid dim maps to -1 (replicate) or a unique data
// dim. No two grid dims map to the same data dim within one map.
static void enumerate_maps(int num_dims,
                           int num_data_dims,
...
// Check if this combo is the lexicographically smallest under grid-dim
// permutations.
...
static std::vector<MapCombo>
    precompute_canonical_combos(int num_grid_dims,
...
std::vector<std::pair<SymbolicKNGraph, OutputMatch>>
  ...
  // Shape pre-check: set all map vars to 0 (replicate), check dims
  {
    ...
    for (size_t i = 0; i < shapes_std.size(); ++i) {
      ...
      for (auto const &dim : output_tensors[i % output_tensors.size()].dims) {
        shape_symbolic.push_back(
            get_value_with_bool_vars_zero_others_random(dim));
      }
  ...
  for (size_t op_idx = 0; op_idx < graph.operators.size(); ++op_idx) {
    if (graph.operators[op_idx].op_type !=
        type::KNOperatorType::KN_CUSTOMIZED_OP) {
      continue;
    }
    auto args = std::static_pointer_cast<KNCustomizedOpArgs const>(
        graph.operators[op_idx].args);
    ...
    for (size_t i = 0; i < tb.operators.size(); ++i) {
      ...
      } else if (tb.operators[i].op_type ==
                 type::TBOperatorType::TB_OUTPUT_OP) {
        auto a =
            static_cast<TBOutputOpArgs const *>(tb.operators[i].args.get());
    ...
    // Constraint: (input_i, data_dim_d) must have same map as (input_j,
    // data_dim_e)
    ...
    std::vector<MapConstraint> map_constraints;
    {
      ...
      auto find_input_dim =
          [&](SymbolicTensorDim const &target) -> std::pair<int, int> {
    ...
    for (auto const &info : input_infos) {
      input_data_dims.push_back(info.num_data_dims);
    }
    std::vector<int> output_data_dims;
    for (auto const &info : output_infos) {
      output_data_dims.push_back(info.num_data_dims);
    }
    ...
    std::vector<MapCombo> const *combos_ptr;
    {
      ...
      if (it == combo_cache.end()) {
        it = combo_cache
                 .emplace(key,
                          precompute_canonical_combos(
                              num_grid_dims, input_data_dims, output_data_dims))
                 .first;
      }
    ...
    // Key includes op types + KN input shapes to avoid cross-workload
    // collisions.
    ...
    for (auto const &combo : combos) {
      ...
      auto collapsed_key = std::make_tuple(kn_input_shapes,
                                           op_type_seq,
                                           collapsed_input_maps,
                                           combo.forloop_dims,
                                           collapsed_output_maps);
      {
      ...
      auto concrete_tb = rebuild_tb_with_concrete_maps(tb,
                                                       collapsed_input_maps,
                                                       combo.forloop_dims,
                                                       collapsed_output_maps,
      ...
      SymbolicKNGraph concrete_kn =
          rebuild_kn_with_tb(graph, op_idx, *concrete_tb);
