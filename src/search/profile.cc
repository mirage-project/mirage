...
std::string generate_cuda_filename_unique(const std::string& base_dir) {
  auto now = std::chrono::system_clock::now();
  auto time_t = std::chrono::system_clock::to_time_t(now);
  auto ms = std::chrono::duration_cast<std::chrono::milliseconds>(
      now.time_since_epoch()) % 1000;
...
static std::string cache_dir()  { return "generated_cuda/cache"; }
static std::string cache_file() { return cache_dir() + "/profile_cache.json"; }
  ...
  try {
    ...
    if (j.contains("compile_entries")) {
      for (auto const &[hkey, val] : j["compile_entries"].items()) {
        ...
        ce.input_num_elements = val["input_num_elements"].get<std::vector<size_t>>();
        ce.output_alloc_sizes = val["output_alloc_sizes"].get<std::vector<size_t>>();
...
ProfileCompileResult profile_compile(kernel::Graph *graph) {
  ...
  try {
    ...
    for (size_t i = 0; i < transpile_result.output_directives.size(); i++) {
      compiled.output_alloc_sizes.push_back(transpile_result.output_directives[i].alloc_size);
    }
    ...
    // Cache save (success)
    {
      ...
      std::error_code ec;
      std::filesystem::copy_file(compiled.so_file, so_dest,
...
ProfileResult profile_run(ProfileCompileResult const &compiled) {
  ...
  if (!compiled.is_success) {
    result.error_message = compiled.error_message.empty() ? "Compile failed" : compiled.error_message;
    return result;
  }
  ...
  try {
    ...
    if (ev_err != cudaSuccess) {
      result.error_message = "cudaEventCreate(start) failed: " + std::string(cudaGetErrorString(ev_err));
      goto run_cleanup;
    }
    ev_err = cudaEventCreate(&stop_event);
    if (ev_err != cudaSuccess) {
      result.error_message = "cudaEventCreate(stop) failed: " + std::string(cudaGetErrorString(ev_err));
      cudaEventDestroy(start_event);
      goto run_cleanup;
    }
    ev_err = cudaEventRecord(start_event, stream);
    if (ev_err != cudaSuccess) {
      result.error_message = "cudaEventRecord(start) failed: " + std::string(cudaGetErrorString(ev_err));
    ...
    if (ev_err != cudaSuccess) {
      result.error_message = "cudaEventRecord(stop) failed: " + std::string(cudaGetErrorString(ev_err));
      cudaEventDestroy(stop_event);
      cudaEventDestroy(start_event);
      goto run_cleanup;
    }
    ...
    if (ev_err != cudaSuccess) {
      result.error_message = "cudaEventSynchronize(stop) failed: " + std::string(cudaGetErrorString(ev_err));
      cudaEventDestroy(stop_event);
      cudaEventDestroy(start_event);
      goto run_cleanup;
    }
    ...
    cudaEventDestroy(start_event);
    if (ev_err != cudaSuccess) {
      result.error_message = "cudaEventElapsedTime failed: " + std::string(cudaGetErrorString(ev_err));
      goto run_cleanup;
    }
