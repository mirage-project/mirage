...
SymbolicTBGraph::SymbolicTBGraph() :
  grid_dim({SymbolicTensorDim(std::make_shared<TensorDimVar>(next_dim_variable_index++)),
              SymbolicTensorDim(std::make_shared<TensorDimVar>(next_dim_variable_index++)),
              SymbolicTensorDim(std::make_shared<TensorDimVar>(next_dim_variable_index++))}),
...
bool SymbolicTBGraph::add_operator(type::TBOperatorType op_type, std::vector<int> input_indices) {
  ...
  switch (op_type) {
    ...
    case type::TBOperatorType::TB_CONCAT_2_OP: {
      ...
      this->operators.push_back(SymbolicTBOp(op_type, std::make_shared<TBConcatOpArgs>(concat_dim)));
      {
    ...
    case type::TBOperatorType::TB_FORLOOP_ACCUM_REDTOX_LD_SUM_OP: {
      ...
      this->operators.push_back(SymbolicTBOp(op_type));
      {
        ...
        if (op_type == type::TBOperatorType::TB_FORLOOP_ACCUM_RED_LD_SUM_OP ||
          ...
          dim_templates[dim_templates.size() - 1] = SymbolicTensorDim(std::make_shared<TensorDimConst>(1));
        } else if (op_type == type::TBOperatorType::TB_FORLOOP_ACCUM_REDTOX_LD_SUM_OP) {
          dim_templates[dim_templates.size() - 1] = SymbolicTensorDim(std::make_shared<TensorDimConst>(this->reduction_dimx));
        }
    ...
    case type::TBOperatorType::TB_MATMUL_OP: {
      ...
      }
      {
        ...
        for (size_t i = 0; i < A.dims.size() - 2; i++) {
          constraints.push_back(make_equal_constraint(A.dims[i], SymbolicTensorDim(std::make_shared<TensorDimConst>(1))));
          constraints.push_back(make_equal_constraint(B.dims[i], SymbolicTensorDim(std::make_shared<TensorDimConst>(1))));
        }
    ...
    case type::TBOperatorType::TB_REDUCTION_2_OP: {
      ...
      this->operators.push_back(SymbolicTBOp(op_type, std::make_shared<TBReductionOpArgs>(reduction_dim, 1)));
      {
        std::vector<SymbolicTensorDim> dim_templates = A.dims;
        dim_templates[reduction_dim] = SymbolicTensorDim(std::make_shared<TensorDimConst>(1));
    ...
    case type::TBOperatorType::TB_REDUCTION_2_TO_DIMX_OP: {
      ...
      this->operators.push_back(SymbolicTBOp(op_type, std::make_shared<TBReductionOpArgs>(reduction_dim, this->reduction_dimx)));
      {
        std::vector<SymbolicTensorDim> dim_templates = A.dims;
        dim_templates[reduction_dim] = SymbolicTensorDim(std::make_shared<TensorDimConst>(this->reduction_dimx));
        SymbolicSTensor B(dim_templates, A.after_accum);
        this->tensors.push_back(B);
      }
      this->input_indices.push_back(input_indices);
...
bool SymbolicTBGraph::add_input(SymbolicDTensor dtensor, int3 input_map, int forloop_dim) {
  // create op template
  ...
  for (int d = 0; d < 3; ++d) {
    ...
    if (dim_idx >= 0) {
      dim_templates.push_back(SymbolicTensorDim(std::make_shared<TensorDimDiv>(dim_templates[dim_idx].dim_expr, grid_dim[d].dim_expr)));
    }
  }
  if (forloop_dim >= 0) {
    dim_templates[forloop_dim] = SymbolicTensorDim(std::make_shared<TensorDimDiv>(dim_templates[forloop_dim].dim_expr, forloop_range.dim_expr));
  }
...
bool SymbolicTBGraph::add_output(int input_index, int3 output_map, int forloop_dim, mirage::type::TBEpilogueType epilogue_type) {
  ...
  for (int d = 0; d < 3; ++d) {
    ...
    if (dim_idx >= 0) {
      dim_templates[dim_idx] = SymbolicTensorDim(std::make_shared<TensorDimMul>(dim_templates[dim_idx].dim_expr, grid_dim[d].dim_expr));
    }
  }
  if (forloop_dim >= 0) {
    dim_templates[forloop_dim] = SymbolicTensorDim(std::make_shared<TensorDimMul>(dim_templates[forloop_dim].dim_expr, forloop_range.dim_expr));
  }
...
bool SymbolicKNGraph::add_operator(type::KNOperatorType op_type, std::vector<int> input_indices) {
  switch (op_type) {
    ...
    case type::KNOperatorType::KN_REDUCTION_2_OP: {
      ...
      this->operators.push_back(SymbolicKNOp(op_type, std::make_shared<KNReductionOpArgs>(reduction_dim, reduction_dim_size)));
      {
        std::vector<SymbolicTensorDim> dim_templates = A.dims;
        dim_templates[reduction_dim] = SymbolicTensorDim(std::make_shared<TensorDimConst>(reduction_dim_size));
...
bool SymbolicKNGraph::add_customized_operator(SymbolicTBGraph tb_graph, std::vector<int> input_indices) {
  this->operators.push_back(SymbolicKNOp(type::KNOperatorType::KN_CUSTOMIZED_OP, std::make_shared<KNCustomizedOpArgs>(tb_graph)));
...
bool SymbolicKNGraph::add_input(std::vector<int> input_dims, std::vector<size_t> input_strides, int3 input_map) {
  this->operators.push_back(SymbolicKNOp(type::KNOperatorType::KN_INPUT_OP, std::make_shared<KNInputOpArgs>(input_strides, input_map)));
  {
    std::vector<SymbolicTensorDim> dim_templates;
    for (size_t i = 0; i < input_dims.size(); i++) {
      dim_templates.push_back(SymbolicTensorDim(std::make_shared<TensorDimConst>(input_dims[i])));
    }
...
bool SymbolicKNGraph::add_output(int input_index, std::vector<size_t> output_strides, int3 output_map) {
  this->operators.push_back(SymbolicKNOp(type::KNOperatorType::KN_OUTPUT_OP, std::make_shared<KNOutputOpArgs>(output_strides, output_map)));
