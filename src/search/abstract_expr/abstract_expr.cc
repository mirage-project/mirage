...
bool AbstractExpr::subexpr_to(
  ...
  auto check_subexpr = [&](DimVarAssignments const &assign) {
    ...
    // Get the expressions
    z3::expr expr1 = to_z3(c, assign), expr2 = other.to_z3(c, assign);
    ...
    s.add(forall(
        x, y, z, implies(subexpr(x, y) && subexpr(y, z), subexpr(x, z))));
...
z3::expr Var::to_z3(z3::context &c, DimVarAssignments const &assign) const {
  z3::sort P = c.uninterpreted_sort("P");
...
z3::expr Add::to_z3(z3::context &c, DimVarAssignments const &assign) const {
  z3::sort P = c.uninterpreted_sort("P");
  z3::func_decl add = c.function("add", P, P, P);
  return add(lhs->to_z3(c, assign), rhs->to_z3(c, assign));
}
...
z3::expr Mul::to_z3(z3::context &c, DimVarAssignments const &assign) const {
  z3::sort P = c.uninterpreted_sort("P");
  z3::func_decl mul = c.function("mul", P, P, P);
  return mul(lhs->to_z3(c, assign), rhs->to_z3(c, assign));
}
...
z3::expr Div::to_z3(z3::context &c, DimVarAssignments const &assign) const {
  z3::sort P = c.uninterpreted_sort("P");
  z3::func_decl div = c.function("div", P, P, P);
  return div(lhs->to_z3(c, assign), rhs->to_z3(c, assign));
}
...
z3::expr Exp::to_z3(z3::context &c, DimVarAssignments const &assign) const {
  z3::sort P = c.uninterpreted_sort("P");
  z3::func_decl exp = c.function("exp", P, P);
  return exp(exponent->to_z3(c, assign));
}
...
z3::expr Silu::to_z3(z3::context &c, DimVarAssignments const &assign) const {
  z3::sort P = c.uninterpreted_sort("P");
  z3::func_decl silu = c.function("silu", P, P);
  return silu(a->to_z3(c, assign));
}
...
std::shared_ptr<AbstractExpr const> Var::simplify() const {
  return abstract_expr_make_var(name);
}

std::shared_ptr<AbstractExpr const> Add::simplify() const {
  return abstract_expr_make_add(lhs->simplify(), rhs->simplify());
}

std::shared_ptr<AbstractExpr const> Mul::simplify() const {
  return abstract_expr_make_mul(lhs->simplify(), rhs->simplify());
}

std::shared_ptr<AbstractExpr const> Div::simplify() const {
  std::shared_ptr<AbstractExpr const> lhs_simplified = lhs->simplify(),
                                      rhs_simplified = rhs->simplify();
  if (std::dynamic_pointer_cast<Div const>(lhs_simplified)) {
    std::shared_ptr<Div const> div =
        std::static_pointer_cast<Div const>(lhs_simplified);
    return abstract_expr_make_div(
        div->lhs, abstract_expr_make_mul(div->rhs, rhs_simplified));
  }
  if (std::dynamic_pointer_cast<Div const>(rhs_simplified)) {
    std::shared_ptr<Div const> div =
        std::static_pointer_cast<Div const>(rhs_simplified);
    return abstract_expr_make_div(
...
std::shared_ptr<AbstractExpr const> Exp::simplify() const {
  return abstract_expr_make_exp(exponent->simplify());
}

std::shared_ptr<AbstractExpr const> Silu::simplify() const {
  return abstract_expr_make_silu(a->simplify());
}

std::shared_ptr<AbstractExpr const> RMS::simplify() const {
  return abstract_expr_make_rms(reduction_degree, elems->simplify());
}

std::shared_ptr<AbstractExpr const> Red::simplify() const {
  std::shared_ptr<AbstractExpr const> summand_simplified = summand->simplify();
  if (std::dynamic_pointer_cast<Red const>(summand_simplified)) {
    std::shared_ptr<Red const> red =
        std::static_pointer_cast<Red const>(summand_simplified);
