...
NKICustomOPTranspileResult
  ...
  CodeKeeper code;
  code.e("@nki.jit(debug_kernel=True)");
  code.e("def $($):",
         func_name,
  ...
  int unroll_iterator_idx = -1;
  std::vector<unsigned int> grid_dim = to_vector(g.grid_dim);
  for (size_t i = 0; i < grid_dim.size(); ++i) {
    if (config.use_double_buffer && grid_dim[i] % 2 == 0) {
      unroll_iterator_idx = i;
    }
  }
  bool use_double_buffer = (unroll_iterator_idx != -1);
  std::vector<std::string> SPMD_iterators;
  for (size_t i = 0; i < grid_dim.size(); ++i) {
    std::string iterator_name = fmt("iter$", iterator_idx_counter++);
    if (i != unroll_iterator_idx) {
      code.e("for $ in nl.affine_range($):", iterator_name, grid_dim[i]);
    } else {
      code.e("for $ in nl.affine_range($):", iterator_name, grid_dim[i] / 2);
    }
    SPMD_iterators.push_back(iterator_name);
    code.inc_indent();
  }
  auto get_SPMD_iterator =
      [&](int idx, int total_buffer, int buffer_id) -> std::string {
    if (idx == unroll_iterator_idx) {
      return fmt("($ * $ + $)", SPMD_iterators[idx], total_buffer, buffer_id);
    } else {
      return SPMD_iterators[idx];
    }
  };
  // Initialize all accum stensors
  auto generate_loop_body = [&](int total_buffer, int buffer_id) {
    for (tb::TBOperator *tb_op : g.operators) {
      ...
      if (tb_op->op_type == type::TB_FORLOOP_ACCUM_NO_RED_OP) {
        ...
        code.e("$ = $",
               get_tensor_variable_name(stensor, buffer_id),
    ...
    std::unordered_set<mirage::type::GuidType> fused_tensor;
    for (tb::TBOperator *tb_op : g.operators) {
      ...
      switch (tb_op->op_type) {
            ...
            for (int j = 0; j < 3; ++j) {
              if (to_vector(imap)[j] == i) {
                ...
                index_terms.push_back(
                    fmt("$ * $",
                        get_SPMD_iterator(j, total_buffer, buffer_id),
                        scale_factor));
              }
            }
            if (forloop_dim == i) {
        ...
        case type::TB_MATMUL_OP: {
          ...
          std::string operand0 = get_tensor_variable_name(input0, buffer_id);
          std::string operand1 = get_tensor_variable_name(input1, buffer_id);
          ...
          std::string accumulator = [&](tb::STensor const &stensor) {
            ...
            for (tb::TBOperator *op : g.operators) {
              for (tb::STensor const &input : op->input_tensors) {
                if (input.guid == stensor.guid) {
                  if (op->op_type == type::TB_FORLOOP_ACCUM_NO_RED_OP) {
                    accumulator = get_tensor_variable_name(
                        op->output_tensors[0], buffer_id);
                  } else {
                    return std::string("");
                  }
                }
          ...
          if (accumulator.empty()) {
            code.e("$ = $",
                   get_tensor_variable_name(output, buffer_id),
                   tiled_matmul_function().get_invocation({
                       operand0,
                       operand1,
                       get_python_literal(meta0.partition_dim == num_dims - 1),
                       get_python_literal(meta1.partition_dim == num_dims - 1),
                       "dtype",
                   }));
          } else {
            fused_tensor.insert(output.guid);
        ...
        case type::TB_FORLOOP_ACCUM_NO_RED_OP: {
          ...
          if (!contains(fused_tensor, input.guid)) {
            code.e("$ += $",
                   get_tensor_variable_name(output, buffer_id),
                   get_tensor_variable_name(input, buffer_id));
          }
          break;
        }
        case type::TB_EXP_OP:
        ...
        case type::TB_MUL_SCALAR_OP: {
          ...
          string first_operand = get_tensor_variable_name(input, buffer_id);
        ...
        case type::TB_POW_OP: {
          ...
          string first_operand = get_tensor_variable_name(input0, buffer_id);
          string second_operand = get_tensor_variable_name(input1, buffer_id);
    ...
    for (tb::TBOperator *tb_op : g.operators) {
      if (tb_op->op_type == type::TB_FORLOOP_ACCUM_NO_RED_OP) {
        ...
        STensorMeta meta1 = stensor_metas.at(output.guid);
        if (meta0.partition_dim != meta1.partition_dim) {
          string var = get_tensor_variable_name(output, buffer_id);
          code.e(
              "$ = $", var, tiled_transpose_function().get_invocation({var}));
        }
      }
    }
  ...
  if (use_double_buffer) {
    // Generate code for double buffer
    generate_loop_body(2, 0);
    generate_loop_body(2, 1);
  } else {
    // Generate code for single buffer
    generate_loop_body(1, 0);
  }
