...
template<cublasComputeType_t COMPUTE_TYPE, typename TA, typename TB, typename TC>
static void gemm(
	// Arguments for matrix info
	TC* C, const TA* A, const TB* B,
	int m, int n, int k,
	size_t stride_m_A, size_t stride_k_A,
	size_t stride_k_B, size_t stride_n_B,
	size_t stride_m_C, size_t stride_n_C,
	...
	using alpha_beta_t = typename CublasComputeType2AlphaBetaType<COMPUTE_TYPE>::type;
	alpha_beta_t alpha = 1.0;
	alpha_beta_t beta = 0.0;
	cublasStatus_t status = cublasGemmStridedBatchedEx(
		cublas_handle_keeper.get(),
		trans_A ? CUBLAS_OP_T : CUBLAS_OP_N,
		trans_B ? CUBLAS_OP_T : CUBLAS_OP_N,
		m, n, k,
		&alpha,
		A, data_t2cuda_data_type_t<TA>(), stride_m_A+stride_k_A-1, batch_stride_A,
		B, data_t2cuda_data_type_t<TB>(), stride_k_B+stride_n_B-1, batch_stride_B,
		&beta,
		C, data_t2cuda_data_type_t<TC>(), stride_n_C, batch_stride_C,
		batch_size,
		COMPUTE_TYPE,
		CUBLAS_GEMM_DEFAULT
	);
	if (status != CUBLAS_STATUS_SUCCESS) {
		fprintf(stderr, "CUBLAS GEMM failed with status %d: %s (%s)\n", status, cublasGetStatusName(status), cublasGetStatusString(status));
		assert(false);
	}
}
