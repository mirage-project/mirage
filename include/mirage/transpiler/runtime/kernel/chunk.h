...
static __global__ void
    chunk_kernel_fwd(typename Config::T *__restrict__ out0,
  ...
  if (idx < Numel{}) {
    ...
    if (get<Config::CHUNK_DIM>(src_coord) < DST0_SIZE) {
      ...
      replace<Config::CHUNK_DIM>(src_coord,
                                 get<Config::CHUNK_DIM>(src_coord) - DST0_SIZE);
...
public:
  ...
  static void run(T *out0, T *out1, T const *in) {
    chunk_kernel_fwd<ChunkKernel<T,
                                 SrcLayout,
                                 Dst0Layout,
                                 Dst1Layout,
                                 CHUNK_SIZE,
                                 CHUNK_DIM>>
        <<<grid_shape, block_shape>>>(out0, out1, in);
  }
};
