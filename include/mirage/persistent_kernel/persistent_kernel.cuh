...
__device__ __forceinline__ bool
...
#ifdef MODE_ONLINE
  __device__ __forceinline__ bool prepare_next_batch(
  ...
  __device__ __forceinline__ bool prepare_next_batch(
      RuntimeConfig const &config, size_t iteration_num = 0) {
  ...
  __device__ __forceinline__ int get_rand_sched_id(
      size_t event_index, int worker_id, int num_workers, int num_schedulers) {
  ...
  __device__ __forceinline__ void get_first_last_ids(
  ...
  __device__ __forceinline__ void terminate_schedulers(RuntimeConfig config) {
    ...
    for (int i = 0; i < num_schedulers; i++) {
      ...
      size_t last_event_id = atom_add_release_gpu_u64(
          &config.sched_queue_next_free_event_id[i], 1);
      st_relaxed_gpu_u64(
          &config.sched_queues[i][last_event_id % config.per_sched_queue_len],
          0);
      // Use st.relaxed to make sure sched_queue updates are visible to
      // scheduler CTAs before incrementing its last_ready_event_id
      size_t old;
      do {
        ...
        //                 last_event_id + 1);
        old =
            atom_cas_release_gpu_u64(&config.sched_queue_last_ready_event_id[i],
                                     last_event_id,
                                     last_event_id + 1);
      ...
      if (queue_pos == queue_len) {
        ...
        if (threadIdx.x < num_loaded_tasks) {
          task_ids[threadIdx.x] = ld_relaxed_gpu_u64(
              &worker_queues[queue_idx]
                            [(next_task_pos[queue_idx] + threadIdx.x) %
        ...
        if (threadIdx.x == 0) {
          ...
          for (int i = 0; i < num_loaded_tasks; i++) {
            printf("[%d][FTCH] worker_id(%d) queue_idx(%d) next_task_pos(%llu, "
  ...
  __device__ __forceinline__ void execute_scheduler(RuntimeConfig config,
    ...
    if (threadIdx.x % 32 == 0 && warp_id < num_schedulers_per_sm) {
      int const sched_id =
          blockIdx.x * num_schedulers_per_sm + warp_id + offset;
      // if (threadIdx.x == 0) {
          ...
          for (size_t i = 0;
            ...
            for (size_t j = my_first_worker; j < my_last_worker; j++) {
              ...
              if (position_index < e.last_task_id) {
                ...
                // Use st.relaxed to make sure writes to worker_queues is
                // visible to worker CTAs before we increase its
                // last_ready_task_id
                atom_add_release_gpu_u64(
                    &config.worker_queue_last_ready_task_id[next_worker], 1);
  ...
  __global__ __launch_bounds__(WORKER_NUM_THREADS, 1) void persistent_kernel(
  ...
  extern "C" void init_persistent_kernel(std::vector<void *> meta_tensors,
    ...
    global_runtime_config.qo_indptr_buffer =
  ...
  extern "C" void launch_persistent_kernel() {
    ...
    if (global_runtime_config.split_worker_scheduler) {
      ...
      int num_sms_to_use =
          global_runtime_config.num_workers + num_schedulers / 4;
#ifdef USE_NVSHMEM
  ...
  extern "C" void finalize_persistent_kernel() {
    ...
    defined(MODE_ONLINE) // || defined(MODE_ONLINE_NOTOKEN)
