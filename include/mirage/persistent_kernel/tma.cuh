 ...
 __host__ inline void fill_tma_desc_by_task(CUtensorMap *tma_desc,
   ...
   switch (task_desc.task_type) {
     ...
     case TASK_PAGED_ATTENTION_HOPPER: 
     {
       ...
       int const total_head_groups = total_head_dims / head_dim / (num_q_heads + 2 * num_kv_heads);
       ...
       fill_tma_desc<T, B, M, S, 3>(tma_desc,
         ...
         uint64_t gmem_shape[4] = {static_cast<uint64_t>(num_pages),
                                   ...
                                   static_cast<uint64_t>(head_dim)};
         uint64_t gmem_stride[4] = {1, static_cast<uint64_t>(head_dim), static_cast<uint64_t>(total_head_groups * head_dim), static_cast<uint64_t>(page_size * total_head_groups * head_dim)};
         ...
         uint64_t gmem_shape[3] = {static_cast<uint64_t>(max_tokens),
                                   static_cast<uint64_t>(num_q_heads * total_head_groups),
                                   static_cast<uint64_t>(head_dim)};
         uint64_t gmem_stride[3] = {1, static_cast<uint64_t>(head_dim), static_cast<uint64_t>(num_q_heads * total_head_groups * head_dim)};
