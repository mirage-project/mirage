...
__device__ __forceinline__ void
  ...
  constexpr int TMA_TRANS_BYTES_RESIDUAL = HAS_RESIDUAL ? sizeof(T) * BATCH_SIZE * OUTPUT_ATOM_SIZE : 0;

  // using SM90_64x64x16_F32BF16BF16
  constexpr int NUM_ITER_N = (OUTPUT_SIZE + OUTPUT_ATOM_SIZE - 1) / OUTPUT_ATOM_SIZE;
  ...
  constexpr size_t SHARED_WEIGHT_BUFFER_OFFSET =
      (SHARED_INPUT_BUFFER_OFFSET + sizeof(T) * Kstages * BATCH_SIZE * TILE_SIZE + 1023) / 1024 * 1024;

  constexpr size_t SHARED_RESIDUAL_BUFFER_OFFSET = HAS_RESIDUAL ?
      (SHARED_WEIGHT_BUFFER_OFFSET +
      sizeof(T) * Kstages * TILE_SIZE * OUTPUT_ATOM_SIZE + 1023) / 1024 * 1024 : 0;

  constexpr size_t SHARED_MM_OUTPUT_BUFFER_OFFSET =
      (SHARED_RESIDUAL_BUFFER_OFFSET + sizeof(T) * BATCH_SIZE * OUTPUT_ATOM_SIZE * Kstages + 1023) / 1024 * 1024;
  ...
  constexpr size_t SHARED_RESIDUAL_BARRIER_OFFSET = HAS_RESIDUAL ?
      (SHARED_WEIGHT_BARRIER_OFFSET + 8 * Kstages + 7) / 8 * 8 : 0;
  ...
  constexpr size_t TOTAL_SHARED_MEMORY = SHARED_RESIDUAL_DONE_OFFSET + 8 * Kstages;
  ...
  // residual
  T *shared_residual = HAS_RESIDUAL ? (T *)(smem + SHARED_RESIDUAL_BUFFER_OFFSET) : nullptr;
  // output
  ...
  Barrier *residual_barrier = HAS_RESIDUAL ?
      reinterpret_cast<Barrier *>(smem + SHARED_RESIDUAL_BARRIER_OFFSET) : nullptr;
  Barrier *compute_done =
      reinterpret_cast<Barrier *>(smem + SHARED_COMPUTE_DONE_OFFSET);
  Barrier *residual_done =
      HAS_RESIDUAL ? reinterpret_cast<Barrier *>(smem + SHARED_RESIDUAL_DONE_OFFSET) : nullptr;
  ...
  if (warpgroup_id == NUM_WARPGROUPS - 1) {
    ...
    if (lane_id() == 0 && warp_idx == (NUM_WARPGROUPS * WARPGROUP_WARPS - 4)) {
      for (int output_atom_idx = 0; output_atom_idx < NUM_ITER_N; output_atom_idx++) {
        ...
        residual_smem.set_ptr(shared_residual + slot_residual * TMA_TRANS_BYTES_RESIDUAL / sizeof(T));
        set_barrier_transaction_bytes(residual_barrier[slot_residual],
                                      TMA_TRANS_BYTES_RESIDUAL);
        tma_residual.tma_cp_async(
            residual_barrier[slot_residual], residual_smem(0, 0), {output_atom_idx * OUTPUT_ATOM_SIZE, 0});
        ...
        for (int i = 0; i < NUM_ITER_K; i++) {
          ...
          int tma_coords_B[2] = {i * TILE_SIZE, output_atom_idx * OUTPUT_ATOM_SIZE};
