...
// MXFP4 TMA 2D descriptor - similar to NVFP4 but uses float_ue8m0_t scale factors
// The main difference is in the scale factor type (8-bit exponent vs 4-bit exponent+3-bit mantissa)
template <typename T,
          int B,
          int M,
          int S,
...
struct tma_2d_mxfp4 {
  ...
  __device__ inline tma_2d_mxfp4(CUtensorMap *desc_ptr) {
    this->desc_ptr = desc_ptr;
  }

  __host__ inline tma_2d_mxfp4(void *src) {
    CUtensorMap host_desc;
    create_tma_desc_mxfp4(&host_desc, src); // host-only function
...
#pragma unroll
  ...
  __device__ inline void launch_tma_store_async(T *smem_ptr,
                                                 int const (&tma_coords)[2]) const {
#if defined(MIRAGE_GRACE_HOPPER) || defined(MIRAGE_GRACE_BLACKWELL)
    uint64_t gmem_int_desc = reinterpret_cast<uint64_t>(desc_ptr);
    uint32_t smem_int_ptr =
        static_cast<uint32_t>(__cvta_generic_to_shared(smem_ptr));
  ...
  __device__ inline void launch_tma_reduce_add_async(
      T *smem_ptr, int const (&tma_coords)[2]) const {
#if defined(MIRAGE_GRACE_HOPPER) || defined(MIRAGE_GRACE_BLACKWELL)
    uint64_t gmem_int_desc = reinterpret_cast<uint64_t>(desc_ptr);
    uint32_t smem_int_ptr =
        static_cast<uint32_t>(__cvta_generic_to_shared(smem_ptr));

    asm volatile("cp.async.bulk.tensor.5d.shared::cluster.global.tile.add"
                 " [%0], [%1, {%3, %4}], [%2];"
                 :
                 : "r"(smem_int_ptr),
                   "l"(gmem_int_desc),
  ...
  // Host-side function to create TMA descriptor for MXFP4
  // MXFP4 uses the same swizzle settings as NVFP4 but with different scale factor handling
  __host__ static inline void create_tma_desc_mxfp4(CUtensorMap *tma_desc,
                                                    void *src) {
    static_assert(ROW_MAJOR == true);
    constexpr uint32_t tma_dim = 5;
    void *global_addr = src;

    // MXFP4 uses same data type as NVFP4 for the FP4 data
  ...
  // Get base pointer for TMA operations
  __device__ void *get_base_ptr() const {
    return desc_ptr;
  }
};
