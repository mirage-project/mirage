...
__device__ __forceinline__ void tcgen05_commit(int mbar_addr) {
    asm volatile("tcgen05.commit.cta_group::1.mbarrier::arrive::one.shared::cluster.b64 [%0];" :: "r"(mbar_addr) : "memory");
}

// Task-internal sync using a simple atomic counter
// Safe for repeated calls, no phase tracking needed
__device__ __forceinline__ void task_sync_atomic(volatile int* counter, int tid, int num_warps) {
    __syncwarp();
    if (tid % 32 == 0 && tid / 32 < num_warps) {
        // Arrive
        int old = atomicAdd((int*)counter, 1);
        // Wait for all warps
        while (*(volatile int*)counter < (old / num_warps + 1) * num_warps) {
            __nanosleep(10);
        }
    }
    __syncwarp();
...
__device__ __noinline__ void simple_linear_task_dummy(float* C, int M, int N) {
    ...
    for (int i = 0; i < 100; i++) a = fmaf(a, b, c);
    if (tid < M * N) C[tid] = a;
}

__device__ __noinline__ void simple_linear_task(
    const CUtensorMap& A_tm, const CUtensorMap& B_tm,
    ...
    constexpr int SMEM_OFFSET = 0;  // dynamic SMEM starts after static; no skip needed
    const int smem_base = __cvta_generic_to_shared(smem_buf) + SMEM_OFFSET;
    const int A_smem = smem_base;
    const int B_smem = smem_base + NUM_STAGES * TILE_BYTES_A;
    ...
    const int mma_bar = __cvta_generic_to_shared(&mbar_buf[NUM_STAGES]);
    const int done_bar = __cvta_generic_to_shared(&mbar_buf[2 * NUM_STAGES]);
    ...
    constexpr uint32_t idesc = (1U << 4) | (1U << 7) | (1U << 10)
        | ((uint32_t)(TILE_N >> 3) << 17) | ((uint32_t)(TILE_M >> 4) << 24);
    ...
    if (wid == 0 && elect_sync()) {
        ...
        for (int ki = 0; ki < num_k_tiles; ki++) {
            ...
            asm volatile("cp.async.bulk.tensor.2d.shared::cta.global.mbarrier::complete_tx::bytes [%0], [%1, {%2, %3}], [%4];"
                :: "r"(a_smem), "l"(&A_tm), "r"(ki * TILE_K), "r"(m_tile * TILE_M), "r"(tma_bar + stage * 8) : "memory");
            asm volatile("cp.async.bulk.tensor.2d.shared::cta.global.mbarrier::complete_tx::bytes [%0], [%1, {%2, %3}], [%4];"
                :: "r"(b_smem), "l"(&B_tm), "r"(ki * TILE_K), "r"(n_tile * TILE_N), "r"(tma_bar + stage * 8) : "memory");
        }
    }
    ...
    task_sync_atomic(&sync_counter, tid, 4);
    if (wid == 0) {
        asm volatile("tcgen05.dealloc.cta_group::1.sync.aligned.b32 %0, %1;" :: "r"(taddr), "r"(TILE_N));
    }
}
