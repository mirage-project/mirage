...
__device__ __forceinline__ void
    linear_nvfp4_swapAB_sm100_task_impl(CUtensorMap const *A_tmap,
...
#pragma nv_diag_suppress static_var_with_dynamic_init
  ...
  if (warp_id == TMA_WARP && elect_sync()) {
    ...
    auto issue_tma = [&](int iter_k,
      ...
      char const *SFB_src = SFB_ptr + (batch_tile * rest_k + off_k / 64) * 512;
...
// Standalone grid launcher: TMA descriptors arrive by value
// (__grid_constant__), tiles are grid-strided over (blockIdx.x, gridDim.x).
template <int MMA_N,
          int OUTPUT_SIZE,
          int REDUCTION_SIZE,
          int BLOCK_K,
