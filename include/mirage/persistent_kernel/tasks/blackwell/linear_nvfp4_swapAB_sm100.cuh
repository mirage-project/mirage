...
linear_nvfp4_swapAB_sm100_kernel(
  ...
  constexpr int TMEM_ALLOC_COLS = (TMEM_USED_COLS <= 128) ? 128 : ((TMEM_USED_COLS <= 256) ? 256 : 512);
...
#pragma nv_diag_suppress static_var_with_dynamic_init
  ...
  if (warp_id == TMA_WARP && elect_sync()) {
    ...
    type::bfloat16_t *out_smem_base = reinterpret_cast<type::bfloat16_t *>(smem_ptr + STAGE_SIZE * NUM_STAGES);
