...
// MXFP4 scale factor offset calculation for interleaved layout
// MXFP4 uses float_ue8m0_t (8-bit exponent, 0-bit mantissa) scale factors
...
// MXFP4 scale factor offset for per-tile swapAB layout
...
// Quantization kernel for MXFP4
// MXFP4 uses float_ue8m0_t as scale factor (8-bit exponent, 0-bit mantissa)
// This is different from NVFP4 which uses float_ue4m3_t (4-bit exponent, 3-bit mantissa)
...
quantize_mxfp4_sm100_task_impl(const void *__restrict__ input_ptr,
  ...
  static_assert(HIDDEN_SIZE % (GROUP_SIZE * 4) == 0,
                "HIDDEN_SIZE must be divisible by 128 for interleaved MXFP4 scales");
  ...
  // MXFP4 uses float_ue8m0_t scale factor (8-bit exponent)
  static_assert(std::is_same_v<SCALE_T, uint8_t>,
                "MXFP4 scales must be stored as ue8m0 bytes");
  ...
  const int output_q_row_offset = row_idx * (GLOBAL_STRIDE / 2);
...
#pragma unroll
  for (int group_idx = warp_idx * GROUPS_PER_WARP + subwarp_idx;
    ...
    // MXFP4 uses float_ue8m0_t scale factor (8-bit exponent, 0-bit mantissa)
    // This is different from NVFP4 which uses float_ue4m3_t
    const cutlass::float_ue8m0_t scale_quant(
