...
//   * GROUP_SIZE = 32 (MXFP4 spec) vs 16 for NVFP4 — twice as many e2m1 elements
//     share one scale, halving the number of scale bytes per row.
//   * Scale dtype is e8m0 (1-byte unbiased exponent). The scale is the
//     power-of-two ceiling of group_max / max_4bit, stored as `floor(log2(s)) + 127`.
//     Computed via frexpf so the result is exact and IEEE-correct without
...
__device__ __forceinline__ int interleaved_mxfp4_scale_offset(int row_idx,
                                                              int group_idx,
                                                              int num_k_outer,
                                                              int scale_outer_stride) {
  int row_in_block = row_idx & 127;
  // 2 MXFP4 scales per 64-K atom (1 scale per 32 K-elements; vec::2X uses NSF=2).
  // group_idx = scale_id along K. k_outer = group_idx >> 1; k_inner = group_idx & 1.
  return (row_idx >> 7) * num_k_outer * scale_outer_stride +
         (group_idx >> 1) * scale_outer_stride +
         (row_in_block & 31) * 16 +
         ((row_in_block >> 5) & 3) * 4 +
         (group_idx & 1);
}
...
// smallest power of two >= x. NaN / non-positive inputs map to 0x00 (subnormal).
...
quantize_mxfp4_sm100_task_impl(const void *__restrict__ input_ptr,
  ...
  static_assert(HIDDEN_SIZE % (GROUP_SIZE * 2) == 0,
                "HIDDEN_SIZE must be divisible by 64 (one SF atom = 2 MXFP4 scales = 64 K-elements)");
  ...
  const int subwarp_idx = lane_idx / SUBWARP_SIZE;  // == 0 (SUBWARP == WARP)
  ...
  const int num_k_outer = NUM_GROUPS_PER_ROW / 2;  // 2 MXFP4 scales per 64-K atom
...
#pragma unroll
  for (int group_idx = warp_idx * GROUPS_PER_WARP + subwarp_idx;
    ...
    // MXFP4 scale: smallest power of two such that group_max / scale <= max_4bit.
    const float raw_scale = valid_row ? group_max / max_4bit : 1.0f;
    const uint8_t scale_e8m0 = encode_e8m0(raw_scale);
    const float applied_scale = decode_e8m0(scale_e8m0);

    if (sublane_idx == 0 && (mma_n == 0 || valid_row)) {
      int sf_offset = (mma_n > 0)
          ? swapab_mxfp4_scale_offset(row_idx, group_idx, num_k_outer, mma_n)
          : interleaved_mxfp4_scale_offset(row_idx, group_idx, num_k_outer, scale_outer_stride);
      output_s[sf_offset] = scale_e8m0;
    }

    const float inv_scale = applied_scale > 0.0f ? 1.0f / applied_scale : 0.0f;
