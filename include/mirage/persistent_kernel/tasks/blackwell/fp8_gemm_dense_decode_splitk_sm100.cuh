 ...
 */
// Generated by ferret kernel-extractor on 2026-05-29 from kernel.cu
// of workspace tag v001. Source workspace: /home/muhengl/ferret/workspace7.
...
// SOTA beaten: MPK in-tree fp8_gemm_dense_mediumm @ <128,NS=3,NE=4>
//   qkv_a_M128 1.60x, gate_up_M128 1.18x (score=1.389).
//
// B36 FIX vs Mirage's original fp8_gemm_dense_decode_splitk_sm100.cuh:
//   The original producer warp used per-tile `s = ki % NS` + `ph` (toggled at
//   s==NS-1). When nk_slice % NS != 0 (qkv_a nk_slice=7 vs NS=3; o_proj
//   nk_slice=4 vs NS=3) the (slot,parity) stream becomes DISCONTINUOUS at
//   tile boundaries, causing producer/consumer mbarrier parity desync on
//   multi-wave runs (gate_up: 144 CTAs > 136 workers) -> deadlock /
//   cudaErrorLaunchFailure. Fix: drive bf/be from a continuous global k-tile
//   counter `gk` (producer) and `gki` (MMA warp, was already continuous).
//   Validated at rel err < 1e-2 for all 4 task.yaml configs including a
//   32-worker stress grid that forces multi-tile iteration.
...
// Called by fp8_gemm_dense_decode_splitk_sm100_task_impl below.
//
// B36 FIX applied here (vs Mirage's original task_impl_splitk_tpl):
//   - Producer warp: continuous `gk` drives `s = gk % NS` and
//     `ph = (gk / NS) & 1`, replacing the per-tile `ki % NS` + toggled `ph`.
//   - MMA warp: `php = (gki / NS) & 1` replaces a separate per-tile `ph`
//     toggle (gki was already continuous, so only the parity expression changed).
//   Both fixes ensure the pipeline (slot,parity) stream is never reset between
//   tile iterations, making multi-wave execution correct regardless of whether
//   nk_slice is divisible by NS.
...
// Public task_impl entry point — signature byte-identical to Mirage's
// fp8_gemm_dense_decode_splitk_sm100.cuh lines 430-444.
// Differs from the in-tree version only in the body of task_impl_splitk_tpl
// (the B36 continuous-counter fix above).
template <int BN, int NS, int NE, int SPLIT_K>
...
// Smem size query — identical to the in-tree formula; SPLIT_K does not affect
// per-stage tile size, only the K iteration count.
template <int BN, int NS, int NE, int SPLIT_K>
