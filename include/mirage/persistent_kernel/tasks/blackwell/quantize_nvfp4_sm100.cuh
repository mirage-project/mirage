...
__device__ __forceinline__ int interleaved_nvfp4_scale_offset(
  ...
  return (row_idx >> 7) * num_k_outer * scale_outer_stride +
         (group_idx >> 2) * scale_outer_stride + (row_in_block & 31) * 16 +
         ((row_in_block >> 5) & 3) * 4 + (group_idx & 3);
}
...
__device__ __forceinline__ int swapab_nvfp4_scale_offset(int row_idx,
  ...
  // stride of [num_n_tiles, sf_k_outer, 32, 4, 4]: innermost = 1, then 4, 16,
  // 32*4*4, ...
...
__device__ __forceinline__ void
    quantize_nvfp4_sm100_task_impl(void const *__restrict__ input_ptr,
