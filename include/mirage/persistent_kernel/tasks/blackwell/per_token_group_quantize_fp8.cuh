...
per_token_group_quantize_fp8_task_impl(const void *__restrict__ input_ptr,
  ...
  constexpr int PADDED_SCALE_K =
      ...
      SCALE_ALIGNMENT;
  constexpr int PACKED_SCALE_K = SCALE_UE8M0 ? (PADDED_SCALE_K / 4) : 0;
  ...
  // Shared memory for collecting UE8M0 bytes before packing (SCALE_UE8M0 only)
  __shared__ uint8_t smem_ue8m0_bytes[SCALE_UE8M0 ? PADDED_SCALE_K : 1];

  if constexpr (!SCALE_UE8M0) {
    // Float32 scale: zero-fill padding
#pragma unroll
    ...
    // UE8M0: zero-fill shared memory for padding
    for (int padded_idx = thread_idx; padded_idx < PADDED_SCALE_K;
         padded_idx += blockDim.x) {
      smem_ue8m0_bytes[padded_idx] = 0;
    }
    __syncthreads();
  }
...
#pragma unroll
  ...
  #pragma unroll
    ...
    float y_scale = 0.0f;
    if constexpr (SCALE_UE8M0) {
      // Full group max for UE8M0: one scale per 128-element group
      ...
      // Reconstruct exact UE8M0 scale for quantization
      y_scale = exp2f(static_cast<float>(static_cast<int>(scale_quant) - 127));

      // Store UE8M0 byte to shared memory for later packing
      if (lane_idx == 0) {
        smem_ue8m0_bytes[group_idx] = scale_quant;
      }
    } else {
      float group_max = group_reduce_max<SUBWARP_SIZE>(local_max);
      group_max = fmaxf(group_max, 1e-10f);
  ...
  // UE8M0 path: pack 4 bytes into uint32 and write column-major
  if constexpr (SCALE_UE8M0) {
    __syncthreads();
    // Pack 4 consecutive UE8M0 bytes into uint32, write column-major:
    // output_s[row_idx + pk * ALIGNED_BATCH]
    for (int pk = thread_idx; pk < PACKED_SCALE_K; pk += blockDim.x) {
      int base = pk * 4;
      uint32_t packed = static_cast<uint32_t>(smem_ue8m0_bytes[base])
                      | (static_cast<uint32_t>(smem_ue8m0_bytes[base + 1]) << 8)
                      | (static_cast<uint32_t>(smem_ue8m0_bytes[base + 2]) << 16)
                      | (static_cast<uint32_t>(smem_ue8m0_bytes[base + 3]) << 24);
      output_s[row_idx + pk * ALIGNED_BATCH] = packed;
    }
  }
}
