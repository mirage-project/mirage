...
per_token_group_quantize_fp8_task_impl(const void *__restrict__ input_ptr,
  ...
  // Pointers (global base; row offset computed via row_idx)
  const T *input = static_cast<const T *>(input_ptr) + row_idx * GLOBAL_STRIDE;
  DST_T *output_q = static_cast<DST_T *>(output_q_ptr) + row_idx * HIDDEN_SIZE;
  SCALE_PACKED_T *output_s_base = static_cast<SCALE_PACKED_T *>(output_s_ptr);
  ...
  // UE8M0 column-major layout constants
  // PACKED_K: number of uint32 elements per column (4 groups packed per uint32)
  constexpr int PACKED_K = (NUM_GROUPS_PER_ROW + 3) / 4;
  // ALIGNED_BATCH: batch dimension padded to multiple of 4 for column-major stride
  constexpr int ALIGNED_BATCH = ((BATCH_SIZE + 3) / 4) * 4;
  // Float scale: row-major, one float per group, padded to multiple of 4
  constexpr int PADDED_SCALE_K = ((NUM_GROUPS_PER_ROW + 3) / 4) * 4;
  ...
  if constexpr (!SCALE_UE8M0) {
    // Float scale: row-major with per-row pointer offset
    SCALE_PACKED_T *output_s = output_s_base + row_idx * PADDED_SCALE_K;
  #pragma unroll
  ...
  // Shared memory for collecting per-group UE8M0 bytes before packing
  // Only needed for UE8M0 path to pack 4 groups into 1 uint32
  __shared__ uint8_t ue8m0_bytes[NUM_GROUPS_PER_ROW];
...
#pragma unroll
  ...
  if constexpr (SCALE_UE8M0) {
    // Synchronize to ensure all ue8m0_bytes are written
    __syncthreads();

    // Pack 4 consecutive groups into 1 uint32 and write column-major
    // Column-major layout: output_s[row + packed_k_idx * ALIGNED_BATCH]
    for (int pk = thread_idx; pk < PACKED_K; pk += blockDim.x) {
      const int g_base = pk * 4;
      const uint8_t b0 = (g_base + 0 < NUM_GROUPS_PER_ROW) ? ue8m0_bytes[g_base + 0] : uint8_t(0);
      const uint8_t b1 = (g_base + 1 < NUM_GROUPS_PER_ROW) ? ue8m0_bytes[g_base + 1] : uint8_t(0);
      const uint8_t b2 = (g_base + 2 < NUM_GROUPS_PER_ROW) ? ue8m0_bytes[g_base + 2] : uint8_t(0);
      const uint8_t b3 = (g_base + 3 < NUM_GROUPS_PER_ROW) ? ue8m0_bytes[g_base + 3] : uint8_t(0);
      uint32_t packed = static_cast<uint32_t>(b0)
                      | (static_cast<uint32_t>(b1) << 8)
                      | (static_cast<uint32_t>(b2) << 16)
                      | (static_cast<uint32_t>(b3) << 24);
      // Column-major: stride = [1, ALIGNED_BATCH]
      output_s_base[row_idx + pk * ALIGNED_BATCH] = packed;
    }
  }
}
