...
//     NS=2 NE=4: 8.544µs, NS=3 NE=4: 7.488µs, NS=4 NE=4: 6.656µs, NS=6
//     NE=4: 7.360µs.
...
//   v006 GEMV-ldg (synchronous): 20.77µs (serial column loop, 16 cols
//   sequential) GEMV approach class exhausted. The tcgen05 body with NS=4 NE=2
//   is the floor. Gap to 5.83µs target (0.79µs) requires reducing the ~1.5µs
//   tcgen05 setup. This is considered the structural ceiling for the
//   tcgen05+GEMV approach space.
//
// CEILING ANALYSIS:
...
//   The 5.83µs target requires either a new hardware feature (persistent
//   tcgen05 alloc across tasks = not available) or a different kernel structure
//   not yet tried.
//
// DEFAULT BUILD: MPK_DSV3_FORCEINLINE not set → __noinline__ (byte-identical
// defaults). PRODUCTION: MPK_DSV3_FORCEINLINE=1 + MPK_DSV3_DENSE_NS=4
// → 6.624µs.
//
...
__device__ MPK_DSV3_TASK_INLINE void
  ...
  // NS=4 under MPK_DSV3_FORCEINLINE (the win); default = template NS
  // (byte-identical original). NE=2 matches original smallm.
...
__device__ MPK_DSV3_TASK_INLINE void
    fp8_gemm_dense_mediumm_fp8out_sm100_task_impl(
        ...
        int const num_workers,
        int const scale_outer_stride) {
  // NS=4 NE=2 under MPK_DSV3_FORCEINLINE (the win, 6.62µs); default = template
  // NS, NE=4 (byte-identical original mediumm).
  fp8_gemm_dense_qout_common::task_impl_tpl<BN,
#ifdef MPK_DSV3_FORCEINLINE
                                            /*NS=*/4,
                                            /*NE=*/2,
