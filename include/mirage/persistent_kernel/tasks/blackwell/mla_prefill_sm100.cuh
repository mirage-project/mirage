...
static constexpr int PF_SMEM_SIZE = PF_Q_NOPE_SIZE + PF_Q_PE_SIZE +
...
__device__ __noinline__ void mla_prefill_sm100_task_impl(
    const __nv_bfloat16* __restrict__ Q_nope,  // [S, H, D_CKV]
    const __nv_bfloat16* __restrict__ Q_pe,    // [S, H, D_KPE]
    const __nv_bfloat16* __restrict__ CKV,     // [S, D_CKV]
    const __nv_bfloat16* __restrict__ KPE,     // [S, D_KPE]
    __nv_bfloat16* __restrict__ O,             // [S, H, D_V]
    const int S, const int H, const float sm_scale_log2,
    const int head, const int q_block          // were blockIdx.x, blockIdx.y
) {
    using namespace mla_prefill;

    const int q_start = q_block * PF_BM;

    const int tid = threadIdx.x;
    const int warp_id = tid / PF_WARP_SIZE;
    const int lane_id = tid % PF_WARP_SIZE;
    const int warp_m  = warp_id / 2;
    const int warp_d  = warp_id % 2;
    ...
    auto ckv_smem = [&](int stage) { return ckv_smem_base + stage * PF_CKV_STAGE_SIZE; };
    auto kpe_smem = [&](int stage) { return kpe_smem_base + stage * PF_KPE_STAGE_SIZE; };
    ...
    // Step 1: Load Q into shared memory
    {
        ...
        for (int i = tid; i < Q_NOPE_LOADS; i += PF_NUM_THREADS) {
            ...
            const bf16* gmem_ptr = Q_nope + (long long)q_idx * H * PF_D_CKV + (long long)head * PF_D_CKV + col * 8;
            if (q_idx < S) {
        ...
        for (int i = tid; i < Q_PE_LOADS; i += PF_NUM_THREADS) {
            ...
            const bf16* gmem_ptr = Q_pe + (long long)q_idx * H * PF_D_KPE + (long long)head * PF_D_KPE + col * 8;
            if (q_idx < S) {
                cp_async_128b(smem_addr, gmem_ptr);
            } else {
                asm volatile("st.shared.v4.u32 [%0], {0, 0, 0, 0};\n" :: "r"(smem_addr));
    ...
    for (int kv_tile = 0; kv_tile < num_kv_tiles; kv_tile++) {
        ...
        // Fused QK
        {
            ...
            #pragma unroll
            for (int mma_k = PF_NUM_MMA_D_KPE_K; mma_k < PF_NUM_MMA_D_CKV_K; mma_k++) {
                uint32_t q_reg[4];
                int q_addr = q_nope_ldm_base ^ (mma_k * 32);
                ldmatrix_x4(q_reg, q_addr);
                #pragma unroll
        ...
        // Online Softmax
        constexpr int M_WG_OFF = PF_Q_NOPE_SIZE + PF_Q_PE_SIZE +
                                 PF_NUM_STAGES * (PF_CKV_STAGE_SIZE + PF_KPE_STAGE_SIZE);
        float* m_wg = reinterpret_cast<float*>(smem_raw + M_WG_OFF);
        {
            ...
            for (int j = 0; j < 2; j++) {
                m_state[j] = fmaxf(
                    m_wg[0 * PF_BM + warp_m * 16 + j * 8 + lane_id / 4],
    ...
    // Normalize O
    {
        constexpr int DWG_OFF = PF_Q_NOPE_SIZE + PF_Q_PE_SIZE +
                                PF_NUM_STAGES * (PF_CKV_STAGE_SIZE + PF_KPE_STAGE_SIZE);
        float* d_wg_smem = reinterpret_cast<float*>(smem_raw + DWG_OFF);
        #pragma unroll
        for (int j = 0; j < 2; j++) {
            if (lane_id % 4 == 0)
                d_wg_smem[warp_d * PF_BM + warp_m * 16 + j * 8 + lane_id / 4] = d_state[j];
        }
        __syncthreads();
        #pragma unroll
        for (int j = 0; j < 2; j++) {
    ...
    // Write O to global memory
    {
        ...
        for (int mma_d = 0; mma_d < PF_NUM_MMA_D_V_HALF; mma_d++) {
            int d_base = warp_d * (PF_D_V / 2) + mma_d * 16;
            {
                ...
                if (q_pos < S) {
                    long long base_offset = (long long)q_pos * H * PF_D_V + (long long)head * PF_D_V + d_base;
