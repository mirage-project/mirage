...
__device__ __forceinline__ void
    tma_ld_impl(int d, void const *t, int x, int y, int m) {
  asm volatile(
...
__device__ __forceinline__ uint32_t ld_shared_u32_impl(void const *p) {
  uint32_t r;
...
__device__
  ...
  // orderings → duplicate / missing tiles. Fix: warp-ballot prefix scan.
  {
    ...
    if (warp_id < 4) {
      ...
      int is_active =
          (active_expert_mask != nullptr) ? active_expert_mask[expert_id] : 1;
