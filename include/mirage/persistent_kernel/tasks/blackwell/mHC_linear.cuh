...
__device__ __noinline__ void
  ...
  cute::Tensor gA = cute::local_tile(mA, mma_tiler, mma_coord, cute::Step<cute::_1, cute::X, cute::_1>{});
  cute::Tensor gB = cute::local_tile(mB, mma_tiler, mma_coord, cute::Step<cute::X, cute::_1, cute::_1>{});
  cute::Tensor gC = cute::local_tile(mC,  cd_tiler, mma_coord, cute::Step<cute::_1, cute::_1, cute::X>{});
  ...
  using WeightSmem = smem_tma<T_, B, M, S, OUTPUT_ATOM_SIZE,
  ...
  static_assert(kNumMTiles == 1,
                "mHC_linear assumes single m_tile per CTA");
  ...
  if (warp_idx == 5) {
    ...
    int const _nt_start = (n_tile_override >= 0) ? n_tile_override : (int)blockIdx.x;
    int const _nt_step  = (n_tile_override >= 0) ? NUM_N_TILES : (int)gridDim.x;
    int const _nt_end   = (n_tile_override >= 0) ? (n_tile_override + 1) : NUM_N_TILES;
