...
template <bool SINGLE_TILE, bool WRITE_FINAL, int TP_SIZE>
__device__ __noinline__ void
    mla_unified_sm100_task_impl(nv_bfloat16 const *__restrict__ q_nope,
                                ...
                                int meta_y,
                                int meta_z) {
  if (prompt_prefill && prefill_q_len >= 1) {
    if (meta_x < num_heads) {
  ...
  if (!enable_decode || prompt_prefill || meta_z != 0 || decode_q_len < 1 ||
      decode_q_len > kDecodeMaxQLen) {
    return;
  }
  ...
  if constexpr (TP_SIZE == 1) {
    ...
    kernel::mla_mtp_decode_sm100_task_impl<SINGLE_TILE, WRITE_FINAL>(
        q_tm_ptr,
        kv_tm_ptr,
        partial_o,
        partial_lse,
    ...
    kernel::mla_mtp_tp2::mla_mtp_tp2_main<SINGLE_TILE, WRITE_FINAL>(
        q_tm_ptr,
        kv_tm_ptr,
        partial_o,
        partial_lse,
    ...
    kernel::mla_mtp_tp4::mla_mtp_tp4_main<SINGLE_TILE, WRITE_FINAL>(
        q_tm_ptr,
        kv_tm_ptr,
        partial_o,
        partial_lse,
    ...
    kernel::mla_mtp_tp8::mla_mtp_tp8_main<SINGLE_TILE, WRITE_FINAL>(
        q_tm_ptr,
        kv_tm_ptr,
        partial_o,
        partial_lse,
