...
// Multi-CTA variant of topk_sigmoid_task_impl. Each CTA owns a contiguous
// chunk of ROWS_PER_CTA rows indexed by blockIdx.x. Skips Phase 7; caller
// must invoke compact_active_experts_impl as a separate kernel on the same
// stream (the kernel-launch boundary acts as the grid-wide barrier).
template <typename T,
          int VPT,
          int NUM_EXPERTS,
          int WARPS_PER_CTA,
          ...
          int EXPERTS_PER_GROUP,
          int TOPK_EXPERTS>
__device__ __forceinline__ void topk_sigmoid_task_impl_multi(
    void *__restrict__ input_ptr,
  ...
  int const thread_row_in_warp = lane_idx / THREADS_PER_ROW;
  int const thread_row =
      cta_base_row + warp_idx * ROWS_PER_WARP + thread_row_in_warp;
  uint32_t const warp_mask = (num_rows % 2 == 1 && thread_row == num_rows - 1)
                                 ? 0x0000ffff
                                 : 0xffffffff;

  // Phase 0: per-CTA init of this CTA's slice of routing_indices.
  // Marker array + counter are initialized redundantly by every CTA
  // (idempotent -1 writes are race-safe given the temporal separation
  // between Phase 0 and any other CTA's Phase 5).
  if (mpk_routing_indices != nullptr) {
  ...
  // Phase 3: broadcast + iterative top-K-groups.
  float all_group_scores[NUM_GROUPS];
  for (int g = 0; g < NUM_GROUPS; ++g) {
    all_group_scores[g] = __shfl_sync(warp_mask, group_score,
                                      g * THREADS_PER_GROUP, THREADS_PER_ROW);
  }
  bool group_selected[NUM_GROUPS];
  for (int g = 0; g < NUM_GROUPS; ++g) {
    group_selected[g] = false;
  ...
  // Phase 4: mask losing groups.
  int const my_group = thread_group_idx / THREADS_PER_GROUP;
  if (!group_selected[my_group]) {
    for (int ii = 0; ii < VPT; ++ii) {
      biased_chunk[ii] = -10000.f;
    }
  ...
  // Phase 5: iterative top-K experts.
  int const start_col = first_elt_read_by_thread;
  float weight_sum = 0.f;
  for (int k_idx = 0; k_idx < TOPK_EXPERTS; ++k_idx) {
    float max_val = biased_chunk[0];
    int expert = start_col;
    ...
    int const owning_thread = expert / VPT;
    int const local_idx = expert % VPT;
    float my_score = 0.f;
    if (thread_group_idx == owning_thread) {
      my_score = row_chunk[local_idx];
    }
    float const orig_score =
        __shfl_sync(warp_mask, my_score, owning_thread, THREADS_PER_ROW);
    if (thread_group_idx == 0) {
      bool const node_uses_expert =
          expert >= start_expert && expert < end_expert;
  ...
  // Phase 6: normalize + scale.
  if (thread_group_idx == 0) {
    float const inv_sum = 1.0f / (weight_sum + 1e-20f);
    for (int k_idx = 0; k_idx < TOPK_EXPERTS; ++k_idx) {
      int const out_idx = TOPK_EXPERTS * thread_row + k_idx;
      output[out_idx] = output[out_idx] * inv_sum * routed_scaling_factor;
    }
...
// Phase-7 compaction (standalone). Walks mpk_active_expert_ids, appends
// any slot whose marker is >= 0 to the compact list at [0, count) using
// the counter at slot [NUM_EXPERTS]. Two-phase (snapshot, sync, append)
// to avoid the read/write race on the shared array.
