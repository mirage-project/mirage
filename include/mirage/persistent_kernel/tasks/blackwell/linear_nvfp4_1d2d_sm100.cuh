...
linear_nvfp4_1d2d_sm100_task_impl(const TMA_A &tma_a,
    ...
    */
    ...
    cute::Tensor mB = cute::make_coord_tensor(
        cute::make_layout(
            cute::make_shape(OUTPUT_SIZE, REDUCTION_SIZE),  // weight
            cute::make_stride(cute::E<1>{}, cute::E<0>{})
        )
    );
    ...
    cute::Tensor mSFA = cute::make_coord_tensor(
        cute::make_layout(
            cute::make_shape(BATCH_SIZE, REDUCTION_SIZE / SCALE_VECTOR_SIZE),   // input SF
            cute::make_stride(cute::E<1>{},cute::E<0>{})
        )
    );
    cute::Tensor mSFB = cute::make_coord_tensor(
        cute::make_layout(
            cute::make_shape(OUTPUT_SIZE, REDUCTION_SIZE / SCALE_VECTOR_SIZE),  // weight SF
            cute::make_stride(cute::E<1>{},cute::E<0>{})
        )
    );
    ...
    auto sC_stride = cute::make_stride(
        cute::make_stride(cute::Int<1>{}, cute::Int<MMA_M>{}),
        cute::Int<0>{},
        cute::Int<0>{},
    ...
    if (warp_idx == 0) {
        ...
        cutlass::arch::detail::initialize_barrier_array_aligned<
            cutlass::arch::ClusterBarrier,
            NUM_AB_STAGE>(shared_storage.sf_empty_mbar_ptr, 1);
        cutlass::arch::detail::initialize_barrier_array_aligned<
            cutlass::arch::ClusterBarrier,
            1>(shared_storage.sf_tmem_full_mbar_ptr, 1);
        cutlass::arch::detail::initialize_barrier_array_aligned<
            cutlass::arch::ClusterBarrier,
            1>(shared_storage.sf_tmem_empty_mbar_ptr, 1);
        cutlass::arch::detail::initialize_barrier_array_aligned<
            cutlass::arch::ClusterBarrier,
    ...
    // 32 threads (mma warp 4) + 128 threads (epilogue warps 0-3)
    ...
    constexpr int tma_transaction_bytes_AB = MMA_M * MMA_K / 2 * NUM_MMA_K   // A (FP4 packed, all K-blocks)
                                           + MMA_N * MMA_K / 2 * NUM_MMA_K;  // B (FP4 packed, all K-blocks)
    constexpr int tma_transaction_bytes_SF = MMA_M * MMA_K_SF * NUM_MMA_K    // SFA (all K-blocks)
                                           + MMA_N * MMA_K_SF * NUM_MMA_K;   // SFB (all K-blocks)
    ...
    // Compute TMEM offsets using find_tmem_tensor_col_offset for correctness
    ...
    if (warp_idx == 7) {
        ...
        for (int m_tile = tile_m_begin; m_tile < tile_m_end; ++m_tile) {
            for (int n_tile = tile_n_begin; n_tile < tile_n_end; ++n_tile) {
                ...
                int sf_rd_k_tile        = 0;
                int smem_rd_buffer      = (num_prev_k_blk + sf_rd_k_tile) % NUM_AB_STAGE;
                int sf_rd_full_phase    = (num_prev_k_blk + sf_rd_k_tile) / NUM_AB_STAGE % 2;
                int sf_tmem_empty_phase = (num_prev_k_blk + sf_rd_k_tile) % 2 ^ 1;

                for (int k_tile = 0; k_tile < k_tile_count; ++k_tile) {
                    int sf_rd_k_tile_next        = sf_rd_k_tile + 1;
                    int smem_rd_buffer_next      = (num_prev_k_blk + sf_rd_k_tile_next) % NUM_AB_STAGE;
                    int sf_rd_full_phase_next    = smem_rd_buffer_next == 0
                                                       ? sf_rd_full_phase ^ 1
                                                       : sf_rd_full_phase;
                    int sf_tmem_empty_phase_next = sf_tmem_empty_phase ^ 1;

                    // Wait for SF in SMEM, empty in TMEM
                    cute::wait_barrier(
                        shared_storage.sf_full_mbar_ptr[smem_rd_buffer],
                        sf_rd_full_phase
                    );
                    cute::wait_barrier(
                        shared_storage.sf_tmem_empty_mbar_ptr[0],
                        sf_tmem_empty_phase
                    );
                    ...
                    if (cute::elect_one_sync()) {
                        ...
                        auto copy_sf_s2t = [&](auto& tCsSF, auto& tCtSF, int stage) {
                        ...
                        copy_sf_s2t(tCsSFA, tCtSFA, smem_rd_buffer);
                        copy_sf_s2t(tCsSFB, tCtSFB, smem_rd_buffer);

                        cute::arrive_barrier(shared_storage.sf_tmem_full_mbar_ptr[0]);
                        cute::arrive_barrier(shared_storage.sf_empty_mbar_ptr[smem_rd_buffer]);
                    }

                    sf_rd_k_tile        = sf_rd_k_tile_next;
                    smem_rd_buffer      = smem_rd_buffer_next;
                    sf_rd_full_phase    = sf_rd_full_phase_next;
                    sf_tmem_empty_phase = sf_tmem_empty_phase_next;
        ...
        for (int m_tile = tile_m_begin; m_tile < tile_m_end; ++m_tile) {
            for (int n_tile = tile_n_begin; n_tile < tile_n_end; ++n_tile) {
                ...
                int mma_rd_sf_tmem_phase    = (num_prev_k_blk + mma_rd_k_tile) % 2;
                ...
                for (int k_tile = 0; k_tile < k_tile_count; ++k_tile) {
                    ...
                    int mma_rd_sf_tmem_phase_next  = mma_rd_sf_tmem_phase ^ 1;

                    // Wait for A/B in SMEM, SFA/AFB in TMEM
                    cute::wait_barrier(
                        shared_storage.ab_full_mbar_ptr[smem_rd_buffer],
                        mma_rd_ab_full_phase
                    );
                    cute::wait_barrier(
                        shared_storage.sf_tmem_full_mbar_ptr[0],
                        mma_rd_sf_tmem_phase
                    );
                    ...
                    mma_rd_sf_tmem_phase  = mma_rd_sf_tmem_phase_next;
        ...
        cute::TiledCopy tiled_copy_t2r = cute::make_tmem_copy(cute::SM100_TMEM_LOAD_32dp32b1x{}, tCtAcc_proto);
        cute::ThrCopy thr_copy_t2r = tiled_copy_t2r.get_slice(threadIdx.x);
        cute::Tensor tTR_tCtC = thr_copy_t2r.partition_S(tCtC(cute::make_coord(cute::_, cute::_), cute::_0{}, cute::_0{}, cute::_));
        cute::Tensor tCrC_ref = cute::make_tensor<C_type>(cute::make_shape(cute::Int<MMA_M>{}, cute::Int<MMA_N>{}));
        cute::Tensor tCrAcc   = cute::make_tensor<C_type>(cute::shape(thr_copy_t2r.partition_D(tCrC_ref)));
        ...
        for (int m_tile = tile_m_begin; m_tile < tile_m_end; ++m_tile) {
          for (int n_tile = tile_n_begin; n_tile < tile_n_end; ++n_tile) {
            ...
            // Bias
            cute::Tensor tCgBias = gBias(cute::_, cute::_, m_tile, n_tile); // (MMA_M, MMA_N)
            cute::Tensor tCrBias = cute::make_tensor<C_type>(cute::shape(tCrAcc));
            if constexpr (!NOBIAS) {
              cute::copy(thr_copy_t2r.partition_D(tCgBias), tCrBias);
            }

            // TMEM -> REG
            cute::wait_barrier(
                shared_storage.acc_full_mbar_ptr[epi_rd_acc_buf],
                epi_rd_acc_full_phase
            );
            cute::copy(tiled_copy_t2r,
                       tTR_tCtC(cute::_, cute::_, cute::_, epi_rd_acc_buf),
                       tCrAcc);
            ...
            // REG -> SMEM
            cute::copy(tCrAcc, cute::flatten(tCsC(cute::_, 0, 0, c_smem_wr_buffer))(cute::_, threadIdx.x));
            cute::tma_store_fence();
            epilogue_wg_barrier.arrive_and_wait();

            // SMEM -> GMEM
            if (warp_idx == 0 && cute::elect_one_sync()) {
              sC.set_ptr(static_cast<void*>(tCsC(cute::_, 0, 0, c_smem_wr_buffer).data().get()));
              if constexpr (SplitK) {
                tma_out.tma_reduce_add_async(
                    sC.base_ptr,
                    {n_tile * MMA_N, m_tile * MMA_M});
              } else {
