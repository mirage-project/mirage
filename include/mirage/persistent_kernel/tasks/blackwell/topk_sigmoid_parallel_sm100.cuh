// Auto-tuned warp-parallel variant of topk_sigmoid_sm100.cuh
// (kernel-agent derived, 2026-06-20).
...
// Templated on LOCAL_EXPERTS: the scan width and the count slot
// [LOCAL_EXPERTS] must follow the caller's instantiation (256 at EP1, 128 at
// EP2 tests, 32 at the TP8 EP2 production shard) — a fixed width compacts the
// wrong range and writes the count where no consumer reads it.
// ────────────────────────────────────────────────────────────────────────────
...
__device__ __forceinline__ void _fbsig_compact_active_experts(int *active_ids) {
  if (threadIdx.x < 32) {
    ...
    int count = 0;
    for (int base = 0; base < LOCAL_EXPERTS; base += 32) {
      int le = base + lane;
      int mark = (le < LOCAL_EXPERTS) ? active_ids[le] : -1;
    ...
    if (lane == 0) {
      active_ids[LOCAL_EXPERTS] = count;
    }
  }
}
...
#pragma unroll
  ...
  _fbsig_compact_active_experts<LOCAL_EXPERTS>(active_ids);
