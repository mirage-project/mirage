...
// Shared device primitives for the SM100 FP4 linear kernels — used by both
// NVFP4 (kind::mxf4nvf4, scale_vec::4X) and MXFP4 (kind::mxf4, block32)
// flavours across all six kernel families (1d2d 1SM, 1d2d 2SM, swapAB ×
// {NVFP4, MXFP4}). Provides PTX wrappers (TMA, mbar, tmem, tcgen05) at file
// scope.
//
// Naming convention:
//   - Format-agnostic helpers (mbarrier_*, tma_*, tmem_*, tcgen05_ld_*,
//     supergroup mapping) have NO format suffix.
//   - Format-specific helpers (the MMA instruction, scale-tile copy) have a
//     trailing `_nvfp4` or `_mxfp4` suffix.
// SM-count differences (cta_group::1 vs ::2, multicast TMA, mbar scope) are
// handled via `int SM` template parameters; the kernels pick the right
// variant at instantiation time.
//
// No namespace: each kernel includes this header directly. We may split the
// helpers across multiple files later; the namespace can be added then.
...
// ----------------------------------------------------------------------------
// Untemplated PTX primitives (same shape across SM=1 and SM=2 paths).
// ----------------------------------------------------------------------------
...
// Cluster sync — only meaningful at SM=2. Callers gate with
// `if constexpr (SM == 2) { cluster_sync(); }`.
...
// CTA-scope wait (parity, .cta scope). Used for tile/scale/mma/output mbars
// that live in the current CTA's smem.
__device__ __forceinline__ void mbarrier_wait_cta(int mbar_addr, int phase) {
...
// Cluster-scope wait (parity, .cluster scope). Used in 2SM for the tile_mbar
// that the MMA leader waits on across the cluster.
...
// 1D bulk copy (raw byte gmem -> smem). Used by 1d2d 1SM for the SF tensor
// (which is fetched as a raw pointer, not via a tensor map).
__device__ __forceinline__ void
tma_load_bulk(int dst, const void *src, int size, int mbar_addr, uint64_t cache_policy) {
  asm volatile(
...
// 2D TMA store (smem -> gmem). Used by 2SM and swapAB epilogues.
__device__ __forceinline__ void
...
// Cluster-multicast TMA (cta_group::1 form, multicast::cluster).
// Inherently a 2SM-cluster primitive; takes a cta_mask so each receiver's
// mbar accumulates the bytes delivered to it. Only meaningful in SM=2.
__device__ __forceinline__ void
...
// 2SM-only: arrive on CTA-0's mbar with no tx-bytes expectation. Used by
// epilogue to signal MMA leader (which runs on CTA 0) that accumulator is
// drained. Wraps the mapa.shared + mbarrier.arrive.shared::cluster sequence.
...
// Supergroup tile mapping. Used by persistent paths; non-persistent kernels
// don't call this (their tile coords come from blockIdx). SUPERGROUP_SIZE=1
// degenerates to plain column-major scan.
...
// ----------------------------------------------------------------------------
// Templated primitives — SM-count selects the PTX variant at compile time.
// ----------------------------------------------------------------------------

// 3D TMA load (gmem -> smem). RANK is fixed at 3 in this file; the parameter
// is here for symmetry with the design and to leave room for a future 2D load
// variant if one is needed.
//
//   SM=1: cta_group::1 form, mbar is local.
//   SM=2: cta_group::2 form, mbar is mapped to CTA 0 (both CTAs report there).
...
// Defined here so the unified kernel does not depend on the legacy headers.
// ----------------------------------------------------------------------------
