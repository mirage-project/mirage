...
template <int REDUCTION_SIZE,
          ...
          bool HAS_BIAS>
__global__
    __launch_bounds__(BLOCK_M + 4 * 32) void linear_mxfp4_1d2d_2sm_sm100_kernel(
  ...
  int const cta_group_m = static_cast<int>(cluster_ctaid_x());
  int const cluster_idx = static_cast<int>(blockIdx.x) / 2;
  int const num_clusters = static_cast<int>(gridDim.x) / 2;
  int const num_m_tiles = M / (2 * BLOCK_M);
  int const num_n_tiles = N / BLOCK_N;
  int const num_output_tiles = num_m_tiles * num_n_tiles;
  ...
  static_assert(BLOCK_N % EPI_TILE_N == 0,
                "BLOCK_N must be divisible by EPI_TILE_N");
  constexpr int EPI_PIPE_DEPTH = BLOCK_N / EPI_TILE_N;
  constexpr int EPI_TILE_BYTES =
      EPI_TILE_N * BLOCK_M * sizeof(type::bfloat16_t);
  static_assert(EPI_BATCH_LA >= 1, "EPI_BATCH_LA must be >= 1");
  static_assert(EPI_BATCH_LA <= EPI_PIPE_DEPTH,
                "EPI_BATCH_LA must not exceed EPI_PIPE_DEPTH");
  static_assert(EPI_PIPE_DEPTH % EPI_BATCH_LA == 0,
...
#pragma nv_diag_suppress static_var_with_dynamic_init
  ...
  int const tile_mbar_addr = static_cast<int>(__cvta_generic_to_shared(mbars));
  int const scale_mbar_addr = tile_mbar_addr + NUM_STAGES * 8;
  int const mma_mbar_addr = scale_mbar_addr + NUM_STAGES * 8;
  ...
  constexpr int SFB_STAGE_STRIDE =
      SF_TMEM_COLS_PER_MMA_K * MMA_PER_TILE * SFB_SCALE_TILES;
  ...
  if (warp_id == TILE_TMA_WARP && elect_sync()) {
    for (int output_tile = cluster_idx, work_idx = 0;
      ...
      for (int iter_k = 0; iter_k < NUM_ITERS; iter_k++) {
        ...
        int const mbar_addr = tile_mbar_addr + stage_id * 8;
        ...
        // Only CTA 0 arms the cluster-scope tile mbar; cta_group::2 TMA reports
        // there.
        if (cta_group_m == 0) {
    ...
    for (int output_tile = cluster_idx, work_idx = 0;
      ...
      for (int iter_k = 0; iter_k < NUM_ITERS; iter_k++) {
        ...
        int const mbar_addr = scale_mbar_addr + stage_id * 8;
        int const A_smem = smem + stage_id * STAGE_SIZE;
        ...
        tma_load_3d_multicast(SFA_smem,
                              &SFA_tmap,
                              0,
                              2 * (off_k / 64),
                              off_m / 128,
                              ...
                              self_mask,
                              cache_A);
        tma_load_3d_multicast(SFB_smem + cta_group_m * SFB_TILE_BYTES,
                              &SFB_tmap,
                              0,
                              2 * (off_k / 64),
                              (off_n / 128) + cta_group_m,
                              mbar_addr,
                              0b11,
                              cache_B);
      }
    ...
    bool const mma_leader = (cta_group_m == 0) && elect_sync();
    ...
    if (mma_leader) {
      for (int output_tile = cluster_idx, work_idx = 0;
        ...
        for (int iter_k = 0; iter_k < NUM_ITERS; iter_k++) {
          ...
          // One cp per MMA-K (NVFP4-style). 4 TMEM cols per MMA-K; vec::2X
          // reads the first 2 (the rest are padding).
          auto copy_scale_k = [&](int k) {
            uint64_t sfa_desc = SFA_desc + static_cast<uint64_t>(k) *
                                               (SF_BYTES_PER_K_TILE >> 4ULL);
            tcgen05_cp_fp4<2>(SFA_tmem + stage_id * SFA_STAGE_STRIDE +
                                  k * SFA_K_STRIDE,
                              sfa_desc);
#pragma unroll
            for (int n_tile = 0; n_tile < SFB_SCALE_TILES; n_tile++) {
              uint64_t sfb_desc =
                  SFB_desc +
                  static_cast<uint64_t>(n_tile) * (SFB_TILE_BYTES >> 4ULL) +
                  static_cast<uint64_t>(k) * (SF_BYTES_PER_K_TILE >> 4ULL);
              tcgen05_cp_fp4<2>(SFB_tmem + stage_id * SFB_STAGE_STRIDE +
                                    k * SFB_K_STRIDE +
                                    n_tile * SFB_N_TILE_STRIDE,
                                sfb_desc);
            }
          };

          mbarrier_arrive_expect_tx_local(scale_mbar_addr + stage_id * 8,
                                          SCALE_EXPECTED_TX);
...
#pragma unroll
          for (int k = 0; k < BLOCK_K / MMA_K; k++) {
            ...
            uint64_t b_desc =
                make_desc_AB(B_smem + k1 * B_LOCAL_N * 128 + k2 * 32);

            int const scale_A_tmem =
                SFA_tmem + stage_id * SFA_STAGE_STRIDE + k * SFA_K_STRIDE;
            int const scale_B_tmem =
                SFB_tmem + stage_id * SFB_STAGE_STRIDE + k * SFB_K_STRIDE +
                ((BLOCK_N < 128) ? (tile_n % (128 / BLOCK_N)) * (BLOCK_N / 32)
                                 : 0);
            int const enable_input_d = (k1 == 0 && k2 == 0) ? iter_k : 1;

            tcgen05_mma_mxfp4<2>(a_desc,
                                 b_desc,
                                 i_desc,
                                 scale_A_tmem,
                                 scale_B_tmem,
      ...
      auto epilogue_M_major = [&]() {
        ...
        auto load_subtile = [&](float *dst, int n) {
          if constexpr (EPI_TILE_N == 128) {
            tcgen05_ld_32x32bx128(
                dst, tmem_row_base + warp_id * 32, n * EPI_TILE_N);
          }
          if constexpr (EPI_TILE_N == 64) {
            tcgen05_ld_32x32bx64(
                dst, tmem_row_base + warp_id * 32, n * EPI_TILE_N);
          }
          if constexpr (EPI_TILE_N == 32) {
            tcgen05_ld_32x32bx32(
                dst, tmem_row_base + warp_id * 32, n * EPI_TILE_N);
          }
        };
        ...
        auto store_subtile = [&](float const *src, int n) {
          ...
          if (warp_id == 0 && elect_sync()) {
            tma_store_2d(out_smem_addr + buffer_id * EPI_TILE_BYTES,
                         &C_tmap,
                         off_m,
                         off_n + n * EPI_TILE_N);
            tma_store_commit();
