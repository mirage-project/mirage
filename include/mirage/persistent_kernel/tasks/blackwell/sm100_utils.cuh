...
// adapted from the DeepGEMM SM100 Blackwell path: provide the minimum descriptor, TMEM, block-scaled MMA helpers needed by Mirage's block-scaled FP8 linear kernel

__device__ __forceinline__
// Build a generic UMMA shared-memory descriptor
cute::UMMA::SmemDescriptor make_smem_desc(cute::UMMA::LayoutType layout, void* smem_ptr, uint32_t stride_byte_offset, uint32_t leading_byte_offset) {
...
// In the current Mirage block-scaled FP8 implementation, scale factors are first staged in shared memory, then copied into TMEM scale columns through UTCCP
// This descriptor uses a non-swizzled layout because the required reordering is handled explicitly before UTCCP
...
__device__ __forceinline__
// Update only the address field of an existing UMMA shared-memory descriptor
...
uint32_t advance_umma_desc_lo(const uint32_t& base, const uint32_t& offset, const uint32_t& k_idx) {
  static_assert(kMajorMode == cute::UMMA::Major::K, "Current Mirage helper only supports K-major path.");
  // For K-major SW128 path, descriptor increments linearly along K.
  return base + (((offset + k_idx) * static_cast<uint32_t>(sizeof(dtype_t))) >> 4u);
}
...
cute::UMMA::SmemDescriptor make_umma_desc(dtype_t* base_smem_ptr, uint32_t mn_idx, uint32_t k_idx) {
  static_assert(kMajorMode == cute::UMMA::Major::K, "Current Mirage helper only supports K-major path.");
  static_assert(kSwizzleMode == BLOCK_K * sizeof(dtype_t), "For K-major path, swizzle mode must equal BLOCK_K * sizeof(dtype_t).");
  ...
  return make_smem_desc(layout_type, base_smem_ptr + mn_idx * BLOCK_K + k_idx, stride_byte_offset, leading_byte_offset);
}
...
// Thin inline-PTX wrapper for the Blackwell tcgen05 block-scaled MMA instruction
