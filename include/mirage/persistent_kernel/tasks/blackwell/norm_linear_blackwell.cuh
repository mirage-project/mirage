...
__device__ __forceinline__ void
    norm_linear_blackwell_impl(void const *input_ptr,
  ...
  constexpr uint32_t TMEM_ALLOC_COLS = 128;
  ...
  // sizeof(T) * 8
  constexpr size_t TMEM_ADDR_OFFSET = 16;
  ...
  *((__uint128_t *)zero_buf) = 0ul;
  ...
  uint32_t *tmem_addr_ptr = (uint32_t *)(smem + TMEM_ADDR_OFFSET);
  uint32_t *tmem_addr_ptr_A = (uint32_t *)(smem + TMEM_ADDR_A_OFFSET);
  ...
  if (warp_idx == 0) {
    tmem_alloc(tmem_addr_ptr, TMEM_ALLOC_COLS);
    tmem_alloc(tmem_addr_ptr_A, TMEM_ALLOC_COLS);
  }
...
#pragma unroll
      ...
      // blockIdx.z == 0) {
      //   printf("BATCH_SIZE is %d, TILE_SIZE is %d, REDUCTION_SIZE is %d,
      //   OUTPUT_ATOM_SIZE is %d, NUM_ITERS_M is %d, NUM_ITERS_N is %d,
      //   NUM_ITERS_K is %d, NUM_WARPS_N is %d, NUM_WARPS_K is %d\n",
      //   BATCH_SIZE, TILE_SIZE, REDUCTION_SIZE, OUTPUT_ATOM_SIZE, NUM_ITERS_M,
      //   NUM_ITERS_N, NUM_ITERS_K, NUM_WARPS_N, NUM_WARPS_K);
      // }
...
#pragma unroll
  ...
  // dealloc TMEM and release lock
  if (warp_idx == 0) {
    tmem_dealloc(*tmem_addr_ptr, TMEM_ALLOC_COLS);
    tmem_dealloc(*tmem_addr_ptr_A, TMEM_ALLOC_COLS);
    release_lock();
  }
}
