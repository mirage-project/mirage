...
linear_fp8_1d2d_sm100_task_impl(const TMA_A &tma_a,
    ...
    static_assert(
        cute::is_same_v<T_, cutlass::float_e4m3_t> ||
        cute::is_same_v<T_, cutlass::float_e5m2_t>,
        "T_ must be float_e4m3_t or float_e5m2_t for FP8 MMA"
    );
    ...
    cute::TiledMMA tiled_mma = cute::make_tiled_mma(
        cute::SM100_MMA_MXF8F6F4_2x1SM_SS<A_type,
                                        B_type,
                                        C_type,
                                        SF_type,
                                        MMA_M,
    ...
    WeightScaleFactorsSmem input_scale_smem(shared_weight_SF);
    ...
    // TODO: What should be the expected shape?
    cute::Tensor tCrSFA = cta_mma.make_fragment_A(tCsSFA); // (MmaA, NumMma_M, NumMma_K, Tiles_K)
    cute::Tensor tCrSFB = cta_mma.make_fragment_B(tCsSFB); // (MmaB, NumMma_M, NumMma_K, Tiles_K)
    ...
    // TODO: Check what pointer to use
    // Currently tCtAcc.data() = shared_storage.tmem_base_ptr
    auto tCrSFA_tmem = cute::make_tensor(
        cute::make_tmem_ptr<SF_type>(shared_storage.tmem_base_ptr),
        cute::layout(tCrSFA)
    );
    auto tCrSFB_tmem = cute::make_tensor(
        cute::make_tmem_ptr<SF_type>(shared_storage.tmem_base_ptr),
        cute::layout(tCrSFB)
    );
    ...
    if (warp_idx == 5) {
            ...
            for (int n_tile = 0; n_tile < cute::size<3>(tCgB); ++n_tile) {
                ...
                for (int k_tile = 0; k_tile < k_tile_count; ++k_tile) {
                    ...
                    for (int k_block = 0; k_block < cute::size<2>(tCrA); ++k_block) {
                        cute::copy(tCsSFA(_, _, k_block, smem_rd_buffer), tCrSFA_tmem(_, _, k_block));
                        cute::copy(tCsSFB(_, _, k_block, smem_rd_buffer), tCrSFB_tmem(_, _, k_block));
                        cute::gemm(
                            tiled_mma.with(
                                tiled_mma.accumulate_,        // ScaleOut mode (Zero or One)
                                tCrSFA_tmem(_,_,k_block),          // Scale factors for A
                                tCrSFB_tmem(_,_,k_block)       // Scale factors for B
                            ),
        ...
        // TODO: Need to allocate this before now?
        if (warp_idx == 0) {
