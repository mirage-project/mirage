...
namespace kernel {
  ...
  template <class SharedStorage,
          ...
          class TmaAtomA, class TmaAtomB, class TmaAtomD, int Num_AB_Stage,
          int Num_ACC_Stage, int Num_D_Stage>
  __device__ __forceinline__ void linear_kernel_ws_sm100(
            ATensor mA,                      // (Gemm_M, Gemm_K)
            BTensor mB,                      // (Gemm_N, Gemm_K)
            CTensor mC,                      // (Gemm_N, Gemm_M)
            DTensor mD,                      // (Gemm_N, Gemm_M)
    ...
    auto cd_tiler = cute::make_shape(cute::size<1>(mma_tiler), cute::size<0>(mma_tiler), cute::size<2>(mma_tiler)); // (MmaTile_N, MmaTile_M, MmaTile_K)
    ...
    cute::Tensor gC = cute::local_tile(mC, cd_tiler, mma_coord, cute::Step<cute::_1,cute::_1,cute::X>{});  // (MmaTile_M, MmaTile_N)
    cute::Tensor gD = cute::local_tile(mD, cd_tiler, mma_coord, cute::Step<cute::_1,cute::_1,cute::X>{});  // (MmaTile_M, MmaTile_N)
    ...
    if (warp_idx == 0) {
      cutlass::arch::detail::initialize_barrier_array_aligned<cutlass::arch::ClusterTransactionBarrier, Num_AB_Stage>(shared_storage.ab_full_mbar_ptr, /* arrival count */ 1);
      cutlass::arch::detail::initialize_barrier_array_aligned<cutlass::arch::ClusterBarrier, Num_AB_Stage>(shared_storage.ab_empty_mbar_ptr, /* arrival count */ 1);
      cutlass::arch::detail::initialize_barrier_array_aligned<cutlass::arch::ClusterBarrier, Num_ACC_Stage>(shared_storage.acc_full_mbar_ptr, /* arrival count */ 1);
      cutlass::arch::detail::initialize_barrier_array_aligned<cutlass::arch::ClusterBarrier, Num_ACC_Stage>(shared_storage.acc_empty_mbar_ptr, /* arrival count */ 4);
    }
    ...
    cute::Tensor sD_epi = shared_storage.tensor_sD();               // (EpiTile)
    ...
    cute::Tensor tCgD = cute::tiled_divide(gD, epi_tiler); // (EpiTile_M, EpiTile_N, Tiles_M, Tiles_N)
    ...
    //   cute::print("tCgD:\t"); cute::print(tCgD); cute::print("\n");  // tCgD:   ArithTuple(_0,_0) o ((_32,_128),_1,_1,1,8):((_1@1,_1@0),_0,_0,_32@1,_128@0)
    //   cute::print("tCsA:\t"); cute::print(tCsA); cute::print("\n");
    //   cute::print("tCsB:\t"); cute::print(tCsB); cute::print("\n");
    //   cute::print("sD_epi:\t"); cute::print(sD_epi); cute::print("\n");
    // } __syncthreads();
    ...
    auto [tDgD, tDsD] = cute::tma_partition(*tma_atom_D,
                                      cute::Int<0>{}, cute::Layout<cute::_1>{},
                                      cute::group_modes<0,3>(sD_epi), cute::group_modes<0,3>(tCgD));
    ...
    //   cute::print("tDgD:\t"); cute::print(tDgD); cute::print("\n");  // tDgD:   ArithTuple(_0,0) o (((_64,_256),_1),4):(((_1@0,_1@1),_0),_64@0)
    //   cute::print("tDsD:\t"); cute::print(tDsD); cute::print("\n");  // tDsD:   Sw<3,4,3>_smem_ptr[16b](SMEM_ADDR_D) o ((_16384,_1)):((_1,_0))
    ...
    auto acc_shape = cute::partition_shape_C(tiled_mma, cute::make_shape(cute::size<0>(mma_tiler), cute::size<1>(mma_tiler), cute::Int<Num_ACC_Stage>{})); // (MmaC, NumMma_M, NumMma_N, NumAcc_Stage)
    ...
    if (warp_idx == 5){
      ...
      for (int m_tile=0; m_tile < cute::size<3>(tCgA); ++m_tile) {
        for (int n_tile=0; n_tile < cute::size<3>(tCgB); ++n_tile) {
          ...
          bool peek_ab_empty_status = try_wait_barrier(shared_storage.ab_empty_mbar_ptr[smem_wr_buffer], tma_wr_ab_empty_phase);
          ...
          for (int k_tile=0; k_tile < k_tile_count; ++k_tile) {
            ...
            int tma_wr_ab_empty_phase_next = smem_wr_buffer_next == 0 ? tma_wr_ab_empty_phase ^ 1 : tma_wr_ab_empty_phase;
            
            // Wait for an empty buffer
            if (!peek_ab_empty_status){
              cute::wait_barrier(shared_storage.ab_empty_mbar_ptr[smem_wr_buffer], tma_wr_ab_empty_phase);
            }
            ...
            if (cute::elect_one_sync()) {
              ...
              copy(tma_atom_A->with(shared_storage.ab_full_mbar_ptr[smem_wr_buffer]), tAgA_Slice(cute::_, k_tile), tAsA(cute::_, smem_wr_buffer));
              copy(tma_atom_B->with(shared_storage.ab_full_mbar_ptr[smem_wr_buffer]), tBgB_Slice(cute::_, k_tile), tBsB(cute::_, smem_wr_buffer));
              // printf("loaded ab tile m=%d n=%d k=%d into smem_buf=%d\n", m_tile, n_tile, k_tile, smem_wr_buffer);
            }

            if (tma_wr_k_tile_next < k_tile_count){
              peek_ab_empty_status = try_wait_barrier(shared_storage.ab_empty_mbar_ptr[smem_wr_buffer_next], tma_wr_ab_empty_phase_next);
            }
      ...
      for (int m_tile=0; m_tile < cute::size<3>(tCgA); ++m_tile) {
        for (int n_tile=0; n_tile < cute::size<3>(tCgB); ++n_tile) {
          ...
          // Peek full phase
          bool peek_ab_full_status = try_wait_barrier(shared_storage.ab_full_mbar_ptr[smem_rd_buffer], mma_rd_ab_full_phase);
          ...
          for (int k_tile=0; k_tile < k_tile_count; ++k_tile) {
            ...
            if (mma_rd_k_tile_next < k_tile_count){
              peek_ab_full_status = try_wait_barrier(shared_storage.ab_full_mbar_ptr[smem_rd_buffer_next], mma_rd_ab_full_phase_next);
            }
      ...
      using TypeBias = typename decltype(gC)::value_type;
      using TypeC = typename decltype(tDsD)::value_type;
      ...
      // cute::Tensor tTR_rAcc = cute::make_tensor<AccType>(cute::shape(tTR_tAcc(cute::_, 0, cute::_, cute::_, 0))); // (T2R, T2R_M, T2R_N)
      ...
      for (int m_tile=0; m_tile < cute::size<3>(tCgA); ++m_tile) {
        for (int n_tile=0; n_tile < cute::size<3>(tCgB); ++n_tile) {
          ...
          int d_smem_wr_buffer_idx = num_tiles_executed % Num_D_Stage;

          cute::Tensor tDgBias = gC(cute::_, cute::_, n_tile, m_tile); // (Mma_M, Mma_N)
          cute::Tensor tDrBiasTypeBias = cute::make_tensor<TypeBias>(cute::shape(tTR_rAcc(0, cute::_, 0, 0))); // (T2R_M, T2R_N)
          cute::Tensor tDrBiasTypeAcc = cute::make_tensor<AccType>(cute::shape(tDrBiasTypeBias));
          cute::Tensor tDrC = cute::make_tensor<TypeC>(cute::shape(tDrBiasTypeBias));

          // if(threadIdx.x == 0 and m_tile == 0 and n_tile == 0) {
          //   cute::print("tDgBias:\t"); cute::print(tDgBias); cute::print("\n"); //
          //   cute::print("tDrBiasTypeBias:\t"); cute::print(tDrBiasTypeBias); cute::print("\n"); // 
          //   cute::print("tDrBiasTypeAcc:\t"); cute::print(tDrBiasTypeAcc); cute::print("\n"); // 
          //   cute::print("tDrC:\t"); cute::print(tDrC); cute::print("\n"); // 
          // } epilogue_wg_barrier.arrive_and_wait();

          // this copy might conflict with TMA load, might add a wait barrier if needed
          cute::copy(tDgBias(cute::_, threadIdx.x), tDrBiasTypeBias);
          // optimize with vectorized type conversion

          // T2R and register operations

          CUTE_UNROLL
          for (int i = 0; i < tDrBiasTypeBias.size(); i++) {
            tDrBiasTypeAcc[i] = tDrBiasTypeBias[i];
          }
          ...
          for (int i = 0; i < tTR_rAcc.size(); i++) {
            tTR_rAcc[i] += tDrBiasTypeAcc[i];
          }

          CUTE_UNROLL
          for (int i = 0; i < tDrC.size(); i++) {
            tDrC[i] = converter(tTR_rAcc[i]);
          }

          // R2S copy

          cute::Tensor sD_epi_slice = cute::flatten(sD_epi(cute::_, 0, 0, d_smem_wr_buffer_idx)); 

          // if(threadIdx.x == 0 and m_tile == 0 and n_tile == 0) {
          //   cute::print("sD_epi_slice:\t"); cute::print(sD_epi_slice); cute::print("\n"); //
          // } epilogue_wg_barrier.arrive_and_wait();

          copy(tDrC, sD_epi_slice(cute::_, threadIdx.x));

          // S2G TMA
          cute::tma_store_fence(); // Ensure D smem stores are visible to TMA
          epilogue_wg_barrier.arrive_and_wait(); // Ensure all threads have issued fence

          if (warp_idx == 0 && cute::elect_one_sync()){
            cute::copy(*tma_atom_D, tDsD(cute::_, d_smem_wr_buffer_idx), tDgD(cute::_, n_tile, m_tile));
            cute::tma_store_arrive();
            cute::tma_store_wait<Num_D_Stage-1>();
          }
