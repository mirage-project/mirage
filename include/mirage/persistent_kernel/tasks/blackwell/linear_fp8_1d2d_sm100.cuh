...
linear_fp8_1d2d_sm100_task_impl(const TMA_A &tma_a,
  ...
  constexpr int SF_BLOCK_M = ((OUTPUT_ATOM_SIZE + detail::kNumUTCCPAlignedElems - 1) /
                              detail::kNumUTCCPAlignedElems) * detail::kNumUTCCPAlignedElems;
  ...
  constexpr int num_tmem_columns = kNumAccumTmemCols + kNumSFATmemCols + kNumSFBTmemCols; // TODO

  int warp_idx = cutlass::canonical_warp_idx_sync();
  int lane_idx = kernel::lane_id();

  auto mma_coord_vmnk = cute::make_coord(
      0,       // peer CTA coordinate
      cute::_,
      cute::_,
      cute::_);
  ...
  using SharedStorage = PipedSharedStorageWithSF<
      T_, T_, TypeC, TypeScale,
      decltype(sA_layout),
      decltype(sB_layout),
      decltype(sC_layout),
  ...
  auto smem_sfa = [&](int stage) -> TypeScale* {return shared_storage.SFA.begin() + stage * SF_BLOCK_M;};

  auto smem_sfb = [&](int stage) -> TypeScale* {return shared_storage.SFB.begin() + stage * SF_BLOCK_N;};
...
#pragma unroll
    for (int i = lane_idx; i < block_rows; i += 32) {
      ...
      dst[i] = global_row < total_rows ? src[global_row * PADDED_SCALE_K + k_tile]
                                       : TypeScale(0);
    }
  };
  ...
  // NOTE: tCrA/tCrB are still useful for shape/k-looping, but actual FP8 block-scaled
  // issue below uses low-level descriptors, not cute::gemm(tiled_mma, ...).
  ...
  using WeightSmem = smem_tma<T_, B, M, S, OUTPUT_ATOM_SIZE, WEIGHT_TMA_TILE_SIZE, 1>;
    ...
    for (int m_tile = 0; m_tile < cute::size<3>(tCgA); ++m_tile) {
      for (int n_tile = 0; n_tile < cute::size<3>(tCgB); ++n_tile) {
        ...
        for (int k_tile = 0; k_tile < k_tile_count; ++k_tile) {
          ...
          load_packed_scale_tile(
              smem_sfa(smem_wr_buffer),
              weight_scale_ptr,
              m_base,
              OUTPUT_SIZE,
              SF_BLOCK_M,
...
#pragma unroll
  ...
  // --------------------------------------------------------------------------
    ...
    // These helpers are expected to exist in Mirage, or be ported from deepgemm.
    auto instr_desc =
        cute::UMMA::make_instr_desc_block_scaled<T_, T_, float,
                                                 cutlass::float_ue8m0_t,
                                                 MMA_M, MMA_N,
                                                 cute::UMMA::Major::K,
                                                 cute::UMMA::Major::K>();
    ...
    auto a_desc = kernel::sm100::make_umma_desc<cute::UMMA::Major::K, OUTPUT_ATOM_SIZE, TILE_K, 128>(shared_weight, 0, 0);

    auto b_desc = kernel::sm100::make_umma_desc<cute::UMMA::Major::K,MMA_N, TILE_K, 128>(shared_input, 0, 0);

    uint32_t a_desc_lo =
        lane_idx < NUM_AB_STAGE
            ? (a_desc.lo + lane_idx * (OUTPUT_ATOM_SIZE * TILE_K * sizeof(T_)) / 16)
            : 0u;
    uint32_t b_desc_lo =
        lane_idx < NUM_AB_STAGE
    ...
    for (int m_tile = 0; m_tile < cute::size<3>(tCgA); ++m_tile) {
      for (int n_tile = 0; n_tile < cute::size<3>(tCgB); ++n_tile) {
        ...
        // For block-scaled low-level mma_t::fma, this is controlled by the bool "accumulate".
...
#pragma unroll
            for (int i = 0; i < SF_BLOCK_M / detail::kNumUTCCPAlignedElems; ++i) {
              auto smem_ptr = smem_sfa(smem_rd_buffer) + i * detail::kNumUTCCPAlignedElems;
...
#pragma unroll
            for (int i = 0; i < SF_BLOCK_N / detail::kNumUTCCPAlignedElems; ++i) {
              auto smem_ptr = smem_sfb(smem_rd_buffer) + i * detail::kNumUTCCPAlignedElems;
              kernel::sm100::replace_smem_desc_addr(sf_desc, smem_ptr);
...
#pragma unroll
            for (int k_sub = 0; k_sub < NUM_K_SUBTILES; ++k_sub) {
              ...
              const auto runtime_instr_desc = kernel::sm100::make_runtime_instr_desc_with_sf_id(instr_desc, sfa_id, sfb_id);

              b_desc.lo = kernel::sm100::advance_umma_desc_lo<cute::UMMA::Major::K, MMA_N, 128, T_>(b_desc_base_lo, 0, k_sub * UMMA_K);

              a_desc.lo = kernel::sm100::advance_umma_desc_lo<cute::UMMA::Major::K, OUTPUT_ATOM_SIZE, 128, T_>(a_desc_base_lo, 0, k_sub * UMMA_K);

              mma_issue_t::fma(a_desc, b_desc, acc_buf_idx * MMA_N, accumulate, runtime_instr_desc, kTmemStartColOfSFA, kTmemStartColOfSFB);
              accumulate = true;
            }
          }
  ...
  // --------------------------------------------------------------------------
    ...
    for (int m_tile = 0; m_tile < cute::size<3>(tCgA); ++m_tile) {
      for (int n_tile = 0; n_tile < cute::size<3>(tCgB); ++n_tile) {
        ...
        cute::Tensor tCgBias =
            gBias(cute::_, cute::_, n_tile, m_tile);
