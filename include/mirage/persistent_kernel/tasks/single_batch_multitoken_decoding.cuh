 ...
 */
 ...
 template <typename T,
           ...
           int WEIGHT_STRIDE,
           int NUM_TOKENS>
           // new tokens to be processed in addition the kv cache
 __device__ __forceinline__ void
   ...
   const __restrict__ T *d_k = 
        static_cast<T const *>(qkv_ptr) + HEAD_DIM * NUM_Q_HEADS * NUM_TOKENS;
   const __restrict__ T *d_v = 
        static_cast<T const *>(qkv_ptr) + HEAD_DIM * (NUM_Q_HEADS + NUM_KV_HEADS) * NUM_TOKENS;
...
// First, naively process the new tokens independently
for (int token_idx = 0; token_idx < NUM_TOKENS; token_idx++) {
    ...
    for (int i = threadIdx.x; i < NUM_Q_HEADS * (HEAD_DIM / 8);
        ...
        int row = i / 16;
        int col = (i % 16) * 8;
        load_smem(q_smem(row, col), q_dmem(token_idx * NUM_Q_HEADS + row, col));
    }
    ...
    for (int n = 0; n < 8; n++) {
        for (int i = 0; i < 2; i++) {
            o_smem[threadIdx.x * 32 + n * 4 + i * 2 + token_idx * 1024] = o[n][i * 4];
            o_smem[threadIdx.x * 32 + n * 4 + i * 2 + 1 + token_idx * 1024] = o[n][i * 4 + 1];
        }
    }
    d_smem[threadIdx.x + token_idx * 128] = d_sum;
    
    // norm m
    if (m != -inf) {
        m *= sm_scale;
    }
    max_smem[threadIdx.x + token_idx * 128] = m;
    ...
    #pragma unroll
        ...
        int shmem_idx = (idx_in_warp / 4) * 4 + tidx * 32 + (idx_in_warp % 4) + token_idx * 128;
    ...
    #pragma unroll
            for (uint32_t frag_idx = 0; frag_idx < 2; frag_idx++) {
                float o_new1 = o_smem[shmem_idx * 32 + n * 4 + frag_idx * 2 + token_idx * 1024];
                float o_new2 = o_smem[shmem_idx * 32 + n * 4 + frag_idx * 2 + 1 + token_idx * 1024];
    ...
    for (int i = threadIdx.x; i < 128; i += NUM_THREADS) {
        ...
        int row = seq_len - 1;
        int col = i;
        k_cache_dmem.at(seq_len - 1 + token_idx, col) = k_cache_smem.at(last_seq_len - 1, col);
    }
    ...
    for (int i = threadIdx.x; i < 128; i += NUM_THREADS) {
        ...
        int row = seq_len - 1;
        int col = i;
        v_cache_dmem.at(seq_len - 1 + token_idx, col) = v_cache_smem.at(last_seq_len - 1, col);
    }
    ...
    for (int i = threadIdx.x; i < (NUM_Q_HEADS * 128); i += NUM_THREADS) {
        ...
        int row = i / 128;
        int col = (i % 128);
        output_dmem.at(token_idx * NUM_Q_HEADS + row, col) = output_smem.at(row, col);
    }
    }
