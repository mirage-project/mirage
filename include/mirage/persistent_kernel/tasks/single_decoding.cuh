...
__device__ __forceinline__ void
    single_batch_decoding_kernel(const void *qkv_ptr,
                                 void *k_cache_ptr,
                                 void *v_cache_ptr,
                                 void *output_ptr) {
  ...
  dmem_row<T, SEQ_LEN, 128, 128> k_cache_dmem(d_k_cache);
  dmem_row<T, SEQ_LEN, 128, 128> v_cache_dmem(d_v_cache);
  ...
  // copy weight
  T *shared_k = (T *)(smem + 3712);
  T *shared_k_buffer = (T *)(smem + 7808);

  T *shared_v = (T *)(smem + 11904);
  T *shared_v_buffer = (T *)(smem + 16000);
  // intermidiate
  T *mm_output = (T *)(smem + 128);
  T *shared_output = (T *)(shared_output + 128);
  T *zero_buf = (T *)(smem);

  // flashattn metadata
  float *d_smem = (float *)(smem + 20096);
  float *max_smem = (float *)(smem + 20608);
  float *o_smem = (float *)(smem + 21120);
  ...
  smem_row<T, 1, 1, 1, 7, 128, 128> output_smem(shared_output);
...
#pragma unroll
  for (int i = threadIdx.x; i < 112; i += NUM_THREADS) {
    ...
    int row = i / 16;
    int col = (i % 16) * 8;
    load_smem(q_smem_buffer(row, col), q_dmem(row, col));
  }
...
#pragma unroll
    ...
    for (int n = 0; n < 2; n++) {
      for(int k = 0; k < 4; k++){
        ...
        // mma_m16n16k16_bf16bf16bf32(o[n], s_frag, v_frag, o[n]);
      }
...
#pragma unroll
      for (uint32_t frag_idx = 0; frag_idx < 8; frag_idx++) {
        o_smem[threadIdx.x * 64 + n * 8 + frag_idx] = o[n][frag_idx];
      }
    }
...
#pragma unroll
        for (uint32_t frag_idx = 0; frag_idx < 8; frag_idx++) {
          o_other[n][frag_idx] = o_smem[shmem_idx * 64 + n * 8 + frag_idx];
        }
      }
...
#pragma unroll
        for (uint32_t frag_idx = 0; frag_idx < 8; frag_idx++) {
          o[n][frag_idx] = o[n][frag_idx] * ptx_exp2(m_prev - m) +
                           o_other[n][frag_idx] * ptx_exp2(other_m - m);
        }
      }
    }
...
// update KV cache
#pragma unroll
  for (int i = threadIdx.x; i < 16; i += NUM_THREADS) {
    // offset
    int row = SEQ_LEN;
    int col = i * 8;
...
#pragma unroll
  for (int i = threadIdx.x; i < (NUM_Q_HEADS * 16); i += NUM_THREADS) {
    // offset
    int row = i / 16;
    int col = (i % 16) * 8;
