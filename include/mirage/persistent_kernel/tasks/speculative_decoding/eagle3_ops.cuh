...
// --- Eagle3 Step 0 Input Prep (select-by-ac + aux concat fused) ---
// For the new serial K>1 draft chain design, step 0 needs to pick the chain
// starting from slot `ac-1` (= the last accepted target argmax / aux hidden).
// This kernel does the select + concat in one pass:
//   selected_token[0]     = argmax_out[ac-1]                 (int64)
//   aux_concat[0, 0..H)   = aux_h0[ac-1]                     (bf16)
//   aux_concat[0, H..2H)  = aux_h1[ac-1]
//   aux_concat[0, 2H..3H) = aux_h2[ac-1]
//
// `ac` is clamped to [1, BATCH_SIZE] so the slot index ac-1 is in range. When
// ac > BATCH_SIZE (the "exhausted chain" case), the last slot is used — see
// eagle3_commit_kernel for the matching clamp policy.
//
// Inputs:
//   argmax_out: [BATCH_SIZE, 1]  int64 — target's argmax at each q-position
//   aux_h0..2:  [BATCH_SIZE, H]  bf16  — target's mid-layer hidden, per q-pos
//   ac:         [1]              int32 — verify_strict accepted_count
// Outputs:
//   selected_token: [1, 1]   int64
//   aux_concat:     [1, 3*H] bf16
template <typename T, int BATCH_SIZE, int HIDDEN_DIM>
__device__ __forceinline__ void eagle3_step0_input_prep_kernel(
    void const *__restrict__ argmax_out_ptr,
    void const *__restrict__ aux_h0_ptr,
    void const *__restrict__ aux_h1_ptr,
    void const *__restrict__ aux_h2_ptr,
    void const *__restrict__ accepted_count_ptr,
    void *__restrict__ selected_token_ptr,
    void *__restrict__ aux_concat_ptr) {
  long long const *__restrict__ argmax =
      static_cast<long long const *>(argmax_out_ptr);
  T const *__restrict__ h0 = static_cast<T const *>(aux_h0_ptr);
  T const *__restrict__ h1 = static_cast<T const *>(aux_h1_ptr);
  T const *__restrict__ h2 = static_cast<T const *>(aux_h2_ptr);
  int const *__restrict__ ac_ptr =
      static_cast<int const *>(accepted_count_ptr);
  long long *__restrict__ sel_token =
      static_cast<long long *>(selected_token_ptr);
  T *__restrict__ out = static_cast<T *>(aux_concat_ptr);

  // Clamp slot index to [0, BATCH_SIZE-1]. ac ∈ [1, K+1]; with BATCH_SIZE=K+1,
  // ac=K+1 (all drafts + bonus accepted) maps to slot K which is BATCH_SIZE,
  // out of bounds → clamp to BATCH_SIZE-1.
  int ac = ac_ptr[0];
  int slot = ac - 1;
  if (slot < 0) slot = 0;
  if (slot >= BATCH_SIZE) slot = BATCH_SIZE - 1;

  // Thread 0 writes the selected token id.
  if (threadIdx.x == 0) {
    sel_token[0] = argmax[slot];
  }

  // All threads cooperatively copy the selected slot's H-dim aux into the
  // 3H-dim output buffer (3 slots side by side).
  int const tid = threadIdx.x;
  int const stride = blockDim.x;
  for (int d = tid; d < HIDDEN_DIM; d += stride) {
    out[0 * HIDDEN_DIM + d] = h0[slot * HIDDEN_DIM + d];
    out[1 * HIDDEN_DIM + d] = h1[slot * HIDDEN_DIM + d];
    out[2 * HIDDEN_DIM + d] = h2[slot * HIDDEN_DIM + d];
  }
}
