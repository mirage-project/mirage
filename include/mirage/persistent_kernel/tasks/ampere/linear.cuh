...
__device__ __forceinline__ void linear_kernel(void const *input_ptr,
  ...
  constexpr int ADJUSTED_PIPE_MAX = PIPE_MAX < TOTAL_WEIGHT_BLOCKS_TO_LOAD
                                        ? PIPE_MAX
...
#pragma unroll
    ...
    // Warm up weight and input tiles for the first ADJUSTED_PIPE_MAX - 1 tile.
#pragma unroll
...
#pragma unroll
                for (int chu = 0; chu < NUM_CHUNKS_B / NUM_THREADS; chu++) {
                  ...
                  int col_within = threadCol + chu * COLS_PER_ITERATION;
                  int src_col =
                      (output_atom_idx << log2_OUTPUT_ATOM_SIZE) + col_within;
                  int dst_col = col_within;
