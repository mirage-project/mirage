...
template <typename T,
          ...
          int PIPE_MAX = 3>
__device__ __forceinline__ void linear_kernel_row(void const *input_ptr,
...
template <typename T,
          ...
          int REDUCTION_SIZE,
          int O_STRIDE = OUTPUT_SIZE,
          int RESIDUAL_STRIDE = O_STRIDE,
          int PIPE_MAX = 3>
__device__ __forceinline__ void linear_kernel_col(void const *input_ptr,
                                              void const *weight_ptr,
                                              void const *residual_ptr,
                                              void *output_ptr,
                                              int num_active_tokens,
  ...
  using WeightDmem =
      dmem_col_const<T, TILE_SIZE, OUTPUT_ATOM_SIZE, REDUCTION_SIZE>;
  using ResidualDmem = dmem_row_const<T, BATCH_SIZE, OUTPUT_SIZE, RESIDUAL_STRIDE>;
  using OutputDmem = dmem_col<T, BATCH_SIZE, OUTPUT_SIZE, O_STRIDE>;
...
#pragma unroll
      ...
      // Final writeback: store accumulated output (residual already included if
      // any)
      int row, src_col, dst_col;
      static_assert(BATCH_SIZE >= 8 && BATCH_SIZE % 8 == 0);
      // TODO(Zepeng) Change output_smem to column major to use vectored store
#pragma unroll
      for (int i = threadIdx.x; i < NUM_CHUNKS_OUTPUT * CHUNK_SIZE; i += NUM_THREADS) {
        row = i % BATCH_SIZE;
        src_col = i / BATCH_SIZE;
        dst_col = src_col + (nn << log2_OUTPUT_ATOM_SIZE);
        *((T *)((void *)&output_dmem.at(row, dst_col))) =
            *((T *)((void *)&output_smem.at(row, src_col)));
      }
    } // loop for NUM_ITERS_N, it may not be 1
  }   // loop for NUM_ITERS_M, it should always be 1, no sense loop
}
