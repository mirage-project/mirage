...
// nvcc --expt-relaxed-constexpr -std=c++17 -O3 -arch=sm_80
// -I../../../../../deps/cutlass/include moe_linear_debug.cu --ptxas-options=-v
// -o moe_cutlass.o or nvcc --expt-relaxed-constexpr -std=c++17 -O3 -arch=sm_80
// -I/home/wenqin/study/cutlass/include moe_linear_debug.cu --ptxas-options=-v
// -o moe_cutlass.o
...
// Matrix dimensions
int const m = batch_size;
int const k = 2048;
// const int n = 4096;
int const n = 1536;

int const expert_stride = 5;
...
__global__ void moe_kernel_wrapper(void const *input_ptr,
                                   ...
                                   void *output_ptr) {
  kernel::moe_linear_kernel<T,
                            BATCH_SIZE,
                            OUTPUT_SIZE,
                            OUTPUT_STRIDE,
                            REDUCTION_SIZE,
                            experts_size,
                            activate_experts_size,
                            expert_stride,
                            true>(input_ptr,
                                  weight_ptr,
                                  residual_ptr,
...
void launch_moe(void const *input_ptr,
  ...
  cudaFuncSetAttribute(moe_kernel_wrapper<T,
                                          BATCH_SIZE,
                                          output_size,
                                          OUTPUT_SIZE,
                                          REDUCTION_SIZE>,
...
int main(int argc, char **argv) {
  ...
  // 0 in route means not activate, other postivate number means the index of
  // activate expert for this token.
  std::vector<uint32_t> h_expert_routing(experts_size * m);
  // whether the expert is activte for any token, it will be 0 if it's not
  // activate, the last one is how many expert is activate.
  ...
  for (size_t token_idx = 0; token_idx < m; ++token_idx) {
    ...
    float const *token_scores =
        h_expert_score.data() + token_idx * experts_size;
  ...
  CUDA_CHECK(
      cudaMalloc(&d_output_matrix, h_output_matrix.size() * sizeof(bfloat16)));

  CUDA_CHECK(cudaMalloc(&d_expert_routing,
                        h_expert_routing.size() * sizeof(uint32_t)));
  CUDA_CHECK(
      cudaMalloc(&d_expert_mask, h_expert_mask.size() * sizeof(uint32_t)));

  // Copy data from host to device
  CUDA_CHECK(cudaMemcpy(d_matrix_A,
                        h_matrix_A.data(),
                        h_matrix_A.size() * sizeof(bfloat16),
                        cudaMemcpyHostToDevice));
  CUDA_CHECK(cudaMemcpy(d_matrix_B,
                        h_matrix_B.data(),
                        h_matrix_B.size() * sizeof(bfloat16),
                        cudaMemcpyHostToDevice));
  CUDA_CHECK(cudaMemcpy(d_residual,
                        h_residual.data(),
                        h_residual.size() * sizeof(bfloat16),
                        cudaMemcpyHostToDevice));

  CUDA_CHECK(cudaMemcpy(d_expert_routing,
                        h_expert_routing.data(),
                        h_expert_routing.size() * sizeof(uint32_t),
                        cudaMemcpyHostToDevice));
  CUDA_CHECK(cudaMemcpy(d_expert_mask,
                        h_expert_mask.data(),
                        h_expert_mask.size() * sizeof(uint32_t),
                        cudaMemcpyHostToDevice));
...
#if WARM_UP
  for (int i = 0; i < 10; ++i) {
    launch_moe<bfloat16, batch_size, n, k>(d_matrix_A,
                                           d_matrix_B,
                                           d_residual,
                                           d_expert_routing,
                                           d_expert_mask,
  ...
  double flops =
      2.0 * double(m) * double(n) * double(k) * active_experts.size();
  ...
  // Copy back result
  CUDA_CHECK(cudaMemcpy(h_output_matrix.data(),
                        d_output_matrix,
                        h_output_matrix.size() * sizeof(bfloat16),
                        cudaMemcpyDeviceToHost));

  // Log (same as CUTLASS version)
  std::cout << "============== Baseline =================" << std::endl;
  std::cout << "BF16 GEMM -> FP32 output (M,N,K)=(" << m << "," << n << "," << k
            << ")\n"
            << "iters=" << iters << "\n"
            << "Avg time: " << avg_ms << " ms\n"
            << "Perf: " << gflops << " GFLOP/s\n"
  ...
  for (int i = 0; i < std::min(n, 4); ++i) {
    std::cout << __bfloat162float(h_output_matrix[i])
              << (i + 1 < 4 ? ", " : "\n");
  }
  ...
  for (int i = 0; i < m; i++) {
    ...
    for (int j = 0; j < 4; j++) {
      printf(
          "%.4f, ",
          __bfloat162float(h_output_matrix[i * n * activate_experts_size + j]));
    }
    printf("\n");
  }
  ...
  std::cout << "CUDA memory allocation and data transfer complete."
            << std::endl;
  return 0;
}