...
// co-requisites that don't exist yet: (1) per-task footprint <= half the 14-page
// pool (NUM_STAGES<=3) so two linear tasks coexist, and (2) the SMEM planner
// double-buffering page assignments so consecutive tasks get DISJOINT pages.
// Without those, N and N+1 share all 14 pages and acquire/release only adds sync
// (measured ~+12%). Default OFF (task-end blanket release, baseline perf); flip
// to true once (1)+(2) land.
...
inline ::mirage::runtime::TaskSmemInfo make_smem_info() {
  ...
  // planner-side ordering hint only — the device runtime reads physical_page_start
  // and any assignment is a valid permutation, so it changes WHICH pages back a
  // stage, not behavior. (Real per-stage release at runtime is step two.)
  ...
  // Linear-private cross-warp scratch (TMEM allocation address). 16 B, packable.
  // Lives the whole task → released last (highest release_step).
...
inline constexpr int declared_sem_count() {
  ...
  for (auto const &c : CHANNELS) {
    ...
    if (c.shares_empty_with < 0) n += c.depth;     // empty[] (unless shared)
  }
  n += NUM_ONESHOT_SEMS;
  return n;
}
