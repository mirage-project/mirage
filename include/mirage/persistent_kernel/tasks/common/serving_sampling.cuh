...
// Scores are recomputed on each pass. The 64-bit key breaks score ties by
// token ID, so top-k and top-p boundaries need no vocabulary-sized list.
// candidate_count == -1 scans the full vocabulary.
...
__device__ inline uint64_t cutoff(Reader const &reader, int vocab,
                                  ...
                                  int const *candidates, int candidate_count) {
...
// A thread loads adjacent logits as a vector; the tail or an unaligned row
// falls back to scalar loads. Warp coalescing and per-thread width are separate.
...
struct SamplingScratch {
  int *frequency_counts;
  uint32_t *seen_total;
  uint32_t *seen_generated;
  float *workspace;
  int *history_cursor;
  int bitset_words;

  __device__ SamplingScratch(float *scratch, int padded_vocab)
      : frequency_counts(reinterpret_cast<int *>(scratch)),
        bitset_words(sampling_bitset_words(padded_vocab)) {
    seen_total = reinterpret_cast<uint32_t *>(frequency_counts + sampling_frequency_words(padded_vocab));
    seen_generated = seen_total + bitset_words;
    workspace = reinterpret_cast<float *>(seen_generated + bitset_words);
    history_cursor = reinterpret_cast<int *>(workspace + SCRATCH_WORKSPACE);
  }
};

__device__ inline void prepare_history(SamplingScratch const &scratch,
                                       ServingConfig const *cfg,
                                       long long const *history, int history_len,
                                       int prompt_len, int generation_position,
                                       int vocab, float frequency,
                                       bool reuse_history) {
  ...
  int begin = cached ? *scratch.history_cursor : 0;
  if (!cached) {
    if (frequency != 0.f)
      for (int v = threadIdx.x; v < vocab; v += blockDim.x)
        scratch.frequency_counts[v] = 0;
    for (int v = threadIdx.x; v < scratch.bitset_words; v += blockDim.x) {
      scratch.seen_total[v] = 0;
      scratch.seen_generated[v] = 0;
    }
    __syncthreads();
  }
  for (int i = begin + threadIdx.x; i < history_len; i += blockDim.x) {
    ...
    if (token >= 0 && token < vocab) {
      atomicOr(scratch.seen_total + (token >> 5), uint32_t(1) << (token & 31));
      if (i >= prompt_len) {
        atomicOr(scratch.seen_generated + (token >> 5), uint32_t(1) << (token & 31));
        if (frequency != 0.f) atomicAdd(scratch.frequency_counts + token, 1);
      }
    }
  }
  // All threads must consume the old cursor before it is updated.
  __syncthreads();
  if (threadIdx.x == 0) *scratch.history_cursor = history_len;
}
...
struct SamplingScoreReader {
  ...
  __device__ float apply_bias(int v, float score) const {
    ...
    if (bias_count) {
      ...
      if (lo < bias_count && cfg->biases[lo].token == v)
        score += float(cfg->biases[lo].value);
    }
    return score;
  }

  __device__ float apply_generated_penalties(int v, float score) const {
  ...
  __device__ float raw(int v, float score) const {
    score = apply_bias(v, score);
    if (repetition != 1.f &&
        (seen_total[v >> 5] & (uint32_t(1) << (v & 31))))
      score = score > 0 ? score / repetition : score * repetition;
    score = apply_generated_penalties(v, score);
    return isnan(score) ? -INFINITY : score;
  }
  __device__ float raw(int v) const {
    return raw(v, static_cast<float>(logits[v]));
  }
...
// Both sampling paths use the same vocabulary traversal. The vector load is
// most valuable for plain unfiltered sampling, and also avoids a second logit
// load when penalties or biases are applied.
template<typename T, typename Visit>
__device__ inline void scan_logits(T const *logits, int vocab, Visit visit) {
  ...
  for (int base = Width * threadIdx.x; base < vocab; base += Width * blockDim.x) {
    ...
    for (int i = 0; i < Width; ++i) {
      int v = base + i;
      if (v < vocab) visit(v, static_cast<float>(values.values[i]));
    }
  }
}
...
__device__ inline int dual_pivot_top_p(Reader const &reader, int vocab,
  ...
  for (int round = 0; round < 64 && low < high; ++round) {
    ...
    for (int v = threadIdx.x; v < vocab; v += blockDim.x) {
      float score = reader(v);
      uint64_t key = score_key(score, v);
      // The midpoint is above the pivot, so neither tally needs lower keys.
      if (key <= pivot) continue;
      float weight = expf(score);
      mass_above_pivot += weight;
      uint64_t draw = sampled_key(score, v, 0, rng);
      next_pivot = max(next_pivot, draw);
      if (key > midpoint) {
        mass_above_midpoint += weight;
        next_midpoint = max(next_midpoint, draw);
      }
    }
...
template<bool Plain, typename T, typename Reader>
__device__ inline int sample_unfiltered(T const *logits, int vocab,
                                        Reader const &reader, float temperature,
                                        SamplingRng const &rng, float *workspace) {
  Draw draw;
  ChooseDraw choose{temperature};
  scan_logits(logits, vocab, [&](int v, float logit) {
    float score;
    if constexpr (Plain) {
      score = isnan(logit) ? -INFINITY : logit;
    } else {
      score = reader.raw(v, logit);
    }
...
__device__ inline int sample_filtered(Reader const &reader, int vocab, int k,
  ...
  if (k <= ShortlistCapacity / 4 && vocab >= 4096 && blockDim.x == 128) {
    ...
    for (int v = threadIdx.x; v < vocab; v += blockDim.x) {
      if (score_key(reader(v), v) >= probe) {
        ...
        if (index < ShortlistCapacity) shortlist[index] = v;
      }
    }
    __syncthreads();
    if (shortlist_count >= k && shortlist_count <= ShortlistCapacity) {
      lower = probe;
  ...
  if (candidate_count >= 0) {
    ...
    extern __shared__ __align__(16) unsigned char sampling_shared[];
    auto *compact = reinterpret_cast<int *>(sampling_shared) + ShortlistCapacity;
  ...
  if (top_p < 1.f) {
    ...
    mass = block_reduce(mass, workspace, Sum{});
    if (top_p * mass <= 1.f) return best_token;
...
__device__ inline int sample_token(T const *logits, int vocab,
                                   ...
                                   int generation_position, float *workspace) {
  ...
  int k = int(cfg->top_k);
  float top_p = float(cfg->top_p);
  bool top_k = k > 0 && k < vocab;
  bool greedy = cfg->temperature == 0.0 || k == 1;
  float temperature = fmaxf(float(cfg->temperature), 0x1.0p-126f);
  bool unfiltered = !greedy && !top_k && top_p == 1.f;
  SamplingRng rng{uint64_t(cfg->seed), uint32_t(generation_position)};

  if (unfiltered) {
    if (!penalties && cfg->bias_count == 0)
      return sample_unfiltered<true>(logits, vocab, reader, temperature, rng, workspace);
    return sample_unfiltered<false>(logits, vocab, reader, temperature, rng, workspace);
  }

  uint64_t best = 0;
  scan_logits(logits, vocab, [&](int v, float logit) {
    uint64_t key = score_key(reader.raw(v, logit), v);
    best = key > best ? key : best;
  });
  best = block_reduce(best, workspace, Maximum{});
  int best_token = 0xffffffffu - uint32_t(best);
  if (greedy) return best_token;

  float max_score = reader.raw(best_token);
  if (!isfinite(max_score)) return best_token;
...
__device__ inline void sample(T const *logits, float *scratch, Token *output,
  ...
  SamplingScratch state(scratch, padded_vocab);
  float repetition = float(cfg->repetition_penalty);
  float frequency = float(cfg->frequency_penalty);
  float presence = float(cfg->presence_penalty);
  bool penalties = frequency != 0 || presence != 0 || repetition != 1;
  if (penalties)
    prepare_history(state, cfg, history, history_len, prompt_len,
                    generation_position, vocab, frequency, reuse_history);
  SamplingScoreReader<T> reader{logits, cfg, state.frequency_counts,
