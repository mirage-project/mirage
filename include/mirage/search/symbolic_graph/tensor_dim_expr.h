...
public:
  ...
  virtual std::optional<int>
      maybe_get_value(DimVarAssignment const &assignments) const;
  virtual std::shared_ptr<TensorDimExpr const> with_partial_assignment(
      DimVarAssignment const &partial_assignment) const = 0;
  virtual std::string to_egg() const = 0;
  virtual std::string to_string() const = 0;
  virtual std::unordered_set<std::shared_ptr<TensorDimVar const>>
      get_all_vars() const = 0;
  ...
  bool symbolically_equivalent_to(
      std::shared_ptr<TensorDimExpr const> other) const;
  bool can_be_symbolically_equivalent_to(
      std::shared_ptr<TensorDimExpr const> other) const;
  bool is_one() const;
};
...
struct equal_to<std::shared_ptr<mirage::search::TensorDimExpr const>> {
  bool operator()(
      std::shared_ptr<mirage::search::TensorDimExpr const> const &lhs,
...
public:
  ...
  std::shared_ptr<TensorDimExpr const> with_partial_assignment(
      DimVarAssignment const &partial_assignment) const override;
  std::string to_egg() const override;
  std::string to_string() const override;
  std::unordered_set<std::shared_ptr<TensorDimVar const>>
      get_all_vars() const override;
