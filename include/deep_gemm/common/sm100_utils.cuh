...
make_sf_desc(void *smem_ptr) {
  return make_smem_desc(cute::UMMA::LayoutType::SWIZZLE_NONE,
                        smem_ptr,
                        8 * 16,
                        0);
}
...
get_atom_base(const cute::UMMA::LayoutType &layout_type) {
...
advance_umma_desc_lo(const uint32_t &base,
                     const uint32_t &offset,
                     const uint32_t &k_idx) {
  return base +
         (((offset +
            k_idx *
                get_umma_desc_stride_k<kMajorMode,
                                       BLOCK_MN,
                                       kSwizzleMode,
                                       dtype_t>()) *
...
make_runtime_instr_desc_with_sf_id(cute::UMMA::InstrDescriptorBlockScaled desc,
