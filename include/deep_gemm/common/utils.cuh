...
template <typename FuncT> struct PatternVisitor {
  FuncT func;
...
template <typename T> __device__ __host__ T ceil_div(T a, T b) {
  return (a + b - 1) / b;
}
...
template <typename T> __device__ __host__ T align(T a, T b) {
...
template <typename T> __forceinline__ __device__ void swap(T &a, T &b) {
  T temp = a;
  a = b;
  b = temp;
}
...
__device__ __forceinline__ void st_shared(const void *ptr,
                                          uint32_t x,
                                          uint32_t y,
                                          uint32_t z,
                                          uint32_t w) {
  asm volatile("st.shared.v4.u32 [%0], {%1, %2, %3, %4};"
               :
               : "l"(__cvta_generic_to_shared(ptr)), "r"(x), "r"(y), "r"(z),
                 "r"(w));
}
...
__device__ __forceinline__ int cast_into_bf16_and_pack(old_t &x, old_t &y) {
  auto bf16x2 =
      __float22bfloat162_rn({*reinterpret_cast<float *>(&x),
