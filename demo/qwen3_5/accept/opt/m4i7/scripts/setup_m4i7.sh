#!/usr/bin/env bash
# M4-I7: isolated clone at origin/qwen3-5_support HEAD with its OWN freshly-built
# C++ extension (STALE-EXTENSION TRAP). Pattern from setup_m4i2.sh.
set -uo pipefail
B=$HOME/mpk-qwen35
SRC=$B/mirage
D=$B/mirage-m4i7
...
git -C "$D" checkout --quiet -B m4i7 FETCH_HEAD || { echo "CHECKOUT FAILED"; exit 2; }
...
"
echo "=== df after $(date -Is) ==="; df -h /raid / | tail -2
