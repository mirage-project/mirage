#!/usr/bin/env bash
# M4-I7: the MoE grouped-GEMM integration A/B -- two arms, 5 batch sizes, 3 reps,
...
#
#   arm A (base) MPK_MOE_BLOCKSCALE_BASELINE=1 -- the FROZEN golden body for
#                tasks 241/242, i.e. the pre-M4-I7 generated code, from THIS tree.
#   arm B (new)  default -- the ferret v012 fast body: work-item flattening plus
#                the expert_stride fetch-path rule.
#
# A KERNEL DIR PER (arm, bs) IS MANDATORY: the arm is a -D in the generated TU,
# so two arms sharing a --kernel-dir under --reuse-kernel would run ONE binary
# and report themselves identical while nothing changed (M3-I7 defect 3). Dir
# names carry the arm AND the bs; MPK_MOE_PATH_POLICY, when set, joins the name
# for the same reason.
#
...
M=${M:-/var/tmp/m4i7_sweep}
SEED_BASE=20260730
ARMS="${ARMS:-A B}"
# optional: pin ONE fetch path for arm B (0/1/2) instead of the shipped rule.
# It changes the TU, so it joins the kernel-dir and out-dir names.
PIN="${MPK_MOE_PATH_POLICY:-}"
SUF=""; [ -n "$PIN" ] && SUF="_p$PIN"
REPS="${REPS:-0 1 2}"
...
echo "########## M4-I7 MoE grouped-GEMM A/B  gpu=$GPU  $(date -Is) ##########"
...
run () {
  ...
  local OD=$M/noprof_${ARM}${SUF}
  [ -f "$OD/meta_bs${BS}_rep${REP}_${ARM}.json" ] && { echo "  [$TAG] cached"; return; }
  local KDIR=$M/kernel_${ARM}${SUF}_bs${BS}
  ...
  local ENVA=""
  if [ "$ARM" = A ]; then ENVA="MPK_MOE_BLOCKSCALE_BASELINE=1"
  elif [ -n "$PIN" ]; then ENVA="MPK_MOE_PATH_POLICY=$PIN"; fi
