#!/usr/bin/env python3
"""M4-I7 tables: the MoE grouped-GEMM integration A/B, per-rep and median.

Two arms:
  A  MPK_MOE_BLOCKSCALE_BASELINE=1  -- the frozen golden body (pre-M4-I7 code)
  B  default                        -- the ferret v012 fast body
...
def main():
    ...
    ap.add_argument("--root", default="/var/tmp/m4i7_sweep")
    ...
    lines = []
    P = lines.append
    P("M4-I7 MoE grouped-GEMM A/B -- geometry B (synthetic 256-token")
    ...
    txt = "\n".join(lines) + "\n"
    open(os.path.join(a.out, "m4i7_tables.txt"), "w").write(txt)
    json.dump({"arms": ARM_LABEL, "rows": rows,
              ...
              open(os.path.join(a.out, "m4i7_tables.json"), "w"), indent=1)
