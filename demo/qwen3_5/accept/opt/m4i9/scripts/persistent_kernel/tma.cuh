...
__host__ inline void create_tma_desc_by_task(FullTaskDesc &task_desc) {
  switch (task_desc.task_type) {
    ...
    // M4-I9 flag A's fused norm+quantize task (id 244): cp.async + register
    // elementwise over one token row, no TMA, but its id is inside the window.
    case TASK_RMS_NORM_QUANTIZE_FP8_SM100: {
      break;
    }
    default:
      assert(false);
