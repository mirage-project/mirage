...
enum TaskType {
  ...
  TASK_COPY = 232,
  TASK_CONCAT = 233,
  // Gated-DeltaNet causal depthwise conv1d + persistent conv-state pool.
  // Inside the (231, 256) TMA window, so tma.cuh needs a case for it even
  // though it uses no TMA descriptor.
  ...
  // M4-I9 flag A: RMS norm + fp32-block-scale FP8 quantize in one task, i.e.
  // `quantize_fp8` fused into its producer at the two pre-norm sites. The norm
  // already stages its bf16 output in shared memory, so the quantize reads the
  // same bytes from smem instead of from global. Also inside the (231, 256) TMA
  // window -> tma.cuh needs a no-TMA case.
  TASK_RMS_NORM_QUANTIZE_FP8_SM100 = 244,
