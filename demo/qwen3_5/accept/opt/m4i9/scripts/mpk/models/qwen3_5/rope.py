...
MOVED HERE by M2-I8 from `demo/qwen3_5/rope_permutation.py` (which now re-exports
this module): the registry builder lives inside the installed `mirage` package
and cannot import from `demo/`, and the permutation must have exactly ONE
definition — probe P4's proof, the M2-I6 kernel tests and the M2-I8 loader all
have to be talking about the same map.
...
Probe P4 (`demo/qwen3_5/accept/probes/attn/p4_rope_perm.py`) proves this
numerically against the HF oracle before the loader relies on it.
...
BLOCK-SCALE INTERACTION (found in M2-I8, see
`transforms.permute_fp8_blockscale_rows`): because both members of a rotated
Qwen pair live inside the first 64 columns of a head — one 128-row scale block —
and MPK's partner is exactly 128 rows away, this permutation ALWAYS moves rows
across `weight_scale_inv` block boundaries. It therefore cannot be applied to a
block-scaled fp8 weight by permuting the scale alongside it; see that function
for the resolution.
"""
