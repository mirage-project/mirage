#!/usr/bin/env bash
# M4-I9 GPU etiquette guard -- 3 samples 3s apart per candidate; the first
...
if [ "${1:-}" == "--" ]; then shift; fi
SAMPLES=3; SLEEP_S=3
LOCKFILE=$HOME/mpk-qwen35/.gpu-locks/M4-I9.lock
...
for gpu in "${CANDS[@]}"; do
  ...
  if [ "$ok" -eq 1 ]; then
    echo "GPU $gpu stable-idle across $SAMPLES samples -- claiming it (M4-I9)."
    echo "M4-I9 $gpu $(date -Iseconds) pid=$$" > "$LOCKFILE"
