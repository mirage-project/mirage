#!/usr/bin/env python3
"""M3-I10 remeasure fork of opt/schedule_sim.py -- ONE line changed in
steady_window's tie-break key (marked below). Everything else, including this
docstring's description of the underlying model, is verbatim.
...
--- THE ONE FIX (M3-I10 remeasure, arm A) -----------------------------------
steady_window's original tie-break key was (n_live, tokens, n_decode_active,
run_length) -- "most tokens in the step" ranked ABOVE "most of those tokens
being decode". That is silently correct only while decode's tokens-per-step
(<= batch_size, 1 per live request) stays below a prefill iteration's
mbt-bounded tokens-per-step. It holds for the AC-3 geometry (bs 1-8: mbt=16
usually exceeds a small bs) but INVERTS for arm A's uniform 256-token
synthetic prompts, where prefill iterations are ALSO mbt=16-token-bounded
while decode is still bs-per-step: at bs1/bs8 (1 and 8 tokens/step) the
all-prefill regime (16 tokens/step, n_live constant at 1/8 throughout,
tying the FIRST key) wins the old tie-break over the real decode_full regime
outright, so steady_window silently returned the PREFILL window instead of
decode (caught by this remeasure: bs1 arm A returned iterations [8,16), which
is entirely inside the 16-iteration prefill phase). Prepending "is this
regime prefill-free" to the key fixes the ordering (True=1 > False=0 in a
tuple compare, so an n_prefill==0 regime always outranks one that has
prefill activity) without changing anything else: ties among n_prefill==0
regimes, or the case where NO n_prefill==0 regime of length >= min_len
exists (bs16's own documented exception -- a live request is still
prefilling throughout), fall through to the exact original ranking.
------------------------------------------------------------------------------
"""
...
def steady_window(sim: dict, min_len: int = 5):
    ...
    batch size": PREFILL-FREE first (see module docstring's "THE ONE FIX"),
    then most live requests, then most tokens in the step, then most of those
    tokens coming from decode, then longest.
