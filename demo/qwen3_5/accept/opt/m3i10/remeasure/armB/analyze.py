#!/usr/bin/env python3
"""M3-I10 remeasure fork of demo/qwen3_5/accept/opt/analyze.py -- IDENTICAL
logic, only BSL is narrowed to the batch sizes this isolated re-measure
actually captured (arm A: tier 1 = {1,8,16}; tier 3 would add {2,4}), and it
is deployed under opt/m3i10/remeasure/armA/ instead of opt/ itself so it
never overwrites the M3-I1-committed opt/{tables,meta,meta_noprof,*.csv}
(msl=132) baseline. VLLM/M2_WAVE below are the SAME stale M1-I6/M2-I9
constants the original file has always used for gap_vs_vllm.csv; that output
is not this issue's comparison (m3i10/scripts/build_comparison.py is) and is
kept only so this stays a faithful fork of the committed pipeline.

Inputs (under this directory, i.e. .../remeasure/armA/):
...
Outputs (same directory):
  ...
  gap_vs_vllm.csv       decode-throughput gap re-derivation (stale baseline, informational only)
"""
...
BSL = [1, 8, 16]  # <-- ONLY DELTA from the committed opt/analyze.py (was [1,2,4,8,16])
NW = 128
