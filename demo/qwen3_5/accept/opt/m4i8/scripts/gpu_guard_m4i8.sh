#!/usr/bin/env bash
# M4-I8 GPU etiquette guard -- 3 samples 3s apart per candidate; the first
# candidate idle on EVERY sample wins, claim a lock, pin CUDA_VISIBLE_DEVICES,
# exec. Fails closed (rc 97). Two ferret chains (workspaces 3 and 6) pick their
...
if [ "${1:-}" == "--" ]; then shift; fi
SAMPLES=3; SLEEP_S=3
LOCKFILE=$HOME/mpk-qwen35/.gpu-locks/M4-I8.lock
...
for gpu in "${CANDS[@]}"; do
  ...
  if [ "$ok" -eq 1 ]; then
    echo "GPU $gpu stable-idle across $SAMPLES samples -- claiming it (M4-I8)."
    echo "M4-I8 $gpu $(date -Iseconds) pid=$$" > "$LOCKFILE"
