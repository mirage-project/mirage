#!/usr/bin/env bash
# M4-I8 ARM S: the SAME profiled capture with MPK_EVENT_WAIT_GPU_SCOPE=1, so
# sched_gap.py can re-derive data_gap / resource_gap / step and show the
# dispatch-latency terms actually moved. An e2e win with unchanged gaps would
# mean something else moved and the attribution is wrong.
...
export MPK_EVENT_WAIT_GPU_SCOPE=1
echo "########## M4-I8 profiled capture ARM S  gpu=$GPU  $(date -Is) ##########"
echo "MPK_EVENT_WAIT_GPU_SCOPE=$MPK_EVENT_WAIT_GPU_SCOPE"
