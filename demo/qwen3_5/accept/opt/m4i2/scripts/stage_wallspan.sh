#!/usr/bin/env bash
# M4-I2: the dense-fp8 STAGE WALLSPAN, before vs after, and the width residual.
#
...
echo "########## M4-I2 stage wallspan  gpu=$GPU  $(date -Is) ##########"
...
for BS in $BSLIST; do
  for ARM in $ARMS; do
    ...
    if [ ! -f "$OD/raw_bs${BS}_rep0.npz" ]; then
      ...
      drain
      ENVA=""; [ "$ARM" = A ] && ENVA="MPK_FP8_DENSE_BASELINE=1"
