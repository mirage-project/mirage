#!/usr/bin/env bash
# M4-I2 GATE 3 -- FULL AC-3 at all five batch sizes on the integrated tree.
#
...
# is bit-exact against the golden path (Gate 1: 30/30 shapes, both nvcc flag
# lanes, both data regimes; and the A/B's tokens_sha256 matched in all 15 pairs)
...
#
# Arm: DEFAULT (the shipped configuration). MPK_FP8_DENSE_BASELINE is deliberately
# unset -- the baseline arm's generated megakernel source is byte-identical to the
# pre-M4-I2 tree's (verified: same TU sha256), so it is the committed state and
...
M=${M:-/var/tmp/m4i2_ac3}
REPS=${REPS:-3}
BSLIST=${BSLIST:-1,2,4,8,16}
mkdir -p "$M"

echo "########## M4-I2 AC-3 (shipped arm)  gpu=$GPU  $(date -Is) ##########"
echo "tree: $T  HEAD=$(git -C "$T" rev-parse --short HEAD)"
echo "task279 kernel:"; sha256sum "$T/include/mirage/persistent_kernel/tasks/blackwell/linear_fp8_blockscale_sm100.cuh"
"$PY" -c "import sys;sys.path.insert(0,'$T/python');import mirage,os;print('mirage from',os.path.realpath(mirage.__file__))"
echo "MPK_FP8_DENSE_BASELINE=${MPK_FP8_DENSE_BASELINE:-<unset, shipped arm>}"
...
if ls "$ASSM"/bs*.json >/dev/null 2>&1; then
  "$PY" -u "$ACC/harness/run_ac3.py" --engine-dump-dir "$ASSM" \
      --batch-sizes 1,2,4,8,16 --output-json "$M/run_report_m4i2.json" \
      2>&1 | tail -30
