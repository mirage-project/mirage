#!/usr/bin/env bash
# M3-I2b A/B capture driver (runs ON the B200 box, under the GPU guard).
#
#   bash gpu_guard_m3i2b.sh 6,0,1,2,3,4,5,7 -- bash run_m3i2b.sh <arm> <mode>
#
# arm  = base | v1 | v2 ...   (source snapshot under $M/arms/<arm>/)
# mode = all | prof | noprof | parse
#
# Reuses M3-I1's capture pipeline VERBATIM (profile_wave.py / parse_profile.py /
# trace_lib.py) so the before/after numbers are produced by the same instrument.
# One wave per PROCESS, AC-3 geometry pinned (msl=132, mbt=16, page 256, 64 new
# tokens), the same ascending-length prompt sets as I1.
set -uo pipefail
...
echo "M3I2B_DRIVER_EXIT=0 arm=$ARM mode=$MODE $(date -Is)"
