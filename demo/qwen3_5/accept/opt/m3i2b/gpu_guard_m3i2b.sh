#!/usr/bin/env bash
# M3-I2b GPU etiquette guard — same contract as opt/gpu_guard_m3i1.sh:
# 3 samples 3s apart per candidate, first candidate idle on EVERY sample wins,
# claims .gpu-locks/M3-I2b.lock, pins CUDA_VISIBLE_DEVICES, execs the command.
# Fails closed (rc 97). MPK megakernel runs need the GPU EXCLUSIVE -- M3-I2a
# root-caused the wave deadlock as CO-TENANCY, so this is correctness, not just
# etiquette.
set -uo pipefail
CANDIDATES="$1"; shift
if [ "${1:-}" == "--" ]; then shift; fi
SAMPLES=3; SLEEP_S=3
LOCKFILE=$HOME/mpk-qwen35/.gpu-locks/M3-I2b.lock
...
for gpu in "${CANDS[@]}"; do
  ...
  if [ "$ok" -eq 1 ]; then
    echo "GPU $gpu stable-idle across $SAMPLES samples -- claiming it (M3-I2b)."
    echo "M3-I2b $gpu $(date -Iseconds) pid=$$" > "$LOCKFILE"
