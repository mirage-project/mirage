#!/usr/bin/env bash
# M3-I9 GPU etiquette guard -- same contract as opt/gpu_guard_m3i1.sh and
# m3i2b/gpu_guard_m3i2b.sh: 3 samples 3s apart per candidate, first candidate
# idle on EVERY sample wins, claims .gpu-locks/M3-I9.lock, pins
...
if [ "${1:-}" == "--" ]; then shift; fi
SAMPLES=3; SLEEP_S=3
LOCKFILE=$HOME/mpk-qwen35/.gpu-locks/M3-I9.lock
...
for gpu in "${CANDS[@]}"; do
  ...
  if [ "$ok" -eq 1 ]; then
    echo "GPU $gpu stable-idle across $SAMPLES samples -- claiming it (M3-I9)."
    echo "M3-I9 $gpu $(date -Iseconds) pid=$$" > "$LOCKFILE"
