#!/usr/bin/env bash
# M3-I1 GPU etiquette guard (same contract as scripts/gpu_guard_i8.sh):
# 3 samples 3s apart per candidate, first candidate idle on EVERY sample wins,
# claims .gpu-locks/M3-I1.lock, pins CUDA_VISIBLE_DEVICES, execs the command.
# Fails closed (rc 97). MPK megakernel runs need the GPU EXCLUSIVE -- it can
# deadlock when shared (resources.md).
set -uo pipefail
CANDIDATES="$1"; shift
if [ "${1:-}" == "--" ]; then shift; fi
SAMPLES=3; SLEEP_S=3
LOCKFILE=$HOME/mpk-qwen35/.gpu-locks/M3-I1.lock
...
for gpu in "${CANDS[@]}"; do
  ...
  if [ "$ok" -eq 1 ]; then
    echo "GPU $gpu stable-idle across $SAMPLES samples -- claiming it (M3-I1)."
    echo "M3-I1 $gpu $(date -Iseconds) pid=$$" > "$LOCKFILE"
