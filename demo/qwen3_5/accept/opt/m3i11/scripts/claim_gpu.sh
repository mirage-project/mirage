#!/usr/bin/env bash
# M3-I11 GPU etiquette guard (print-only): 3 samples 3s apart per candidate,
# first candidate idle on EVERY sample wins, claims .gpu-locks/M3-I11.lock and
# PRINTS the index on stdout. Fails closed (rc 97, nothing on stdout).
set -uo pipefail
CANDIDATES="${1:-6,5,4,3,2,0}"
SAMPLES=3; SLEEP_S=3
LOCKFILE=$HOME/mpk-qwen35/.gpu-locks/M3-I11.lock
...
for gpu in "${CANDS[@]}"; do
  ...
  if [ "$ok" -eq 1 ]; then
    echo "M3-I11 $gpu $(date -Iseconds) pid=$$" > "$LOCKFILE"
    echo "GPU $gpu stable-idle across $SAMPLES samples -- claimed (M3-I11)." >&2
    echo "$gpu"
    exit 0
  fi
