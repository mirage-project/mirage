#!/usr/bin/env bash
# M4-I5 GPU etiquette guard -- same contract as opt/gpu_guard_m3i1.sh and
# m3i6a/scripts/gpu_guard_i6a.sh: 3 samples 3s apart per candidate, the first
# candidate idle on EVERY sample wins, claim a lock, pin CUDA_VISIBLE_DEVICES,
# exec the command.  Fails closed (rc 97).  An MPK megakernel run needs the GPU
# EXCLUSIVE -- it can deadlock when shared (resources.md), and a single read at
# claim time is not evidence of an idle device (.memory/main/b200-env.md).
set -uo pipefail
CANDIDATES="$1"; shift
if [ "${1:-}" == "--" ]; then shift; fi
SAMPLES=3; SLEEP_S=3
LOCKFILE=$HOME/mpk-qwen35/.gpu-locks/M4-I5.lock
...
for gpu in "${CANDS[@]}"; do
  ...
  if [ "$ok" -eq 1 ]; then
    echo "GPU $gpu stable-idle across $SAMPLES samples -- claiming it (M4-I5)."
    echo "M4-I5 $gpu $(date -Iseconds) pid=$$" > "$LOCKFILE"
