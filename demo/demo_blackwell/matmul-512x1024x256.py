...
if __name__ == "__main__":
    ...
    tb_graph = mi.new_threadblock_graph(grid_dim=(4,4,1), block_dim=(256,1,1), forloop_range=4, reduction_dimx=64)
    ...
    input_tensors = [
        torch.randn(512, 256, dtype=torch.float16, device='cuda:0')*0.01,
        torch.randn(256, 1024, dtype=torch.float16, device='cuda:0')*0.01,
    ]

    outputs = graph(inputs=input_tensors, num_warp_groups = 2, pipeline_stages = 4, profiling = args.profiling, file_id = 3, target_cc = 100)
