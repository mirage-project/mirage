...
if __name__ == "__main__":
    ...
    Q = graph.new_input(dims=(8, 256, 64), dtype=mi.float16)
    K = graph.new_input(dims=(8, 64, 4096), dtype=mi.float16)
    V = graph.new_input(dims=(1, 4096, 64), dtype=mi.float16)
    W = graph.new_input(dims=(8, 64, 64), dtype=mi.float16)
    ...
    bV = tbgraph1.new_input(dtensor=V, input_map=(-1, 1, -1), forloop_dim=1)
    bW = tbgraph1.new_input(dtensor=W, input_map=(0, -1, -1), forloop_dim=-1)
    bV = tbgraph1.matmul(bV, bW)
    ...
    O = graph.customized([Q, K, V, W], tbgraph1)
    ...
    input_tensors = [
        torch.randn(8, 256, 64, dtype=torch.float16, device='cuda:0'),
        torch.randn(8, 64, 4096, dtype=torch.float16, device='cuda:0'),
        torch.randn(1, 4096, 64, dtype=torch.float16, device='cuda:0'),
        torch.randn(8, 64, 64, dtype=torch.float16, device='cuda:0')
    ]
