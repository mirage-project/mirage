...
@triton.jit
def rotary_kernel(
    out_q_ptr, q_ptr, cos_ptr, sin_ptr, seq_len, rotary_dim,
    # batch_size, seq_len, num_heads, head_dim, half_dim,
    ...
    q_ptr = q_ptr + (rm[:, None] * q_stride_s + rk_half[None, :] * q_stride_d)
    cos_ptr = cos_ptr + (rm_cs[:, None] * rotary_dim_half*2 + rk_half[None, :])
    sin_ptr = sin_ptr + (rm_cs[:, None] * rotary_dim_half*2 + rk_half[None, :])
    ...
    cos = tl.load(
        cos_ptr, mask=(rm_cs[:, None] < seq_len) & (rk_half[None, :] < rotary_dim_half), other=1.0
        )
    sin = tl.load(
        sin_ptr, mask=(rm_cs[:, None] < seq_len) & (rk_half[None, :] < rotary_dim_half), other=0.0
        )

    q_first = tl.load(
        q_ptr, mask=(rm[:, None] < seq_len) & (rk_half[None, :] < rotary_dim_half), other=0.0
        )
    q_second = tl.load(
        q_ptr + rotary_dim_half * q_stride_d, 
        mask=(rm[:, None] < seq_len) & (rk_half[None, :] < rotary_dim_half), 
        other=0.0
        )
    ...
    out_q_ptr = out_q_ptr + (rm[:, None] * out_q_stride_s + rk_half[None, :] * out_q_stride_d)
    
    tl.store(out_q_ptr, q_out_first, 
             mask = (rm[:, None] < seq_len) & (rk_half[None, :] < rotary_dim_half))
            #  mask=s_mask[:, None] & rk_half_mask[None, :])
    tl.store(out_q_ptr + rotary_dim_half * out_q_stride_d, q_out_second, 
             mask = (rm[:, None] < seq_len) & (rk_half[None, :] < rotary_dim_half),)
...
def apply_rotary_pos_emb_triton(q, k, cos, sin, position_ids=None, unsqueeze_dim=1):
    ...
    _, seqlen_ro, rotary_dim = cos.shape
    ...
    print(batch_size, seq_len, num_heads, head_dim, half_dim, BLOCK_M, BLOCK_K, rotary_dim)
    print("cos shape: ", cos.shape)
    print("sin shape: ", sin.shape)
    time1 = time.time()
    with torch.cuda.device(q.device.index):
        rotary_kernel[grid](
            q_out, q, cos, sin, seq_len, rotary_dim,
            # batch_size, seq_len, num_heads, head_dim, half_dim,
            q_stride[0], q_stride[1], q_stride[2], q_stride[3],
            # q.stride(0), q.stride(-3), q.stride(-2), q.stride(-1),
