...
if __name__ == "__main__":
    ...
    if args.prompts_file is not None:
        ...
        assert isinstance(prompt_texts, list) and all(
            isinstance(p, str) for p in prompt_texts
        ), "--prompts-file must contain a JSON list of strings"
        assert len(prompt_texts) > 0, "--prompts-file must contain at least one prompt"
        assert args.use_mirage, "--prompts-file is currently supported for Mirage runs"
        assert len(prompt_texts) <= args.max_num_batched_requests, (
            f"got {len(prompt_texts)} prompts but max_num_batched_requests is "
            f"{args.max_num_batched_requests}"
        )

    total_num_requests = (
        len(prompt_texts)
        if prompt_texts is not None
        else (1 if not args.use_mirage else args.max_num_batched_requests)
    )
    ...
    if prompt_texts is None:
        prompt_texts = [prompt] * total_num_requests
    texts = []
    ...
    prompt_lengths_cpu = model_inputs.attention_mask.sum(dim=1).to(torch.int32)
    for r in range(total_num_requests):
        prompt_len = int(prompt_lengths_cpu[r].item())
        valid_input_ids = model_inputs.input_ids[r][model_inputs.attention_mask[r].bool()]
        tokens[r, :prompt_len] = valid_input_ids
    prompt_lengths = prompt_lengths_cpu.to(device="cuda", dtype=torch.int32)
    ...
    if args.use_mirage:
        ...
        for i, layer in enumerate(model.model.layers):
            ...
            elif mpk.target_cc == 100 and not args.split_kv_cache:
                mpk.planning_paged_attention_layer(
                    input=attn_in,
                    k_cache=k_cache,
                    v_cache=v_cache,
                    q_norm=w_q_norm,
    ...
    if not args.use_mirage:
        ...
        if save_path and rank == 0:
            ...
            requests = []
            for r in range(total_num_requests):
                req_end_idx = step[r].item() + 1
                req_prompt_len = prompt_lengths[r].item()
                req_slice_end = min(req_end_idx, req_prompt_len + MAX_SAVE_TOKENS)
                requests.append(
                    {
                        "request_id": r,
                        "token_ids": tokens[r, req_prompt_len:req_slice_end].tolist(),
                        "text": tokenizer.decode(
                            tokens[r, :req_end_idx], skip_special_tokens=True
                        ),
                        "prompt_length": req_prompt_len,
                        "generate_length": max(0, req_end_idx - req_prompt_len),
                    }
                )
            out = {
                "token_ids": token_ids,
