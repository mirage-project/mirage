...
if __name__ == "__main__":
    ...
    if args.use_mirage:
        ...
        mpk = mi.PersistentKernel(
            ...
            max_seq_length=512,
            eos_token_id=0,  # Set to 0 to trigger immediate termination after one forward
        ...
        # Instead of argmax, we'll use softmax and then extract top-k
        softmax_in = mpk.new_tensor(
            dims=(batch_size, vocab_size),
            dtype=mi.bfloat16,
            name="softmax_in",
            io_category="cuda_tensor",
        )
        # Create softmax output buffer that we can read back
        softmax_output_buffer = torch.zeros(batch_size, vocab_size, dtype=torch.bfloat16, device='cuda')
        softmax_out = mpk.attach_input(
            torch_tensor=softmax_output_buffer,
            name="softmax_out"
        )
        ...
        # Add softmax layer instead of argmax
        mpk.softmax_layer(
            input=softmax_in,
            output=softmax_out,
            grid_dim=(1, 1, 1),
        ...
        # Note: MPK doesn't have built-in topk_layer
        # We'll extract top-k from softmax output after execution
    ...
    if not args.use_mirage:
        # Non-Mirage path - do single forward pass
        ...
        # Also predict what comes after the selected token using PyTorch
        print(f"\n--- PyTorch prediction for comparison ---")
        next_token_torch = topk_indices[0].item()  # Get the top token
        print(f"\nPrefill selected token: '{tokenizer.decode([next_token_torch])}' (id: {next_token_torch})")
        tokens[0, prompt_len] = next_token_torch
        
        # Do one more forward pass to predict what comes after
        step.fill_(prompt_len)
        input_ids = tokens[:, prompt_len:prompt_len+1]
        cos_embeddings = position_embeddings[0][:, prompt_len:prompt_len+1]
        sin_embeddings = position_embeddings[1][:, prompt_len:prompt_len+1]
        
        logits_next = model.forward(
            input_ids=input_ids,
            position_embeddings=(cos_embeddings, sin_embeddings),
            step=step,
            stream=None,
        ...
        probs_next = torch.nn.functional.softmax(logits_next[0, -1, :], dim=-1)
        topk_probs_next, topk_indices_next = torch.topk(probs_next, k=k)
        
        print(f"After '{tokenizer.decode([next_token_torch])}', PyTorch predicts:")
        for i in range(k):
            token_id = topk_indices_next[i].item()
            prob = topk_probs_next[i].item()
            token_str = tokenizer.decode([token_id])
            if prob < 0.0001:
                print(f"Token {i+1}: '{token_str}' (id: {token_id}, probability: {prob:.2e})")
            else:
                print(f"Token {i+1}: '{token_str}' (id: {token_id}, probability: {prob:.4f})")
    else:
        # Mirage path - we'll use MPK to process the prompt and get softmax output
        # First, we need to do a prefill phase to populate KV caches
        print("\nRunning prefill phase to populate KV caches...")
        step.fill_(prompt_len - 1)
        input_ids = tokens[:, 0:prompt_len]
        cos_embeddings = position_embeddings[0][:, 0:prompt_len]
        sin_embeddings = position_embeddings[1][:, 0:prompt_len]
        
        # Run prefill to populate KV caches and get logits
        logits = model.forward(
            input_ids=input_ids,
            position_embeddings=(cos_embeddings, sin_embeddings),
            step=step,
        ...
        # Get probabilities from the prefill phase using softmax
        probs = torch.nn.functional.softmax(logits[0, -1, :], dim=-1)
        ...
        # Now prepare for MPK generation
        next_token = logits.argmax(dim=-1)
        next_token = next_token[0, -1]
        tokens[0, prompt_len] = next_token
        input_tokens[0, 0] = next_token
        
        print(f"\nPrefill selected token: '{tokenizer.decode([next_token])}' (id: {next_token})")
        
        # Set step to prompt_len for MPK to predict the next token
        step.fill_(prompt_len)
        tokens[0, 1] = 0  # This ensures tokens[1] == eos_token_id, triggering termination
        
        print(f"\nNow using MPK to predict what comes after '{tokenizer.decode([next_token])}'...")
        starter.record()
        ...
        print(f"MPK forward pass completed in {run_time:.2f} ms")
        
        # Extract top-k from MPK's softmax output
        topk_probs_mpk, topk_indices_mpk = torch.topk(softmax_output_buffer[0, :model.config.vocab_size], k=k)
        
        print(f"\nMPK's top-{k} predictions for next token:")
        for i in range(k):
            token_id = topk_indices_mpk[i].item()
            prob = topk_probs_mpk[i].item()
            token_str = tokenizer.decode([token_id])
            if prob < 0.0001:
                print(f"Token {i+1}: '{token_str}' (id: {token_id}, probability: {prob:.2e})")
            else:
