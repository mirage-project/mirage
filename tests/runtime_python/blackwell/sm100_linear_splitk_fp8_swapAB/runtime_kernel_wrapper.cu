...
struct ShapeCfg {
  static_assert(OUTPUT_SIZE % 128 == 0,
                "OUTPUT_SIZE must be a multiple of 128");
  static_assert(BATCH > 0 && (BATCH <= 16 || BATCH == 128),
                "BATCH must be in (0, 16] or 128");
  static_assert(K_PER_TASK % 128 == 0, "K_PER_TASK must be a multiple of 128");

  using TMA_A = kernel::tma::tma_2d<
      ...
      /*GMEM_STRIDE_COL=*/1,
      /*SMEM_REPEAT_ROW=*/1,
      /*SMEM_REPEAT_COL=*/(TILE_SIZE + TMA_CP_ASYNC_SIZE - 1) /
          TMA_CP_ASYNC_SIZE,
      /*SMEM_STRIDE=*/MMA_M * TMA_CP_ASYNC_SIZE,
      /*ROW_MAJOR=*/true>;

  using TMA_B = kernel::tma::tma_2d<
      cutlass::float_e4m3_t,
      B_swz,
      M_swz,
      S_swz,
...
CUtensorMap make_input_desc(void *gmem_base_for_slice,
  ...
  CUresult r = cuTensorMapEncodeTiled(&desc,
...
CUtensorMap make_output_desc(void *gmem_ptr, int batch, int output_size) {
  CUtensorMap desc;
...
template <int BATCH, int OUTPUT_SIZE, int K_PER_TASK>
__global__ void
    linear_splitk_fp8_swapAB_kernel_wrapper(CUtensorMap *input_desc,
  ...
  auto layout_bias =
      cute::make_layout(cute::make_shape(BATCH, OUTPUT_SIZE),
...
void launch_linear_splitk_fp8_swapAB(torch::Tensor &input_q,
  ...
  CUtensorMap host_out_desc =
      make_output_desc(output.data_ptr(), BATCH, OUTPUT_SIZE);
  TORCH_CHECK(cudaMemcpy(d_out_desc,
                         &host_out_desc,
                         sizeof(CUtensorMap),
  ...
  for (int s = 0; s < split_k_factor; ++s) {
    ...
    CUtensorMap host_w =
        make_weight_desc(w_slice, OUTPUT_SIZE, K_PER_TASK, full_K);
    ...
    TORCH_CHECK(cudaMemcpy(d_in_descs[s],
                           &host_in,
                           sizeof(CUtensorMap),
                           cudaMemcpyHostToDevice) == cudaSuccess);
    TORCH_CHECK(cudaMemcpy(d_w_descs[s],
                           &host_w,
                           sizeof(CUtensorMap),
                           cudaMemcpyHostToDevice) == cudaSuccess);
  }
  ...
  TORCH_CHECK(
      cudaFuncSetAttribute(linear_splitk_fp8_swapAB_kernel_wrapper<BATCH,
                                                                   OUTPUT_SIZE,
                                                                   K_PER_TASK>,
                           cudaFuncAttributeMaxDynamicSharedMemorySize,
                           static_cast<int>(smem_bytes)) == cudaSuccess,
      "cudaFuncSetAttribute failed");
  TORCH_CHECK(cudaDeviceSetLimit(cudaLimitStackSize, 128 * 1024) ==
              cudaSuccess);
  ...
  for (int r = 0; r < repeat; ++r) {
    for (int s = 0; s < split_k_factor; ++s) {
      ...
      uint32_t const *w_scale_slice = wscale_base + s * packed_k_per_task;
      uint32_t const *i_scale_slice = iscale_base + s * packed_k_per_task;
      cudaError_t le = cudaLaunchKernelEx(
          &cfg,
          linear_splitk_fp8_swapAB_kernel_wrapper<BATCH,
                                                  OUTPUT_SIZE,
                                                  K_PER_TASK>,
          d_in_descs[s],
          d_w_descs[s],
...
#define DISPATCH_SHAPE(b, n, k)                                                \
  if (BATCH == (b) && OUTPUT_SIZE == (n) && K_PER_TASK == (k)) {               \
    launch_linear_splitk_fp8_swapAB<(b), (n), (k)>(input_q,                    \
                                                   input_scale,                \
                                                   weight_q,                   \
                                                   weight_scale,               \
                                                   output,                     \
...
void linear_splitk_fp8_swapAB_sm100(
  ...
  TORCH_CHECK(K_PER_TASK % 512 == 0,
              ...
              "divide full_K / 512 evenly): got K_per_task=",
              K_PER_TASK);

  DISPATCH_FOR_BATCH(1)
  DISPATCH_FOR_BATCH(4)
  DISPATCH_FOR_BATCH(16)
  DISPATCH_FOR_BATCH(128)
...
std::vector<std::tuple<int, int, int>> supported_shapes() {
  ...
  static constexpr int kNs[] = {
      128, 128, 128, 128, 128, 128, 128, 128, 256, 256};
  static constexpr int kKs[] = {
      512, 1024, 1536, 2048, 3584, 4096, 4608, 7168, 2048, 4096};
  for (int b : {1, 4, 16, 128}) {
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  m.def("linear_splitk_fp8_swapAB_sm100",
        &linear_splitk_fp8_swapAB_sm100,
        ...
        pybind11::arg("input_q"),
        pybind11::arg("input_scale"),
        pybind11::arg("weight_q"),
        pybind11::arg("weight_scale"),
        pybind11::arg("output"),
        pybind11::arg("split_k_factor"),
        pybind11::arg("repeat") = 1);
  m.def("supported_shapes",
        &supported_shapes,
