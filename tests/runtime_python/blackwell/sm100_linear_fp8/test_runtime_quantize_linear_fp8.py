...
for reduction_size in reduction_sizes:
    for output_size in output_sizes:
        print(
            f"\n=== Runtime quantize + linear test: batch_size={batch_size} output_size={output_size} reduction_size={reduction_size} ==="
        )
        ...
        x_scale = torch.empty((batch_size, padded_scale_k), device="cuda", dtype=torch.uint32)
        w_scale = torch.empty((output_size, padded_scale_k), device="cuda", dtype=torch.uint32)
        ...
        linear_kernel.linear_fp8_1d2d_sm100(x_q, x_scale, w_q, w_scale, residual, output)
