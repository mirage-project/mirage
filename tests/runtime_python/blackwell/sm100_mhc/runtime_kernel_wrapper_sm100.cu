...
__global__ __launch_bounds__(256, 1) void mHC_linear_wrapper(
  ...
  using TMA_B = kernel::tma::tma_2d<mpk_bf16, B, M, S,
                                    ...
                                    true>;
  using TMA_A = kernel::tma::tma_2d<mpk_bf16, B, M, S,
                                    ...
                                    true>;
  using TMA_OUT = kernel::tma::tma_2d<mpk_bf16, 0, M, S,
                                      BATCH_SIZE,             /*GMEM_ROW_*/
  ...
  kernel::mHC_linear_task_impl<mpk_bf16, TMA_A, TMA_B, TMA_OUT,
...
void launch_mHC_linear(void *input_ptr,
  ...
  mirage::runtime::fill_tma_desc<mpk_bf16, B, M, S, 2>(
      &host_w_desc, static_cast<mpk_bf16 *>(weight_ptr),
      w_gmem_shape, w_gmem_stride, w_smem_shape, 1, TILE_REPEAT_COL);
  ...
  uint32_t i_smem_shape[2] = {static_cast<uint32_t>(MMA_N),
                              static_cast<uint32_t>(TMA_CP_ASYNC_SIZE)};
  mirage::runtime::fill_tma_desc<mpk_bf16, B, M, S, 2>(
      &host_i_desc, static_cast<mpk_bf16 *>(input_ptr),
      i_gmem_shape, i_gmem_stride, i_smem_shape, 1, TILE_REPEAT_COL);
  ...
  mirage::runtime::fill_tma_desc<mpk_bf16, 0, M, S, 2>(
      &host_o_desc, static_cast<mpk_bf16 *>(output_ptr),
      o_gmem_shape, o_gmem_stride, o_smem_shape, 1, 1);
  ...
  cudaMemcpyAsync(desc_w_ptr, &host_w_desc, sizeof(CUtensorMap),
                  cudaMemcpyHostToDevice, stream);
  cudaMemcpyAsync(desc_i_ptr, &host_i_desc, sizeof(CUtensorMap),
                  cudaMemcpyHostToDevice, stream);
  cudaMemcpyAsync(desc_o_ptr, &host_o_desc, sizeof(CUtensorMap),
                  cudaMemcpyHostToDevice, stream);
  ...
  cutlass::launch_kernel_on_cluster(params, (void const *)kernel_ptr,
                                    desc_w_ptr, desc_i_ptr, desc_o_ptr);
}
...
#define DISPATCH_MHC_LINEAR_BS_K(BS)                                           \
  switch (red) {                                                               \
    ...
    case 16384:                                                                \
      ...
      break;                                                                   \
    default:                                                                   \
      TORCH_CHECK(false, "Unsupported K=", red,                                \
                  " (must be one of {256,512,1024,2048,4096,8192,16384})");    \
  }
...
void mHC_linear(torch::Tensor input,
  ...
  switch (bs) {
    case 16:    DISPATCH_MHC_LINEAR_BS_K(16);    break;
    case 64:    DISPATCH_MHC_LINEAR_BS_K(64);    break;
    case 256:   DISPATCH_MHC_LINEAR_BS_K(256);   break;
    case 1024:  DISPATCH_MHC_LINEAR_BS_K(1024);  break;
    case 4096:  DISPATCH_MHC_LINEAR_BS_K(4096);  break;
    case 8192:  DISPATCH_MHC_LINEAR_BS_K(8192);  break;
    case 16384: DISPATCH_MHC_LINEAR_BS_K(16384); break;
    default:
...
__global__ __launch_bounds__(256) void mHC_affine_split_activation_kernel(
...
void mHC_affine_split_activation(torch::Tensor mixes,
  ...
  int const num_ctas = resolve_num_ctas(num_ctas_arg, mixes.get_device());
...
__global__ __launch_bounds__(256, 1) void mHC_hc_pre_v3_kernel(
  ...
  // wrapper. Each CTA loops grid-strided over n_tiles inside the task impl.
  {
    ...
    using TMA_B = kernel::tma::tma_2d<mpk_bf16, B, M, S,
                                      ...
                                      MMA_N * TMA_CP_ASYNC_SIZE, true>;
    using TMA_A = kernel::tma::tma_2d<mpk_bf16, B, M, S,
                                      OUTPUT_SIZE, REDUCTION_SIZE,
                                      MMA_M, TMA_CP_ASYNC_SIZE,
                                      REDUCTION_SIZE, 1, 1,
                                      TMA_CP_ASYNC_REPEAT_COL,
                                      MMA_M * TMA_CP_ASYNC_SIZE, true>;
    using TMA_OUT = kernel::tma::tma_2d<mpk_bf16, 0, M, S,
    ...
    kernel::mHC_linear_task_impl<mpk_bf16, TMA_A, TMA_B, TMA_OUT,
                                 MMA_M, MMA_N, BATCH_SIZE,
                                 OUTPUT_SIZE, REDUCTION_SIZE>(tma_a, tma_b, tma_out);
  }
  ...
  kernel::mHC_hc_pre_tail_fused_v2_dyn_smem_task_impl<
      mpk_bf16, /*N=*/4, TAIL_C, TOKENS_PER_CTA,
      /*BLOCK_THREADS=*/256, /*MIX_STRIDE=*/OUTPUT_SIZE>(
      mixes_pad, scale_ptr, base_ptr, x_orig_bf16,
...
void mHC_hc_pre_v3(torch::Tensor x_fp32,
  ...
  TORCH_CHECK(x_norm_scratch.sizes() == x_fp32.sizes() &&
                  x_norm_scratch.is_cuda() &&
                  x_norm_scratch.is_contiguous() &&
  ...
  mirage::runtime::fill_tma_desc<mpk_bf16, B_, M_, S_, 2>(
      &host_w_desc, static_cast<mpk_bf16 *>(weight_padded.data_ptr()),
      w_gmem_shape, w_gmem_stride, w_smem_shape, 1, TILE_REPEAT_COL);
  ...
  mirage::runtime::fill_tma_desc<mpk_bf16, B_, M_, S_, 2>(
      &host_i_desc, static_cast<mpk_bf16 *>(x_norm_scratch.data_ptr()),
      i_gmem_shape, i_gmem_stride, i_smem_shape, 1, TILE_REPEAT_COL);
  ...
  mirage::runtime::fill_tma_desc<mpk_bf16, 0, M_, S_, 2>(
      &host_o_desc, static_cast<mpk_bf16 *>(mixes_pad_scratch.data_ptr()),
      o_gmem_shape, o_gmem_stride, o_smem_shape, 1, 1);
  ...
  cudaMemcpyAsync(desc_buf + 0, &host_w_desc, sizeof(CUtensorMap),
                  cudaMemcpyHostToDevice, stream);
  cudaMemcpyAsync(desc_buf + 1, &host_i_desc, sizeof(CUtensorMap),
                  cudaMemcpyHostToDevice, stream);
  cudaMemcpyAsync(desc_buf + 2, &host_o_desc, sizeof(CUtensorMap),
                  cudaMemcpyHostToDevice, stream);
  ...
  int const grid_count =
      num_ctas < linear_n_tiles ? num_ctas : linear_n_tiles;
...
#define LAUNCH_V3(BS, K, C, TPC)                                               \
  do {                                                                         \
    auto *kernel_ptr = &mHC_hc_pre_v3_kernel<                                  \
        OUT_PAD, K, MMA_M, MMA_N, BS, K, C, TPC>;                              \
    CUTE_CHECK_ERROR(cudaFuncSetAttribute(                                     \
    ...
    void *kargs[] = {&x_fp32_p, &x_norm_p, &desc_a, &desc_b, &desc_o,          \
                     &mixes_p, &scale_p, &base_p, &x_orig_p,                   \
                     &f_pre_p, &h_post_p, &comb_p,                             \
                     &bs_arg, &sk_repeat_arg, &sk_eps_f, &rms_eps_f};          \
    CUTE_CHECK_ERROR(cudaLaunchCooperativeKernel(                              \
        (void const *)kernel_ptr, grid_dim, block_dim, kargs,                  \
        smemBytes, stream));                                                   \
  } while (0)
  ...
  switch (c) {                                                                 \
    case 128:                                                                  \
      switch (tokens_per_cta) {                                                \
        case 32:  LAUNCH_V3(BS_, K_, 128, 32);  break;                         \
        case 64:  LAUNCH_V3(BS_, K_, 128, 64);  break;                         \
        case 128: LAUNCH_V3(BS_, K_, 128, 128); break;                         \
      }                                                                        \
      break;                                                                   \
    case 1024:                                                                 \
      switch (tokens_per_cta) {                                                \
        case 32:  LAUNCH_V3(BS_, K_, 1024, 32);  break;                        \
        case 64:  LAUNCH_V3(BS_, K_, 1024, 64);  break;                        \
        case 128: LAUNCH_V3(BS_, K_, 1024, 128); break;                        \
      }                                                                        \
      break;                                                                   \
    case 4096:                                                                 \
      switch (tokens_per_cta) {                                                \
        case 32:  LAUNCH_V3(BS_, K_, 4096, 32);  break;                        \
        case 64:  LAUNCH_V3(BS_, K_, 4096, 64);  break;                        \
        case 128: LAUNCH_V3(BS_, K_, 4096, 128); break;                        \
      }                                                                        \
      break;                                                                   \
    default:                                                                   \
...
#define DISPATCH_V3_BS_K(BS_)                                                  \
  switch (k) {                                                                 \
    case 1024: DISPATCH_V3_C(BS_, 1024); break;                                \
    case 4096: DISPATCH_V3_C(BS_, 4096); break;                                \
    case 16384: DISPATCH_V3_C(BS_, 16384); break;                              \
    default: TORCH_CHECK(false, "Unsupported K=", k);                          \
  }

  switch (bs) {
    case 1024: DISPATCH_V3_BS_K(1024); break;
    case 4096: DISPATCH_V3_BS_K(4096); break;
    case 8192: DISPATCH_V3_BS_K(8192); break;
    default: TORCH_CHECK(false, "Unsupported bs=", bs);
  }
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  ...
  m.def("mHC_hc_pre_v3",
        ...
        "v3 persistent megakernel: rmsnorm + linear + tail fused, single launch.");
}
