// Standalone test for the device-function port of MLA MTP TP=2 (no PDL).
...
//
// Goal: verify the port (mla_mtp_tp2_main / mla_mtp_tp2_reduce) is
...
#include "mirage/persistent_kernel/tasks/blackwell/mla_mtp_decode_tp2_sm100.cuh"
...
using namespace kernel::mla_mtp_tp2;
...
__global__ __launch_bounds__(TB) void shim_main(
    ...
    int Q_LEN,
    int qpg) {
  mla_mtp_tp2_main<SINGLE_TILE>(&Q_tm,
                                &KV_tm,
                                Oa,
                                La,
                                ss,
...
int main(int argc, char **argv) {
  ...
  fprintf(stderr,
          "=== MLA MTP TP=2 (Mirage device-function port, no PDL) ===\n");
  fprintf(stderr,
