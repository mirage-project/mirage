"""
Test: fused mla_mtp_decode_tp2_sm100 -> mla_mtp_decode_tp2_reduce_sm100 via test_mode.
...
``mla_full_ref`` with NUM_HEADS=128, taking the first H_per_rank=64 heads.

This simulates rank 0 of a TP=2 run. The kernel processes 64 heads (half of
the 128 total).
...
OFFLINE TEST-MODE CONSTRAINT (same as TP1):
  ...
  the partial-write path (WRITE_FINAL=false); the runtime-inactive second split
  takes the t0>=t1 no-op and the pre-filled LSE=-inf reduces it away.

Buffer shapes (TP=2, H_per_rank=64):
  q:              [batch * q_len * 64,   D_K=576]  bf16
  ...
  final_out:      [batch * q_len,        64 * D_V]  bf16
...
def _run_case(batch_size, q_len, static_kv_len=STATIC_KV_LEN):
    ...
    print(f"\n{'='*64}")
    print("Test: mla_mtp_decode_tp2 + reduce (TP=2) via test_mode")
    ...
    # Build full-128-head Q for reference, then slice to rank-0 (first 64 heads).
    q_full = torch.randn(
    ...
    pk.mla_mtp_decode_tp2_layer(q_dt, kv_dt, po_dt, pl_dt, q_len, static_kv_len)
    pk.mla_mtp_decode_tp2_reduce_layer(po_dt, pl_dt, out_dt, q_len, static_kv_len)
    ...
    # Full causal-MLA reference with 128 heads, slice first H_per_rank=64.
...
def test_mla_mtp_decode_tp2_testmode():
    results = []
    ...
    print(f"\n{'='*64}")
    print("SUMMARY  mla_mtp_decode_tp2 + reduce (TP=2) final-output")
    n_pass = 0
...
if __name__ == "__main__":
    ok = test_mla_mtp_decode_tp2_testmode()
