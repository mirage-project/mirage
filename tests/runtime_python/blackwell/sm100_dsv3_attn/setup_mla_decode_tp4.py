...
os.environ["PATH"] = (
    os.path.join(cuda_home, "bin") + os.pathsep + os.environ.get("PATH", "")
)
...
setup(
    name="runtime_kernel_mla_decode_tp4",
    ext_modules=[
        CUDAExtension(
            name="runtime_kernel_mla_decode_tp4",
            sources=[
                os.path.join(this_dir, "runtime_kernel_wrapper_mla_decode_tp4.cu"),
            ],
            depends=blackwell_depends,
            include_dirs=[
                os.path.join(
                    this_dir, "../../../../include/mirage/persistent_kernel/"
                ),
                os.path.join(
                    this_dir,
