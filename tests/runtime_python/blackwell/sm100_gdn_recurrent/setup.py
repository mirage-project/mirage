...
# The MEGAKERNEL compiles with -use_fast_math (persistent_kernel.py:290); this
# harness historically does not. The decode split path's bit-exactness claim has
# to hold under BOTH, because fast-math rewrites expf / log1pf / rsqrtf and the
# reciprocal, and the claim is that it rewrites them IDENTICALLY on the golden
# and the split path. Set GDN_TEST_FAST_MATH=1 to build the fast-math arm.
extra_nvcc = (['-use_fast_math']
              if os.environ.get("GDN_TEST_FAST_MATH") == "1" else [])

setup(
    name='runtime_kernel_blackwell_gdn_recurrent',
    ext_modules=[
        CUDAExtension(
            name='runtime_kernel_blackwell_gdn_recurrent',
            sources=[
