...
template <typename T,
          ...
          int EXPERTS_PER_GROUP,
          int TOPK_EXPERTS>
__global__ __launch_bounds__(256) void topk_sigmoid_kernel_multi(
    void *__restrict__ gating_output,
    void *__restrict__ bias,
    void *__restrict__ topk_weights,
    void *__restrict__ mpk_routing_indices,
  ...
  static constexpr int VPT = C::VPT;
  static constexpr int WARPS_PER_TB = 8;
  kernel::topk_sigmoid_task_impl_multi<T,
                                       VPT,
                                       EXPERTS,
                                       WARPS_PER_TB,
                                       BYTES_PER_LDG,
                                       ...
                                       EXPERTS_PER_GROUP,
                                       TOPK_EXPERTS>(
      gating_output, bias, /*finished=*/nullptr, topk_weights, num_rows,
      mpk_routing_indices, mpk_active_expert_ids,
      /*start_expert=*/0, /*end_expert=*/EXPERTS, routed_scaling_factor);
}

template <int EXPERTS>
__global__ __launch_bounds__(256) void compact_active_experts_kernel(
    int *mpk_active_expert_ids) {
  kernel::compact_active_experts_impl<EXPERTS>(
      mpk_active_expert_ids, /*start_expert=*/0, /*end_expert=*/EXPERTS);
}
