...
void mla_mtp_decode_wrapper(
    ...
    int bi = blockIdx.y;
    mla_mtp_decode_sm100_task_impl<SINGLE_TILE>(
        &Q_tm, &KV_tm, Oa, La,
        ss, kv_len, sk, num_head_groups, Q_LEN,
        gi, si, bi);
}

__global__ __launch_bounds__(RD_TB)
void mla_mtp_reduce_wrapper(
    ...
    mla_mtp_reduce_sm100_task_impl(
        Oa, La, O, sk, num_head_groups, Q_LEN,
        blockIdx.x * RD_DV, blockIdx.y, blockIdx.z);
}
...
void mla_mtp_init(
    torch::Tensor Q, torch::Tensor KV, torch::Tensor O,
    int kv_len, float sm_scale, int q_len
) {
    g_B = Q.size(0);
    g_Q_LEN = q_len;
    g_kv_len = kv_len;
    ...
    {
        uint64_t gd[3] = {64, (uint64_t)g_B * g_Q_LEN * NUM_HEADS, (uint64_t)K_ITERS};
        uint64_t gs[2] = {(uint64_t)D_K * 2, 128};
        uint32_t bd[3] = {64, (uint32_t)g_hpb, 1};
        uint32_t es[3] = {1, 1, 1};
        check_cu(cuTensorMapEncodeTiled(&g_Qtm, CU_TENSOR_MAP_DATA_TYPE_BFLOAT16, 3,
            (void*)dQ.data_ptr(), gd, gs, bd, es,
    ...
    }
    {
        ...
        uint32_t es[3] = {1, 1, 1};
        check_cu(cuTensorMapEncodeTiled(&g_KVtm, CU_TENSOR_MAP_DATA_TYPE_BFLOAT16, 3,
            (void*)dKV.data_ptr(), gd, gs, bd, es,
            CU_TENSOR_MAP_INTERLEAVE_NONE, CU_TENSOR_MAP_SWIZZLE_128B,
            CU_TENSOR_MAP_L2_PROMOTION_NONE, CU_TENSOR_MAP_FLOAT_OOB_FILL_NONE));
    }

    cudaFuncSetAttribute(mla_mtp_decode_wrapper<true>, cudaFuncAttributeMaxDynamicSharedMemorySize, smem_size);
    cudaFuncSetAttribute(mla_mtp_decode_wrapper<false>, cudaFuncAttributeMaxDynamicSharedMemorySize, smem_size);
    g_initialized = true;
}
...
void mla_mtp_run(torch::Tensor O) {
    ...
    if (g_single_tile)
        mla_mtp_decode_wrapper<true><<<grid, TB, smem_size>>>(
            g_Qtm, g_KVtm,
            (nv_bfloat16*)g_Oa.data_ptr(), (float*)g_La.data_ptr(),
            g_ss, g_kv_len, g_sk, g_num_head_groups, g_Q_LEN);
    else
        mla_mtp_decode_wrapper<false><<<grid, TB, smem_size>>>(
            g_Qtm, g_KVtm,
            (nv_bfloat16*)g_Oa.data_ptr(), (float*)g_La.data_ptr(),
            g_ss, g_kv_len, g_sk, g_num_head_groups, g_Q_LEN);

    dim3 rg((D_V + RD_DV - 1) / RD_DV, g_num_head_groups, g_B);
    mla_mtp_reduce_wrapper<<<rg, RD_TB>>>(
        (nv_bfloat16*)g_Oa.data_ptr(), (float*)g_La.data_ptr(),
        (nv_bfloat16*)O.data_ptr(),
        g_sk, g_num_head_groups, g_Q_LEN);
}
