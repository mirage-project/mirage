...
void mla_prefill_wrapper(
    const bf16* __restrict__ Q_nope,
    const bf16* __restrict__ Q_pe,
    ...
    int q_block = REVERSE_BLOCKS ? (num_q_blocks_total - 1 - blockIdx.y) : blockIdx.y;
    kernel::mla_prefill_sm100_task_impl(
        Q_nope, Q_pe, CKV, KPE, O,
        S, H, sm_scale_log2,
        blockIdx.x, q_block
    );
}
...
void mla_prefill_init(
    torch::Tensor Q_nope,  // [B, S, H, D_CKV]
    ...
    cudaFuncSetAttribute(mla_prefill_wrapper<true>,
                         cudaFuncAttributeMaxDynamicSharedMemorySize, g_smem_size);
    cudaFuncSetAttribute(mla_prefill_wrapper<false>,
                         cudaFuncAttributeMaxDynamicSharedMemorySize, g_smem_size);
    g_initialized = true;
}

void mla_prefill_run(
    torch::Tensor Q_nope,
    torch::Tensor Q_pe,
    torch::Tensor CKV,
    torch::Tensor KPE,
    ...
    if (g_S <= 2048) {
        mla_prefill_wrapper<true><<<grid, block, g_smem_size>>>(
            (const bf16*)Q_nope.data_ptr(),
            (const bf16*)Q_pe.data_ptr(),
            (const bf16*)CKV.data_ptr(),
            (const bf16*)KPE.data_ptr(),
            (bf16*)O.data_ptr(),
            g_S, g_H, g_sm_scale_log2);
    } else {
        mla_prefill_wrapper<false><<<grid, block, g_smem_size>>>(
            (const bf16*)Q_nope.data_ptr(),
            (const bf16*)Q_pe.data_ptr(),
            (const bf16*)CKV.data_ptr(),
            (const bf16*)KPE.data_ptr(),
