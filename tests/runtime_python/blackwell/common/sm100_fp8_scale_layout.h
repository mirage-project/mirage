...
inline void check_scale_tensor(torch::Tensor const &scale,
  ...
  TORCH_CHECK(
      detect_scale_tensor_layout(scale, outer_dim, reduction_size) !=
          mirage::blackwell::sm100_fp8_runtime::PackedScaleLayout::Invalid,
      name,
      " must use packed UE8M0 layout with shape [",
      outer_dim,
      ", ",
      mirage::blackwell::sm100_fp8_runtime::packed_scale_k_for_reduction_size(
          reduction_size),
      "] and either row-major stride [",
      mirage::blackwell::sm100_fp8_runtime::packed_scale_k_for_reduction_size(
          reduction_size),
      ", 1] or legacy DeepGEMM column-major stride [1, ",
      mirage::blackwell::sm100_fp8_runtime::aligned_scale_outer_dim(outer_dim),
      "], but got shape [",
      scale.size(0),
      ", ",
      scale.size(1),
