...
using FP8TMA = kernel::tma::tma_2d<
    uint8_t, 3, 3, 3,
    (DSV3_NUM_EXPERTS - 1) * ORIG_OUTPUT_SIZE_ + OUTPUT_SIZE_,
    DSV3_REDUCTION_SIZE,
    DSV3_MMA_M, 128,
    DSV3_REDUCTION_SIZE, 1,
    1, 1,
    DSV3_MMA_M * 128,
    true>;
...
using MaskLayout = cute::Layout<
    cute::Shape<cute::Int<DSV3_NUM_EXPERTS + 1>>,
...
void dsv3_fp8_moe_2d_kernel(
    ...
    uint8_t const *weight_fp8_base)   // only used for reference, TMA handles actual loads
{
  ...
  //   offset = OUTPUT_SIZE_PER_CTA * blockIdx.y * K  (in elements = bytes for uint8)
  //   (Handled by per-y TMA descriptor, so no explicit offset needed here)
  //
  // weight_scale [E, N, K/128]: input_map.y=1 (split dim 1=N)
  //   block_size = N / n_splits = OUTPUT_SIZE_PER_CTA
  //   stride[1] = K_SCALE
  //   offset = OUTPUT_SIZE_PER_CTA * blockIdx.y * K_SCALE  (in float elements)
  const float *weight_scale = weight_scale_base
      + (size_t)OUTPUT_SIZE_PER_CTA * blockIdx.y * DSV3_K_SCALE;

  // output [B, topk, N]: input_map.y=2 (split dim 2=N)
  //   block_size = N / n_splits = OUTPUT_SIZE_PER_CTA
  //   stride[2] = 1
  //   offset = OUTPUT_SIZE_PER_CTA * blockIdx.y * 1  (in bfloat16 elements)
  cute::bfloat16_t *output = output_base
      + (size_t)OUTPUT_SIZE_PER_CTA * blockIdx.y;
  ...
  constexpr int WS_ROWS = (DSV3_NUM_EXPERTS - 1) * ORIG_OUTPUT_SIZE + OUTPUT_SIZE_PER_CTA;
  ...
  kernel::fp8_moe_group_gemm_sm100_task_impl<
      ...
      DSV3_MMA_M, DSV3_MMA_N, DSV3_BATCH_SIZE,
      OUTPUT_SIZE_PER_CTA, ORIG_OUTPUT_SIZE,
      DSV3_REDUCTION_SIZE, DSV3_NUM_EXPERTS, DSV3_NUM_TOPK,
      EXPERT_STRIDE,
      true,  // W13_LINEAR
      DSV3_NUM_AB_STAGE, DSV3_NUM_ACC_STAGE, DSV3_NUM_C_STAGE>(
      tma_weight, mInput, mInputScale, mWeightScale,
      mRoutingIndices, mMask, mOutput,
    ...
    int expert_offset)
{
  ...
  auto mInputScale = cute::make_tensor(
      cute::make_gmem_ptr(const_cast<float*>(input_scale)), InputScaleLayout{});
  ...
  auto mRoutingIndices = cute::make_tensor(
      cute::make_gmem_ptr(const_cast<cute::int32_t*>(routing_indices)), IndicesLayout{});
  auto mMask = cute::make_tensor(
  ...
  kernel::fp8_moe_group_gemm_sm100_task_impl<
      TMA_t, decltype(mInput), decltype(mInputScale), decltype(mWeightScale),
      decltype(mRoutingIndices), decltype(mMask), decltype(mOutput),
      DSV3_MMA_M, DSV3_MMA_N, DSV3_BATCH_SIZE,
      DSV3_OUTPUT_SIZE, DSV3_OUTPUT_SIZE,
      DSV3_REDUCTION_SIZE, DSV3_NUM_EXPERTS, DSV3_NUM_TOPK,
      1,     // EXPERT_STRIDE=1 (single CTA)
      true,  // W13_LINEAR
      DSV3_NUM_AB_STAGE, DSV3_NUM_ACC_STAGE, DSV3_NUM_C_STAGE>(
      tma_weight, mInput, mInputScale, mWeightScale,
      mRoutingIndices, mMask, mOutput, expert_offset);
}
...
// Each descriptor has a different base pointer (offset by y * output_per_cta * K).
CUtensorMap** create_tma_desc_array(void *weight_base_ptr, int n_splits,
                                     int output_per_cta, int orig_output_size) {
  ...
  for (int y = 0; y < n_splits; ++y) {
    ...
    constexpr int B = 3, M = 3, S = 3;
    constexpr int bK = 128;
    uint64_t gmem_shape[2]  = {(uint64_t)total_rows, (uint64_t)DSV3_REDUCTION_SIZE};
  ...
  cudaMemcpy(dev_descs, host_descs, n_splits * sizeof(CUtensorMap), cudaMemcpyHostToDevice);

  // Create device array of pointers (each pointing to the corresponding descriptor)
  ...
  cudaMemcpy(dev_ptr_array, host_ptr_array, n_splits * sizeof(CUtensorMap*), cudaMemcpyHostToDevice);
...
// ================================================================
void fp8_moe_gemm_test(
    torch::Tensor input_fp8, torch::Tensor input_scale,
    torch::Tensor weight_fp8, torch::Tensor weight_scale,
    torch::Tensor routing_indices, torch::Tensor mask,
    torch::Tensor output, int expert_offset)
{
...
#define DISPATCH_2D_CASE(ES, NS) \
  if (expert_stride == (ES) && n_splits == (NS)) { \
    launch_2d_impl<(ES), (NS)>(input_fp8, input_scale, weight_fp8, \
                                 weight_scale, routing_indices, mask, output); \
    dispatched = true; \
  }

void fp8_moe_gemm_2d(
    torch::Tensor input_fp8, torch::Tensor input_scale,
    torch::Tensor weight_fp8, torch::Tensor weight_scale,
    torch::Tensor routing_indices, torch::Tensor mask,
    torch::Tensor output,
    int expert_stride, int n_splits)
{
  c10::cuda::CUDAGuard guard(input_fp8.device());
...
void fp8_moe_gemm_bench_setup(
    torch::Tensor weight_fp8, int expert_stride, int n_splits)
{
...
template <int EXPERT_STRIDE, int N_SPLITS>
void bench_launch_impl(
    torch::Tensor input_fp8, torch::Tensor input_scale,
    torch::Tensor weight_scale, torch::Tensor routing_indices,
...
void fp8_moe_gemm_bench_launch(
    torch::Tensor input_fp8, torch::Tensor input_scale,
    torch::Tensor weight_scale, torch::Tensor routing_indices,
    torch::Tensor mask, torch::Tensor output)
{
...
void fp8_moe_gemm_bench_cleanup() {
  if (g_bench_tma_array) {
    ...
    cudaMemcpy(&first_desc, g_bench_tma_array, sizeof(CUtensorMap*), cudaMemcpyDeviceToHost);
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  m.def("fp8_moe_gemm_test", &fp8_moe_gemm_test,
        "FP8 MoE GEMM - single CTA correctness test");
  m.def("fp8_moe_gemm_2d", &fp8_moe_gemm_2d,
        "FP8 MoE GEMM - 2D grid (expert_stride x n_splits), single launch");
  m.def("fp8_moe_gemm_bench_setup", &fp8_moe_gemm_bench_setup,
        "Setup TMA descriptors for benchmark (expert_stride, n_splits)");
  m.def("fp8_moe_gemm_bench_launch", &fp8_moe_gemm_bench_launch,
        "Async launch for benchmark (no sync)");
  m.def("fp8_moe_gemm_bench_cleanup", &fp8_moe_gemm_bench_cleanup,
