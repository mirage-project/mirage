...
// Matches the production encoding in tma.cuh for TASK_FP8_GEMM_DENSE_SMALLM_SM100:
...
CUtensorMap make_tma_fp8_2d(void *ptr, int K, int outer) {
  ...
  check_driver_success(cuTensorMapEncodeTiled(&tm,
...
__global__ void __launch_bounds__(256) fp8_dense_smallm_test_kernel(
    CUtensorMap const *ta,
...
__global__ void __launch_bounds__(256) fp8_dense_smallm_multi_cta_test_kernel(
    CUtensorMap const *ta,
    CUtensorMap const *tb,
    float const *     sa,
    float const *     sb,
...
void fp8_gemm_dense_smallm_launch(torch::Tensor a_fp8,
  ...
  TORCH_CHECK(cudaMemcpy(d_ta, &ta, sizeof(CUtensorMap), cudaMemcpyHostToDevice)
                  == cudaSuccess,
              "cudaMemcpy d_ta failed");
  TORCH_CHECK(cudaMemcpy(d_tb, &tb, sizeof(CUtensorMap), cudaMemcpyHostToDevice)
                  == cudaSuccess,
  ...
  TORCH_CHECK(
      cudaFuncSetAttribute(fp8_dense_smallm_test_kernel,
  ...
  // Use plain cudaLaunchKernel (matching the existing linear_fp8 test wrapper pattern).
  ...
  void *kernel_params[] = {&arg_ta, &arg_tb, &arg_sa, &arg_sb,
                            &arg_C,  &arg_M,  &arg_N,  &arg_K};
...
void fp8_gemm_dense_smallm_multi_cta_launch(torch::Tensor a_fp8,
  ...
  constexpr int kSmemBytes =
      kernel::fp8_gemm_dense_smallm::fp8_gemm_dense_smallm_smem_size<128, 3>();
  TORCH_CHECK(
      cudaFuncSetAttribute(fp8_dense_smallm_multi_cta_test_kernel,
                           cudaFuncAttributeMaxDynamicSharedMemorySize,
                           kSmemBytes) == cudaSuccess,
      "cudaFuncSetAttribute MaxDynamicSharedMemorySize failed");
  ...
  void *kernel_params[] = {&arg_ta, &arg_tb, &arg_sa, &arg_sb,
                            &arg_C,  &arg_M,  &arg_N,  &arg_K, &arg_NW};
...
__global__ void __launch_bounds__(128) quantize_fp8_f32_test_kernel(
...
void quantize_then_gemm_launch(torch::Tensor a_bf16,
  ...
  cudaStream_t stream = at::cuda::getCurrentCUDAStream().stream();
  auto opts_u8 = torch::TensorOptions()
                     .dtype(at::kFloat8_e4m3fn)
                     .device(a_bf16.device());
  auto opts_f = torch::TensorOptions().dtype(at::kFloat).device(a_bf16.device());
  ...
  cudaError_t qerr = cudaLaunchKernel(
      ...
      quant_grid, quant_block, quant_params,
      /*sharedMem=*/0, stream);
  ...
  fp8_gemm_dense_smallm_multi_cta_launch(a_fp8, b_fp8, sa, sb, output,
                                          num_workers);
}
...
__global__ void __launch_bounds__(128) quantize_fp8_slice_test_kernel(
    __nv_bfloat16 const *__restrict__ input,
    __nv_fp8_e4m3 *__restrict__ output_q,
    float *__restrict__ output_s) {
  int const row_idx = blockIdx.y;
