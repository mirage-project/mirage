...
template <int BATCH, int OUTPUT_SIZE, int K_>
__global__ void
    linear_fp8_swapAB_kernel_wrapper(CUtensorMap *input_desc,
...
void launch_linear_fp8_swapAB(torch::Tensor &input_q,
  ...
  CUtensorMap host_w_desc =
      make_weight_desc<OUTPUT_SIZE, K_>(weight_q.data_ptr());
  CUtensorMap host_out_desc =
      make_output_desc<BATCH, OUTPUT_SIZE>(output.data_ptr());
  ...
  TORCH_CHECK(cudaMemcpy(d_in_desc,
                         &host_in_desc,
                         sizeof(CUtensorMap),
                         cudaMemcpyHostToDevice) == cudaSuccess);
  TORCH_CHECK(cudaMemcpy(d_w_desc,
                         &host_w_desc,
                         sizeof(CUtensorMap),
                         cudaMemcpyHostToDevice) == cudaSuccess);
  ...
  // synccheck reported "Stack overflow" inside
  // linear_fp8_swapAB_sm100_task_impl.
...
// =========================================================================
void linear_fp8_swapAB_sm100(
    torch::Tensor input_q,      // [BATCH, K]    fp8_e4m3
...
std::vector<std::tuple<int, int, int>> supported_shapes() {
  ...
  static constexpr int kNs[] = {
      128, 128, 128, 128, 128, 256, 256, 256, 512, 512};
  static constexpr int kKs[] = {
      128, 1536, 4608, 7168, 16384, 1536, 4608, 7168, 1536, 7168};
  for (int b : {1, 4, 8, 16}) {
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  m.def("linear_fp8_swapAB_sm100",
        &linear_fp8_swapAB_sm100,
