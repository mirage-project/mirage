...
__global__ __launch_bounds__(256, 1) void fp8_group_gemm_smallm_wrapper(
  ...
  kernel::fp8_group_gemm_smallm::fp8_group_gemm_smallm_sm100_task_impl(
      &ta, &tb, &tsfa, &tsfb, &td, m_indices,
      M_total, N, K, E, blockIdx.x, gridDim.x);
}
...
static void run_impl(Kernel k_func, int smem,
                     void const *A_ptr, void const *B_ptr,
                     uint32_t const *sfa_packed, uint32_t const *sfb_packed,
                     bf16 *D, int const *m_indices,
  ...
  // A: [K, M_total] FP8 (uint8 raw)
  {
    ...
    uint32_t e[2] = {1, 1};
    chk(cuTensorMapEncodeTiled(&ta, CU_TENSOR_MAP_DATA_TYPE_UINT8, 2,
        (void *)A_ptr, g, s, b, e, CU_TENSOR_MAP_INTERLEAVE_NONE,
  ...
  // B: [K, E*N] FP8
  {
    ...
    uint32_t b[2] = {128, BN};
    uint32_t e[2] = {1, 1};
    chk(cuTensorMapEncodeTiled(&tb, CU_TENSOR_MAP_DATA_TYPE_UINT8, 2,
        (void *)B_ptr, g, s, b, e, CU_TENSOR_MAP_INTERLEAVE_NONE,
  ...
  // SFA: [M_total, num_sf_k] uint32
  {
    ...
    uint32_t b[2] = {128, 1};
    uint32_t e[2] = {1, 1};
    chk(cuTensorMapEncodeTiled(&tsfa, CU_TENSOR_MAP_DATA_TYPE_UINT32, 2,
        (void *)sfa_packed, g, s, b, e, CU_TENSOR_MAP_INTERLEAVE_NONE,
  ...
  // SFB: [E*N, num_sf_k] uint32
  {
    ...
    uint32_t b[2] = {BN, 1};
    uint32_t e[2] = {1, 1};
    chk(cuTensorMapEncodeTiled(&tsfb, CU_TENSOR_MAP_DATA_TYPE_UINT32, 2,
        (void *)sfb_packed, g, s, b, e, CU_TENSOR_MAP_INTERLEAVE_NONE,
  ...
  // D: [N, M_total] BF16
  {
    ...
    uint32_t b[2] = {64, 128};
    uint32_t e[2] = {1, 1};
    chk(cuTensorMapEncodeTiled(&td, CU_TENSOR_MAP_DATA_TYPE_BFLOAT16, 2,
        (void *)D, g, s, b, e, CU_TENSOR_MAP_INTERLEAVE_NONE,
        CU_TENSOR_MAP_SWIZZLE_128B, CU_TENSOR_MAP_L2_PROMOTION_NONE,
        CU_TENSOR_MAP_FLOAT_OOB_FILL_NONE));
  }
  ...
  if (smem > 48000) {
    cudaFuncSetAttribute(k_func, cudaFuncAttributeMaxDynamicSharedMemorySize, smem);
  }
  k_func<<<grid, 256, smem>>>(ta, tb, tsfa, tsfb, td, m_indices,
                              M_total, N, K, E);
}

void fp8_group_gemm_decode(torch::Tensor A,           // [M_total, K] fp8 e4m3 raw u8
  ...
  // Variant dispatch: K > 4096 && MPE <= 8 -> smallm (BN=64); else largem (BN=128).
  if (K > 4096 && MPE <= 8) {
    run_impl<64>(fp8_group_gemm_smallm_wrapper,
    ...
    run_impl<128>(fp8_group_gemm_largem_wrapper,
