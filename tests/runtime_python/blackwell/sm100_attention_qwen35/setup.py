...
setup(
    name='runtime_kernel_blackwell_attention_qwen35',
    ext_modules=[
        CUDAExtension(
            name='runtime_kernel_blackwell_attention_qwen35',
            sources=[
            ...
            # MPK_TARGET_CC and MODE_OFFLINE are NOT optional here. The
            # attention task guards its arena with
            #   static_assert(S_TOTAL_OFFSET <= MAX_DYNAMIC_SHARED_MEMORY_SIZE)
            # and that constant is selected by those two macros
            # (runtime_header.h:35-60). Without them the TU compiles against
            # the 160 KiB fallback instead of the 201 KiB Blackwell budget the
            # megakernel actually runs with, so admissible MAX_TOKENS values
            # fail to build and the test would silently cover a different
            # configuration than production. The megakernel build passes
            # exactly these (persistent_kernel.py:237-241).
            extra_compile_args={
                'cxx': ['-DMIRAGE_GRACE_BLACKWELL', '-DMPK_TARGET_CC=100',
                        '-DMODE_OFFLINE'],
                'nvcc': [
                    '-O3',
