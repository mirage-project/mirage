    ...
    bfloat16 *output_base)
{
  ...
  using TMA_A = kernel::tma::tma_2d<bfloat16, B, M, S,
  ...
  auto mInput = cute::make_tensor(
      ...
      cute::make_layout(
          cute::make_shape(cute::Int<DSV3_BATCH_SIZE>{}, cute::Int<DSV3_REDUCTION_SIZE>{}),
  ...
  auto mBias = cute::make_tensor(
      ...
      cute::make_layout(
          ...
          cute::make_stride(cute::Int<OUTPUT_SIZE_PER_CTA>{},
                            cute::Int<1>{},
  ...
  auto mRoutingIndices = cute::make_tensor(
      ...
      cute::make_layout(
          cute::make_shape(cute::Int<DSV3_NUM_EXPERTS>{}, cute::Int<DSV3_BATCH_SIZE>{}),
  ...
  auto mMask = cute::make_tensor(
      ...
      cute::make_layout(
          cute::make_shape(cute::Int<DSV3_NUM_EXPERTS + 1>{}),
...
// ================================================================
static CUtensorMap** create_bf16_tma_desc_array(
    void *weight_base_ptr, int n_splits, int output_per_cta, int orig_output_size)
{
  constexpr int B = 3, M = 3, S = 3;
  constexpr int TMA_CP_ASYNC_SIZE = 64;
  constexpr int TILE_SIZE = 64;
  ...
  for (int y = 0; y < n_splits; ++y) {
    ...
    uint32_t smem_shape[2]  = {(uint32_t)DSV3_MMA_M, (uint32_t)TMA_CP_ASYNC_SIZE};

    mirage::runtime::fill_tma_desc<bfloat16, B, M, S, 2>(
        &host_descs[y], w_ptr, gmem_shape, gmem_stride, smem_shape,
        1, w_smem_repeat_col);
  }
...
template <int EXPERT_STRIDE, int N_SPLITS>
void bf16_bench_launch_impl(
    torch::Tensor input, torch::Tensor routing, torch::Tensor mask,
    torch::Tensor output)
{
  constexpr int OUTPUT_PER_CTA = DSV3_OUTPUT_SIZE / N_SPLITS;
  constexpr int smem_size = 224 * 1024;
...
void bf16_moe_bench_cleanup() {
  if (g_bf16_tma_array) {
    ...
    cudaMemcpy(&first_desc, g_bf16_tma_array, sizeof(CUtensorMap*), cudaMemcpyDeviceToHost);
    cudaFree(first_desc);
...
// ================================================================
void bf16_moe_gemm_test(
    torch::Tensor input, torch::Tensor weight,
    torch::Tensor routing, torch::Tensor mask,
    torch::Tensor output)
{
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  m.def("bf16_moe_gemm_test", &bf16_moe_gemm_test,
        "BF16 MoE GEMM - single CTA test");
  m.def("bf16_moe_bench_setup", &bf16_moe_bench_setup,
        "Setup TMA for BF16 benchmark");
  m.def("bf16_moe_bench_launch", &bf16_moe_bench_launch,
        "Async launch BF16 MoE benchmark");
  m.def("bf16_moe_bench_cleanup", &bf16_moe_bench_cleanup,
