...
for reduction_size in reduction_sizes:
    for output_size in output_sizes:
        ...
        residual = torch.randn(batch_size, output_size, device="cuda", dtype=torch.float)
        output = torch.empty(batch_size, output_size, device="cuda", dtype=torch.float)

        runtime_kernel_blackwell.linear(x, w, residual, output) # with residual
        # runtime_kernel_blackwell.linear(x, w, None, output) # without residual
        ...
        # Warm-up
        for _ in range(16):
            runtime_kernel_blackwell.linear(x, w, residual, output)
