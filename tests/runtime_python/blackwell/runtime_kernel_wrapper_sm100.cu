...
__global__ __launch_bounds__(256, 1) void linear_kernel_ws_sm100_wrapper(
  ...
  kernel::linear_kernel_ws_sm100<SharedStorage,
                                 ATensor,
                                 BTensor,
                                 BiasTensor,
                                 CTensor,
...
void launch_linear_sm100_warp_specialized(TypeA const *device_ptr_input,
  ...
  int const num_ab_stage = 8;
  int const num_c_stage = 4;
  int const num_acc_stage = 2;

  // Represent the full tensors in global memory
  cute::Tensor mA = cute::make_tensor(cute::make_gmem_ptr(device_ptr_weight),
                                      layout_A); // (Gemm_M, Gemm_K)
  cute::Tensor mB = cute::make_tensor(cute::make_gmem_ptr(device_ptr_input),
                                      layout_B); // (Gemm_N, Gemm_K)
  cute::Tensor mBias = cute::make_tensor(cute::make_gmem_ptr(device_ptr_bias),
                                         layout_Bias); // (Gemm_N, Gemm_M)
  cute::Tensor mC = cute::make_tensor(cute::make_gmem_ptr(device_ptr_output),
                                      layout_C); // (Gemm_N, Gemm_M)
  ...
  cute::TiledMMA tiled_mma = cute::make_tiled_mma(
      cute::SM100_MMA_F16BF16_SS<TypeA,
                                 TypeB,
                                 TypeAcc, // Mma's A, B, and Accumulator types
                                 mma_m,
  ...
  int const num_tmem_columns = bN * num_acc_stage;
  ...
  // // mma_shape_A:  ((_128,_16),_1,_4,_8) cute::print("mma_shape_B:\t");
  // cute::print(mma_shape_B); cute::print("\n");  // mma_shape_B:
  // ((_32,_16),_1,_4,_8) cute::print("mma_shape_C:\t");
  // cute::print(mma_shape_C); cute::print("\n");  // mma_shape_C:
  // ((_32,_128),_1,_1,_4)

  auto epi_tiler = cute::make_tile(cute::size<0, 0>(mma_shape_C),
                                   cute::size<0, 1>(mma_shape_C));
  ...
  // sA_layout:   Sw<3,4,3> o smem_ptr[16b](unset) o
  // ((_128,_16),_1,_4):((_64,_1),_0,_16) cute::print("sB_layout:\t");
  // cute::print(sB_layout); cute::print("\n");      // sB_layout:   Sw<3,4,3> o
  // smem_ptr[16b](unset) o ((_256,_16),_1,_4):((_64,_1),_0,_16)
  // cute::print("sC_layout:\t"); cute::print(sC_layout); cute::print("\n"); //
  // sC_layout:   Sw<3,4,3> o smem_ptr[16b](unset) o
  // ((_256,_16),_1,_4):((_64,_1),_0,_16)

  // The cluster shape and layout
  auto cluster_shape =
      cute::make_shape(cute::Int<1>{}, cute::Int<1>{}, cute::Int<1>{});
  cute::Layout cluster_layout_vmnk = cute::tiled_divide(
  ...
  cute::Copy_Atom tma_atom_A = cute::make_tma_atom_A_sm100(
      ...
      sA_layout(
          cute::_, cute::_, cute::_, cute::Int<0>{}), // Destination SMEM layout
      mma_tiler,
      tiled_mma,
      cluster_layout_vmnk // MK Tiler for TMA operation
  );
  cute::Tensor mA_tma =
      tma_atom_A.get_tma_tensor(cute::shape(mA)); // (Gemm_M, Gemm_K)
  ...
  cute::Tensor mB_tma =
      tma_atom_B.get_tma_tensor(cute::shape(mB)); // (Gemm_N, Gemm_K)
  ...
  cute::Copy_Atom tma_atom_C = cute::make_tma_atom(
      ...
      epi_tiler);
  cute::Tensor mC_tma =
      tma_atom_C.get_tma_tensor(cute::shape(mC)); // (Gemm_N, Gemm_K)

  using SMEMStorage = kernel::PipedSharedStorage<TypeA,
                                                 TypeB,
                                                 TypeC,
                                                 decltype(sA_layout),
                                                 decltype(sB_layout),
...
void linear_kernel_warp_specialized(torch::Tensor input,
  ...
  auto const *input_ptr = static_cast<bfloat16 const *>(input.data_ptr());
  auto const *weight_ptr = static_cast<bfloat16 const *>(weight.data_ptr());
  bool has_residual = residual.has_value();
  auto const *residual_ptr =
      has_residual ? static_cast<bfloat16 const *>(residual->data_ptr())
                   : nullptr;
  ...
  auto layout_A = cute::make_layout(
      cute::make_shape(weight.size(0), weight.size(1)),
      cute::make_stride(weight.size(1),
                        cute::Int<1>{})); // (Gemm_M,Gemm_K):(Gemm_K,_1)
  // B tensor NxK K-major (Layout N = Column-Major)
  auto layout_B = cute::make_layout(
      cute::make_shape(input.size(0), input.size(1)),
      cute::make_stride(input.size(1),
                        cute::Int<1>{})); // (Gemm_N,Gemm_K):(Gemm_K,_1)
  // C tensor MxN N-major (Layout T = Row-Major)
  auto layout_C = cute::make_layout(
      cute::make_shape(input.size(0), weight.size(0)),
      cute::make_stride(weight.size(0),
  ...
  launch_linear_sm100_warp_specialized(input_ptr,
                                       weight_ptr,
                                       residual_ptr,
                                       output_ptr,
                                       layout_A,
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  m.def("linear_warp_specialized",
        &linear_kernel_warp_specialized,
        "Linear kernel Warp Specialized");
  m.def(
