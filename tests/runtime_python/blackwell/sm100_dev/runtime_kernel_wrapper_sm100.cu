...
void gate_topk_sm100_kernel(torch::Tensor input,
  ...
  const int WEIGHT_REDUCTION = static_cast<int>(weight.size(1));
  ...
  // Compute gating_output = input @ weight^T in float on current CUDA stream
  auto input_f = input.to(at::kFloat);
  auto weight_f_t = weight.to(at::kFloat).transpose(0, 1);
  auto gating_output = at::matmul(input_f, weight_f_t);

  // Launch fused topk softmax (strictly 256 threads per block inside)
...
// Forward declaration so we can bind both in one module
