...
template <typename T, int BATCH_SIZE, int OUTPUT_SIZE, int REDUCTION_SIZE,int NUM_EXPERTS, int NUM_TOPK, int EXPERT_STRIDE>
void launch_moe_linear(void const *input_ptr,
                   ...
                   void const *expert_routing_ptr,
                   void const *expert_mask_ptr) {
  static_assert(OUTPUT_SIZE % 64 == 0);
  dim3 grid_dim(EXPERT_STRIDE, OUTPUT_SIZE / 64, 1);
  dim3 block_dim(128, 1, 1);
  size_t smem_size = 96 * 1024;

  cudaFuncSetAttribute(
      moe_linear_kernel_wrapper<T, BATCH_SIZE, 64, OUTPUT_SIZE, REDUCTION_SIZE, NUM_EXPERTS, NUM_TOPK, EXPERT_STRIDE>,
      cudaFuncAttributeMaxDynamicSharedMemorySize,
      smem_size);
...
#define MOE_LINEAR_DISPATCH_OUTPUT_SIZE(BATCH_SIZE, OUTPUT_SIZE)                   \
  case OUTPUT_SIZE:                                                            \
    launch_moe_linear<bfloat16, BATCH_SIZE, OUTPUT_SIZE, 2048, 128, 8, 5>(                    \
        input_ptr, weight_ptr, residual_ptr, output_ptr, expert_routing_ptr, expert_mask_ptr);                      \
    break;
