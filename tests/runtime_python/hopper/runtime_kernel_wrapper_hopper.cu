...
#define DISPATCH_NORM_LINEAR_HOPPER_BATCH_SIZE_CASE(BATCH_SIZE)                \
  ...
  void launch_multitoken_paged_attention_hopper(
    ...
    using TMA_KV = kernel::tma::tma_3d<bfloat16,
                                B,
                                M,
                                S,
                                num_tokens,
                                ...
                                1,
                                (HEAD_DIM + TMA_CP_SIZE - 1) / TMA_CP_SIZE,
                                KV_TILE_SIZE * TMA_CP_SIZE, // skip number of rows between current 64 cols and next 64 cols
                                (NUM_QO_HEADS + 2 * NUM_KV_HEADS) * HEAD_DIM,
                                HEAD_DIM,
                                1,
                                true>;
    ...
    using TMA_OUTPUT = kernel::tma::tma_2d<bfloat16,
                                                3,
                                                3,
                                                3,
