...
template <typename T,
          ...
          typename TMA_OUT,
          int Kstages = 2>
__global__ __launch_bounds__(256, 1) void linear_kernel_hopper_no_residual_wrapper(
    const __grid_constant__ TMA_A tma_a,
    const __grid_constant__ TMA_B tma_b,
    const __grid_constant__ TMA_OUT tma_out) {
...
void launch_linear_hopper(void *input_ptr,
  ...
  linear_kernel_hopper_wrapper<T,
    ...
    cudaFuncSetAttribute(linear_kernel_hopper_no_residual_wrapper<T,
      BATCH_SIZE,
      OUTPUT_SIZE,
      REDUCTION_SIZE,
      TMA_A,
      TMA_B,
      TMA_OUT>, cudaFuncAttributeMaxDynamicSharedMemorySize, smem_size);
