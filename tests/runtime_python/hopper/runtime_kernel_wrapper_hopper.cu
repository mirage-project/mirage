...
void launch_linear_swapAB_hopper(void *input_ptr,
                                 void *weight_ptr,
                                 void *residual_ptr,
                                 void *output_ptr) {
  ...
  // tma_out kept for signature compatibility, not used
  ...
  TMA_OUT tma_out(output_ptr); // unused in device function
...
#define DISPATCH_LINEAR_SWAPAB_REDUCTION_SIZE_CASE(                            \
  ...
  case REDUCTION_SIZE:                                                         \
    launch_linear_swapAB_hopper<bfloat16,                                      \
                                BATCH_SIZE,                                    \
                                OUTPUT_SIZE,                                   \
                                REDUCTION_SIZE>(                               \
...
void linear_kernel_swapAB(torch::Tensor input,
                          torch::Tensor weight,
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  ...
  m.def("linear_swapAB", &linear_kernel_swapAB, "Linear swapAB kernel");
