// Poison shared memory before invoking the production attention device
// function.
...
__global__ void run_attention(void *qkv,
                              void *kc,
                              void *vc,
                              void *out,
                              int *qo,
  ...
  __syncthreads();
  kernel::multitoken_paged_attention_hopper_impl<type::bfloat16_t,
                                                 8,
                                                 1,
                                                 1,
                                                 128,
...
int launch_attention(void *qkv,
                     ...
                     int *last) {
  cudaError_t result =
      cudaFuncSetAttribute(run_attention<MAX_TOKENS>,
                           cudaFuncAttributeMaxDynamicSharedMemorySize,
                           SMEM_BYTES);
  if (result != cudaSuccess) {
    return result;
  }
  run_attention<MAX_TOKENS>
      <<<1, 256, SMEM_BYTES>>>(qkv, kc, vc, out, qo, pi, pages, last);
  return cudaDeviceSynchronize();
}

extern "C" int launch(void *qkv,
                      void *kc,
                      void *vc,
                      void *out,
                      int *qo,
