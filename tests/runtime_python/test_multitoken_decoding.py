...
def test_multitoken_kernel():
    ...
    for seq_len, description in test_cases:
        ...
        k_cache_init = torch.randn((max_seq_len, head_dim), device=device, dtype=dtype) * 0.1
        v_cache_init = torch.randn((max_seq_len, head_dim), device=device, dtype=dtype) * 0.1
        ...
        heads_per_token = q_heads + k_heads + v_heads
        qkv = torch.randn(num_tokens * heads_per_token, head_dim, 
                          device=device, dtype=dtype) * 0.1
        
        # 2. Calculate expected results using PyTorch
        expected_outputs, expected_k_cache, expected_v_cache = pytorch_multitoken_attention(
        ...
        kernel_outputs = torch.empty((num_tokens * q_heads, head_dim), 
                                     device=device, dtype=dtype)
        
        runtime_kernel.single_batch_multitoken_decoding(
            qkv, k_cache_kernel, v_cache_kernel, kernel_outputs, seq_len,
            False, False, None, None, None, None, 0.0, 0.0
        )
        ...
        for token_idx in range(num_tokens):
            ...
            if token_diff > 0.01:
                print(f"  Token {token_idx}: FAILED - diff={token_diff:.6f}, "
        ...
        for token_idx in range(num_tokens):
            ...
            if pos < max_seq_len:
                k_diff = (expected_k_cache[pos] - k_cache_kernel[pos]).abs().max().item()
                if k_diff > 1e-5:
                    print(f"  Position {pos} (token {token_idx}): FAILED - diff={k_diff:.6f}")
        ...
        for token_idx in range(num_tokens):
            pos = seq_len - 1 + token_idx
            if pos < max_seq_len:
                v_diff = (expected_v_cache[pos] - v_cache_kernel[pos]).abs().max().item()
                if v_diff > 1e-5:
                    print(f"  Position {pos} (token {token_idx}): FAILED - diff={v_diff:.6f}")
