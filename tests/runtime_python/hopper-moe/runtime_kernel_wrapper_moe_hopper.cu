 ...
 */
 ...
 __global__ __launch_bounds__(256, 1) void moe_linear_sm90_wrapper(
   ...
   kernel::moe_linear_sm90_task_impl<
           T, 
           TMA_A,
           InputTensor,
           BiasTensor,
 ...
 void moe_w13_linear_sm90_kernel(torch::Tensor input,
   ...
   launch_moe_linear_sm90<bfloat16, BATCH_SIZE, OUTPUT_SIZE, REDUCTION_SIZE, NUM_EXPERTS, NUM_TOPK, EXPERT_OFFSET, EXPERT_STRIDE, true>(input_ptr, weight_ptr, mpk_routing_indices_ptr, mpk_expert_mask_ptr, output_ptr, residual_ptr);
 ...
 void moe_w2_linear_sm90_kernel(torch::Tensor input,
   ...
   launch_moe_linear_sm90<bfloat16, BATCH_SIZE, OUTPUT_SIZE, REDUCTION_SIZE, NUM_EXPERTS, NUM_TOPK, EXPERT_OFFSET, EXPERT_STRIDE, false>(input_ptr, weight_ptr, mpk_routing_indices_ptr, mpk_expert_mask_ptr, output_ptr, residual_ptr);
   cudaError_t err = cudaDeviceSynchronize();
   if (err != cudaSuccess) {
     printf("CUDA kernel launch error: %s\n", cudaGetErrorString(err));
   }
 ...
 PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
   m.def("moe_w13_linear_sm90", &moe_w13_linear_sm90_kernel, "MoE W13 Linear kernel sm90");
   m.def("moe_w2_linear_sm90", &moe_w2_linear_sm90_kernel, "MoE W2 Linear kernel sm90");
 }