...
void run_builtin_command(const string &cmd) {
	if (cmd == "ls") {
		cout << "Listing all testcases..." << endl;
		for (const Testcase &testcase : all_testcases) {
			cout << "- " << testcase.name;
			cout << ", [";
			for (const string &tag : testcase.tags) {
				cout << tag << ", ";
			}
			cout << "\b\b], ";
...
int main(int argc, char* argv[]) {
	...
	for (Testcase &testcase : all_testcases) {
		...
		if (!testcase_name.empty() && testcase_name[0] == '+') {
			should_run = testcase_name == "+" || std::count(testcase.tags.begin(), testcase.tags.end(), testcase_name.substr(1));
		} else {
	...
	for (const auto& [testcase, subcase_results] : results) {
		...
		for (const Subcase::RunResult &subcase_result : subcase_results) {
			max_subcase_name_len = std::max(max_subcase_name_len, (int)subcase_result.subcase_name.length());
		}
		for (const Subcase::RunResult &subcase_result : subcase_results) {
			string subcase_name = string(max_subcase_name_len - subcase_result.subcase_name.length(), ' ') + subcase_result.subcase_name;
			if (subcase_name != "") subcase_name += ", ";
			printf("  %30s | %s%s, %6.2f ms%s%s\n",
				is_first_line ? testcase.name.c_str() : "",
				subcase_name.c_str(),
				subcase_result.is_passed ? "\e[97;42;1m PASSED \e[0m" : "\e[97;41;1m FAILED \e[0m",
