...
using std::cout, std::cerr, std::endl, std::string, std::vector, std::optional, std::nullopt, std::pair, std::array;
...
size_t get_cumulative_mul(const vector<int> &v) {
	size_t result = 1;
	for (int x : v)
		result *= x;
	return result;
...
// Print out a dtensor
void print_dtensor(const kn::DTensor &dtensor, const vector<half> &data) {
	int cur_elem_idx = 0;
...
#define CHECK_CUDA(status)                                                     \
  do {                                                                         \
    if (status != 0) {                                                         \
			std::cerr << "At file " << __FILE__ << ", line " << __LINE__ << std::endl; \
...
template<typename T>
inline bool is_equal_helper(const T &a_, const T &b_, float max_abs_err, float max_rel_err) {
...
template<>
inline bool is_equal(const half &a, const half &b) {
...
public:
	...
	vector<half> operator ()(const vector<int> &dims) const {
...
struct IOTensor {
	...
	IOTensor() {}
	IOTensor(const vector<int> &dims, const vector<size_t> &strides, const vector<half> &data) : dims(dims), strides(strides), data(data) {
...
public:
	Subcase(const SubcaseConfig &config, const optional<string> &subcase_name = nullopt, const optional<epilogue_t> &epilogue = nullopt) :
		subcase_name(subcase_name.value_or("")), config(config), epilogue(epilogue) {
		subcase_id = next_subcase_id++;
		cu_file_path = env_config::get_generated_cu_dir() + "/" + std::to_string(subcase_id) + ".cu";
		so_file_path = env_config::get_generated_so_dir() + "/" + std::to_string(subcase_id) + ".so";
	}
	...
	kn::DTensor new_input(const vector<int> &dims, const generator_t &generator, const optional<vector<size_t>> &strides_ = nullopt) {
		vector<size_t> strides;
		...
		kn::DTensor result = g.new_input(dims, type::DT_FLOAT16, layout::DmemRowMajor);
		// print_dtensor(result, data);
		return result;
	}

	void mark_output(const kn::DTensor &dtensor, const vector<int> &dims, const vector<half> &std_answer) {
		// def print_tensor(t): print(','.join(map(lambda x: "%.3f"%x, t.flatten().tolist())))
	...
	using add_custom_op_ret_t = pair<std::shared_ptr<tb::Graph>, array<kn::DTensor, NUM_OUTPUTS>>;

	template<int NUM_OUTPUTS>
	array<kn::DTensor, NUM_OUTPUTS> add_custom_op(
		const vector<kn::DTensor> &inputs,
		const std::function<
			add_custom_op_ret_t<NUM_OUTPUTS>
			(const vector<kn::DTensor> &)> &constructor
	) {
		auto [tb_graph_ptr, result_dtensors] = constructor(inputs);
		// Here is an ugly workaround. The only way (in Mirage's code) to get a valid
		// guid for a DTensor which is an output of some block graph, is to, construct
		// a customized_op from an Execution plan. So here we convert tb_graph to 
		// an execution plan.
		// And in order to let DTensors in `result_dtensors` have valid guids, we
		// get those guids from custom_op->output_tensors and copy them to result_dtensors
		// To avoid misalignment, we enforce the number of output tensors to be 1
		kn::KNOperator *custom_op = this->g.create_customized_op(inputs, tb_graph_ptr->get_plan());
		assert(custom_op && "Error when adding the customized op to the kernel graph");
...
private:
	// Call `mirage::transpiler::Transpile` to transpile the graph, and return the result
	trans::TranspileResult transpile() const {
		...
		trans::TranspileResult trans_result = trans::transpile(&g, config.transpiler_config, input_strides, output_tensor_ptrs);

		// Print transpiled result
		printf("Code: see %s\n", cu_file_path.c_str());
		printf("Buf size: %lu B (%.2f GB)\n", trans_result.buf_size, (float)trans_result.buf_size/(1l<<30));
		printf("Output tensor data:\n");
		for (const auto &output_directive : trans_result.output_directives) {
			printf("  phy_size: %lu B (%lu elems), ", output_directive.alloc_size, output_directive.alloc_size / sizeof(half));
			printf("shape: [");
	...
	void save_and_compile(const string &code) const {
		...
		fprintf(fp, "%s\n", code.c_str());
		fclose(fp);
		printf("Compiling subcase %s (%s -> %s)...\n", subcase_name.c_str(), cu_file_path.c_str(), so_file_path.c_str());
		...
		for (const string &option : compile_options) {
	...
	RunResult run(const trans::TranspileResult &trans_result) {
		...
		auto execute_mugraph_func = (void (*)(std::vector<void*>, std::vector<void*>, void*))dlsym(handle, "execute_mugraph");
		...
		for (const IOTensor &input : this->inputs) {
		...
		for (size_t i = 0; i < outputs.size(); ++i) {
			...
			half* data_d;
			CHECK_CUDA(cudaMalloc((void**)&data_d, trans_result.output_directives[i].alloc_size));
		...
		for (int warmup_iter = 0; warmup_iter < config.num_warmups; ++warmup_iter) {
			...
			if (warmup_iter == 0 && config.check_answer) {
				...
				for (int output_idx = 0; output_idx < (int)this->outputs.size(); ++output_idx) {
					const IOTensor &output = this->outputs[output_idx].first;
					printf("Output #%d:\n", output_idx);
					...
					CHECK_CUDA(cudaMemcpy(output_ptr_h, output_ptrs_d[output_idx], phy_size, cudaMemcpyDeviceToHost));
					...
					printf("%lu out of %lu (%.2f%%) mismatched.\n", mismatch_indexes.size(), numel, (float)mismatch_indexes.size() / numel * 100.0);
					for (size_t i = 0; i < std::min(10ul, mismatch_indexes.size()); ++i) {
						...
						float rel_err = abs_err / std::max(std::abs(truth), std::abs(answer));
						printf("Mismatch at index %lu, phy index %lu, coord: [", logical_pos, phy_pos);
						for (size_t x : coord) {
							printf("%lu,", x);
						}
						printf("\b]:\n");
						printf("  expected %f, got %f (abs: %f, rel: %.2f%%)\n", truth, answer, abs_err, rel_err*100);
					}
...
public:
	...
	Testcase(const string &name, const vector<string> &tags, const string &description, const std::function<void(Testcase*)> &logic):
	...
	vector<Subcase::RunResult> run() {
		...
		for (int start_subcase = 0; start_subcase < num_subcases; start_subcase += num_compile_threads) {
			int end_subcase = std::min(start_subcase + num_compile_threads, num_subcases);
			printf("================================================================\n");
			printf("Processing subcase %d ~ %d\n", start_subcase, end_subcase-1);
