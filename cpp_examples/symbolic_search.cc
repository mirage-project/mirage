...
std::vector<std::vector<int>> get_input_shapes(AttentionConfig const &config) {
  return {
      {config.batch, config.num_heads * config.query_seq_len, config.head_dim},
      ...
      for (int query_seq_len : query_seq_len_list) {
        for (int kv_seq_len : kv_seq_len_list) {
          for (int head_dim : head_dim_list) {
            configs.push_back(AttentionConfig{
                batch, num_heads, query_seq_len, kv_seq_len, head_dim});
          }
        }
      }
    }
...
std::string get_checkpoint_name(AttentionConfig const &config,
                                bool use_symbolic_search) {
  if (!use_symbolic_search) {
    return "checkpoint_attention_batch" + std::to_string(config.batch) +
           "_heads" + std::to_string(config.num_heads) + "_qseq" +
           std::to_string(config.query_seq_len) + "_kvseq" +
           std::to_string(config.kv_seq_len) + "_dim" +
           std::to_string(config.head_dim) + ".json";
  } else {
...
std::vector<json> update_input_shapes_for_symbolic_graphs(
  ...
  for (auto const &graph : graphs) {
    ...
    SymbolicKNGraph updated_graph = construct_graph_with_different_input_shapes(
        symbolic_kn_graph, input_shapes);
    updated_graphs.push_back(updated_graph);
  }
  return updated_graphs;
}

std::vector<json> search_graphs(AttentionConfig const &config,
                                bool use_symbolic_search) {
  ...
  kernel::Graph ref_graph;
  {
    kernel::DTensor Q = ref_graph.new_input(
        ...
        layout::DmemRowMajor);
    kernel::DTensor Kt =
        ref_graph.new_input({config.batch, config.head_dim, h},
  ...
  std::string checkpoint_name =
      get_checkpoint_name(config, use_symbolic_search);

  std::cout << "[Searching] batch: " << config.batch
  ...
  search::KernelGraphGenerator gen(
      ref_graph, search_config, checkpoint_name.data());
  if (use_symbolic_search) {
    gen.search_symbolic();
  } else {
    gen.search();
...
float test_attention(AttentionConfig const &config, bool use_symbolic_search) {
  ...
  if (is_checkpoint_exists(checkpoint_name)) {
    ...
    if (use_symbolic_search) {
      graphs = update_input_shapes_for_symbolic_graphs(
          graphs, get_input_shapes(config));
    }
  } else {
...
int main(int argc, char **argv) {
  bool use_symbolic_search = (argc > 1 && std::string(argv[1]) == "-s");
  ...
  for (AttentionConfig const &config : configs) {
    ...
    auto it =
        std::find_if(results.begin(), results.end(), [&key](json const &e) {
    ...
    if (it != results.end()) {
      ...
      std::cout << "Config (batch=" << config.batch
                << ", num_heads=" << config.num_heads
                << ", ...) already recorded, skipping" << std::endl;
    } else {
