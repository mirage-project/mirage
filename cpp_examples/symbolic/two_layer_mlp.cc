...
//   ./symbolic_two_layer_mlp -d                     – single debug config (n=8,
//   d=4096)
//   ./symbolic_two_layer_mlp --force-nonsym         – re-run non-symbolic
//   search
//   ./symbolic_two_layer_mlp --force-sym            – re-run symbolic search
//   ./symbolic_two_layer_mlp --skip-nonsym          – skip non-symbolic search
//   entirely
//   ./symbolic_two_layer_mlp --sym-checkpoint <f>   – use <f> as symbolic
//   checkpoint
//   ./symbolic_two_layer_mlp --time-limit <sec>     – search time limit
//   (default 3600)
//   ./symbolic_two_layer_mlp --max-kn-ops <n>       – max kernel graph ops
//   (default 5)
//   ./symbolic_two_layer_mlp --explore-all-maps     – explore all input
//   mappings
//
// Output files:
//   checkpoints/two_layer_mlp/  – per-config and shared symbolic checkpoints
//   results_two_layer_mlp.json  – best times for each (n, d) x {non-symbolic,
//   symbolic}

struct TwoLayerMlpConfig {
  int n, d;
};
...
static void run_experiments(std::vector<TwoLayerMlpConfig> const &configs,
  ...
  if (skip_nonsym) {
    std::cout << "\n=== two_layer_mlp: non-symbolic search (skipped) ==="
              << std::endl;
  } else {
    ...
    for (auto const &cfg : configs) {
      ...
      search::GeneratorConfig config = get_generator_config(
          ...
          symbolic_maps);
      if (max_kn_ops > 0) {
        config.max_num_kernel_graph_op = max_kn_ops;
      }
      ...
      double ns_search_time = 0;
      {
        ...
        ns_search_time = std::chrono::duration<double>(t1 - t0).count();
      }
      ...
      auto [best_time, so_path] =
          profile_best_with_so(gen.get_generated_graphs());
  ...
  if (skip_sym) {
    std::cout << "\n=== two_layer_mlp: symbolic search (skipped) ==="
              << std::endl;
  } else {
    ...
    double sym_search_time = 0;
    {
      ...
      if (!checkpoint_exists(sym_ckpt)) {
        ...
        gen.search_symbolic();
        auto t1 = std::chrono::steady_clock::now();
        sym_search_time = std::chrono::duration<double>(t1 - t0).count();
      } else {
        sym_search_time = -1; // cached
      }
    }
    if (search_only) {
...
int main(int argc, char **argv) {
  ...
  if (!config_str.empty()) {
    ...
    configs =
        debug ? std::vector<TwoLayerMlpConfig>{kDebugConfig} : get_configs();
  }
