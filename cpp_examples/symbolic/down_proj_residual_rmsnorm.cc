...
//
// Usage:
//   ./symbolic_down_proj_residual_rmsnorm                        – sweep all (n, d) configs
//   ./symbolic_down_proj_residual_rmsnorm -d                     – single debug config (n=8, d=4096)
//   ./symbolic_down_proj_residual_rmsnorm --force-nonsym         – re-run non-symbolic search
//   ./symbolic_down_proj_residual_rmsnorm --force-sym            – re-run symbolic search
//   ./symbolic_down_proj_residual_rmsnorm --skip-nonsym          – skip non-symbolic search entirely
//   ./symbolic_down_proj_residual_rmsnorm --skip-sym             – skip symbolic search entirely
//   ./symbolic_down_proj_residual_rmsnorm --sym-checkpoint <f>   – use <f> as symbolic checkpoint
//   ./symbolic_down_proj_residual_rmsnorm --time-limit <sec>     – search time limit (default 3600)
//
// Output files:
//   checkpoints/down_proj/  – per-config and shared symbolic checkpoints
//   results_down_proj.json  – best times for each (n, d) x {non-symbolic, symbolic}

struct DownProjConfig { int n, d; };
...
static void build_fused_graph(kernel::Graph &g) {
  ...
  kernel::DTensor Y;
  {
    ...
    threadblock::STensor bX_in =
        bgraph.new_input(X_in, {-1, -1, -1}, /*forloop_dim=*/1,
                         layout::SmemRowMajor);
    // bW_down: (64, 64) per iter – each TB reads its own column block
    threadblock::STensor bW_down =
        bgraph.new_input(W_down, {1, -1, -1}, /*forloop_dim=*/0,
                         layout::SmemRowMajor);
    ...
    bgraph.mark_output(bY_acc, {1, -1, -1}, /*forloop_dim=*/-1,
  ...
  //            + bZ_rms(~0) + bO(8KB) ≈ 40KB
  {
    ...
    threadblock::STensor bY =
        bgraph.new_input(Y,     {0, -1, -1}, /*forloop_dim=*/-1,
                         layout::SmemRowMajor);     // (1, 4096)
    threadblock::STensor bX_res =
        bgraph.new_input(X_res, {0, -1, -1}, /*forloop_dim=*/-1,
                         layout::SmemRowMajor);     // (1, 4096)
    ...
    bgraph.mark_output(bO, {0, -1, -1}, /*forloop_dim=*/-1,
                       type::TB_EPILOGUE_NONE);
...
static void run_experiments(std::vector<DownProjConfig> const &configs,
  ...
  std::cout << "\n=== down_proj_residual_rmsnorm: manually fused kernel ===" << std::endl;
  {
  ...
  if (skip_nonsym) {
    std::cout << "\n=== down_proj_residual_rmsnorm: non-symbolic search (skipped) ===" << std::endl;
  } else {
  std::cout << "\n=== down_proj_residual_rmsnorm: non-symbolic search ===" << std::endl;
  ...
  if (skip_sym) {
    std::cout << "\n=== down_proj_residual_rmsnorm: symbolic search (skipped) ===" << std::endl;
  } else {
  std::cout << "\n=== down_proj_residual_rmsnorm: symbolic search ===" << std::endl;
  ...
  if (!best_nonsym_so.empty() && !best_sym_so.empty()) {
    ...
    std::cout << "./check_correctness " << best_nonsym_so << " " << best_sym_so
              << " --inputs " << input_size << "," << weight_size << "," << residual_size
              << " --outputs " << output_size
              << " --buf 0" << std::endl;
  }
}
