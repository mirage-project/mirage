...
int main(int argc, char **argv) {
  ...
  {
    kernel::DTensor Q = ref_graph.new_input({batch_size, 16 * 64, 64},
                                            {65536, 64, 1},
                                            type::DT_FLOAT16,
                                            layout::DmemRowMajor);
    kernel::DTensor K = ref_graph.new_input({batch_size, 64, 4096},
                                            {262144, 4096, 1},
                                            type::DT_FLOAT16,
                                            layout::DmemColumnMajor);
    kernel::DTensor V = ref_graph.new_input({batch_size, 4096, 64},
                                            {262144, 64, 1},
                                            type::DT_FLOAT16,
                                            layout::DmemColumnMajor);
  ...
  kernel::Graph graph;
  kernel::DTensor Q = graph.new_input({batch_size, 16 * 64, 64},
                                      {65536, 64, 1},
                                      type::DT_FLOAT16,
                                      layout::DmemRowMajor);
  kernel::DTensor K = graph.new_input({batch_size, 64, 4096},
                                      {262144, 4096, 1},
                                      type::DT_FLOAT16,
                                      layout::DmemColumnMajor);
  kernel::DTensor V = graph.new_input({batch_size, 4096, 64},
                                      {262144, 64, 1},
                                      type::DT_FLOAT16,
                                      layout::DmemColumnMajor);
  ...
  config.grid_dim_to_explore = {
      {batch_size, 8, 16}, {batch_size, 32, 1}, {batch_size, 128, 1}};
  std::string checkpoint_file_name =
      "checkpoint_multi_query_attn_spec_decode_bs" +
