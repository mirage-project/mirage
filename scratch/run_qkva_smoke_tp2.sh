#!/usr/bin/env bash
# Two-GPU QKV-a fusion smoke (TP=2 EP=1, layers 0-3, prompt=128 + 1 new tok).
# Verifies the H8 OUTPUT_STRIDE fix end-to-end on TP=2 (cheaper to schedule
# than TP=4 on a contested machine).
set -euo pipefail

ts=${1:-$(date +%H%M%S)}
mode=${2:-fused}  # fused | baseline
gpus=${3:-4,5}     # comma-separated GPU indices
OUT=/home/muhengl/mirage/outputs/dpskv3_qkva_tp2_${mode}_${ts}
...
if [ "${mode}" = "fused" ]; then
    ...
    export MPK_DEEPSEEK_WEIGHT_CACHE_DIR=/tmp/dpskv3_v8_weight_cache_qkva_fused_tp2_${FUSED_N}
else
    export MPK_DSV3_QKV_A_FUSED=0
    export MPK_DEEPSEEK_WEIGHT_CACHE_DIR=/tmp/dpskv3_v8_weight_cache_qkva_baseline_tp2
fi
mkdir -p "${MPK_DEEPSEEK_WEIGHT_CACHE_DIR}"

cd /home/muhengl/mirage
CUDA_VISIBLE_DEVICES=${gpus} mpirun --allow-run-as-root -np 2 \
