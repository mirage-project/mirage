#!/usr/bin/env bash
# Watchdog variant that runs smoke with MPK_DSV3_QKV_A_OUT_ATTACH=1 so qkv_a_out
# is a torch tensor, then reads it directly from Python (bypassing the
# in-megakernel dump task).
set -uo pipefail

LOG=/tmp/qkva_attach_watchdog.log
RESULT=/tmp/qkva_attach_watchdog.result
STOP=/tmp/qkva_attach_watchdog.stop
INTERVAL=30
NEED=4
TAG="attach_test"
...
while true; do
  ...
  if [ "$count" -ge "$NEED" ]; then
    ...
    export MPK_DSV3_QKV_A_OUT_ATTACH=1   # KEY: read qkv_a_out as torch tensor
    ...
    # Analyze the existing dump tensor (it should still work + we'll also
    # see the elementwise_add dump pattern)
...
print(f'rc=$rc, gpus={"$gpus"}, ATTACH=1')
...
for name, s in [('q_a',f[:,:1536]),('c_lat',f[:,1536:2048]),('k_pe',f[:,2048:2112])]:
    ...
    rng = ''
    if zr.numel() > 0 and zr.numel() < 100:
...
PY
    echo "[$(date '+%H:%M:%S')] done; result in $RESULT" >> "$LOG"
    exit 0
  fi
  sleep "$INTERVAL"
done
