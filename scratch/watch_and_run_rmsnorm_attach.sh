#!/usr/bin/env bash
# Variant of watch_and_run_qkva.sh that runs three back-to-back smokes:
#   (1) baseline_unfused  - MPK_DSV3_QKV_A_FUSED=0  (regression sanity)
#   (2) fused_no_attach   - MPK_DSV3_QKV_A_FUSED=1  (current buggy behavior)
#   (3) fused_attach      - MPK_DSV3_QKV_A_FUSED=1 + MPK_DSV3_RMSNORM_OUT_ATTACH=1
#       (tests the buffer-pool-aliasing hypothesis: bypass pool for
#        rmsnorm_out and see whether the rows-1..71-zero bug disappears)
#
# Writes /tmp/qkva_rmsnorm_attach.log and /tmp/qkva_rmsnorm_attach.result.
#
# Usage:
#   bash scratch/watch_and_run_rmsnorm_attach.sh &
# Stop:
#   touch /tmp/qkva_rmsnorm_attach.stop

set -uo pipefail

LOG=/tmp/qkva_rmsnorm_attach.log
RESULT=/tmp/qkva_rmsnorm_attach.result
STOP=/tmp/qkva_rmsnorm_attach.stop
INTERVAL=30
NEED=4

rm -f "$RESULT" "$STOP"
echo "[$(date '+%H:%M:%S')] watchdog (rmsnorm_attach) started; need=$NEED" > "$LOG"

run_one() {
    local tag="$1"
    local extra_env_kvs="$2"
    local gpus="$3"

    local OUT=/home/muhengl/mirage/outputs/dpskv3_qkva_${tag}
    mkdir -p "${OUT}/build" "${OUT}/dump"

    export MPI_HOME=/usr/mpi/gcc/openmpi-4.1.9a1
    export PATH=$MPI_HOME/bin:$PATH
    export MPI_INC_PATH=$MPI_HOME/include
    export MPI_LIB_PATH=$MPI_HOME/lib
    export NVSHMEM_HOME=/home/muhengl/local/nvshmem-3.6.5-dev/usr
    export NVSHMEM_INC_PATH=$NVSHMEM_HOME/include/nvshmem_13
    export NVSHMEM_LIB_PATH=$NVSHMEM_HOME/lib/x86_64-linux-gnu/nvshmem/13
    export LD_LIBRARY_PATH=$NVSHMEM_LIB_PATH:$MPI_HOME/lib:${LD_LIBRARY_PATH:-}
    export LD_PRELOAD=$NVSHMEM_LIB_PATH/libnvshmem_host.so.3.6.5
    export NVSHMEM_MAX_TEAMS=128

    # Per-tag env overrides (e.g., MPK_DSV3_QKV_A_FUSED=0 or =1, etc.)
    eval "$extra_env_kvs"
    export MPK_DEEPSEEK_WEIGHT_CACHE_DIR=/tmp/dpskv3_v8_weight_cache_qkva_fused_2176

    cd /home/muhengl/mirage

    echo "[$(date '+%H:%M:%S')] launching tag=$tag gpus=$gpus env=$extra_env_kvs" >> "$LOG"

    set +e
    CUDA_VISIBLE_DEVICES="$gpus" mpirun --allow-run-as-root -np 4 \
        -x CUDA_VISIBLE_DEVICES -x LD_LIBRARY_PATH -x LD_PRELOAD -x PATH \
        -x MPI_INC_PATH -x MPI_LIB_PATH -x NVSHMEM_INC_PATH -x NVSHMEM_LIB_PATH \
        -x NVSHMEM_MAX_TEAMS \
        -x MPK_DSV3_QKV_A_FUSED -x MPK_DSV3_QKV_A_FUSED_N \
        -x MPK_DSV3_FP8_BUF_ATTACH -x MPK_DSV3_QKV_A_OUT_ATTACH \
        -x MPK_DSV3_RMSNORM_OUT_ATTACH \
        -x MPK_DEEPSEEK_WEIGHT_CACHE_DIR \
        /home/muhengl/mirage/.venv/bin/python demo/deepseek_v3/demo.py \
        --model-path /raid/catalyst/models/DeepSeek-V3 \
        --use-mirage --max-num-batched-tokens 128 --max-num-batched-requests 1 \
        --page-size 128 --max-num-pages 2 --max-seq-length 256 \
        --prompt-length 128 --ignore-eos --max-new-tokens 1 \
        --layers 0-3 --mtp 0 --ep-size 2 \
        --output-dir "${OUT}/build" --dump-hidden-dir "${OUT}/dump" \
        > "${OUT}/run.log" 2>&1
    local rc=$?
    set -e
    echo "[$(date '+%H:%M:%S')] tag=$tag rc=$rc; OUT=$OUT" >> "$LOG"

    /home/muhengl/mirage/.venv/bin/python <<PY >> "$RESULT" 2>&1
import torch, os
TAG = "$tag"
DIR = '$OUT/dump'
print(f'\n=== tag={TAG}  rc=$rc ===')
print(f'gpus=$gpus  env=$extra_env_kvs')
qkv_path = f'{DIR}/layer0_q_a_out.pt'
if not os.path.exists(qkv_path):
    print(f'FAIL: no qkv dump.')
else:
    f = torch.load(qkv_path, weights_only=True).float()
    print(f'qkv_a_out shape: {tuple(f.shape)}')
    for name, sl in [('q_a',f[:,:1536]),('c_lat',f[:,1536:2048]),('k_pe',f[:,2048:2112])]:
        zr = (sl.abs().sum(dim=1)==0).nonzero(as_tuple=True)[0]
        rng = ''
        if zr.numel() > 0:
            zl = zr.tolist(); ranges = []; i = 0
            while i < len(zl):
                j = i
                while j+1 < len(zl) and zl[j+1] == zl[j]+1: j += 1
                ranges.append((zl[i], zl[j])); i = j+1
            rng = ', '.join(f'{a}' if a==b else f'{a}..{b}' for a,b in ranges)
        print(f'  {name:>6s}: zero={zr.numel():>3d}  rng={rng}')
    print(f'  row0_norm={f[0].norm():.3f}  row1_norm={f[1].norm():.3f}  row71_norm={f[71].norm():.3f}  row72_norm={f[72].norm():.3f}')

# residual cos vs baseline_sanitybase
base = '/home/muhengl/mirage/outputs/dpskv3_qkva_baseline_sanitybase/dump'
if os.path.isdir(base):
    for L in range(4):
        try:
            a = torch.load(f'{base}/layer_{L:02d}_residual.pt', weights_only=True).float()
            b = torch.load(f'{DIR}/layer_{L:02d}_residual.pt', weights_only=True).float()
            cos = torch.dot(a.flatten(), b.flatten()) / (a.norm() * b.norm() + 1e-12)
            print(f'  layer_{L:02d}_residual cos: {cos:.6f}')
        except FileNotFoundError as e:
            print(f'  layer_{L:02d}_residual: missing ({e})')
PY
}

poll=0
while true; do
  poll=$((poll + 1))
  if [ -f "$STOP" ]; then
    echo "[$(date '+%H:%M:%S')] stop file detected; exiting" >> "$LOG"
    exit 0
  fi
  free_list=$(bash /home/muhengl/mirage/scratch/scan_free_gpus.sh list 2>/dev/null)
  count=$(echo "$free_list" | grep -c '.' || echo 0)
  echo "[$(date '+%H:%M:%S')] poll $poll: $count free ($(echo $free_list | tr '\n' ','))" >> "$LOG"
  if [ "$count" -ge "$NEED" ]; then
    gpus=$(echo "$free_list" | head -n "$NEED" | paste -sd,)
    echo "[$(date '+%H:%M:%S')] FOUND $NEED free GPUs: $gpus — starting smokes" >> "$LOG"

    # PRIMARY: fused + rmsnorm_attach (hypothesis test).
    run_one rmsnorm_attach \
        'export MPK_DSV3_QKV_A_FUSED=1 MPK_DSV3_QKV_A_FUSED_N=2176 MPK_DSV3_FP8_BUF_ATTACH=0 MPK_DSV3_QKV_A_OUT_ATTACH=0 MPK_DSV3_RMSNORM_OUT_ATTACH=1' \
        "$gpus"

    # CONTROL 1: fused + qkv_a_out_attach=1, no rmsnorm attach (isolate rmsnorm)
    run_one qkv_out_attach_only \
        'export MPK_DSV3_QKV_A_FUSED=1 MPK_DSV3_QKV_A_FUSED_N=2176 MPK_DSV3_FP8_BUF_ATTACH=0 MPK_DSV3_QKV_A_OUT_ATTACH=1 MPK_DSV3_RMSNORM_OUT_ATTACH=0' \
        "$gpus"

    # CONTROL 2: both attaches (verify they compose).
    run_one both_attach \
        'export MPK_DSV3_QKV_A_FUSED=1 MPK_DSV3_QKV_A_FUSED_N=2176 MPK_DSV3_FP8_BUF_ATTACH=0 MPK_DSV3_QKV_A_OUT_ATTACH=1 MPK_DSV3_RMSNORM_OUT_ATTACH=1' \
        "$gpus"

    echo "[$(date '+%H:%M:%S')] all smokes done; result in $RESULT" >> "$LOG"
    exit 0
  fi
  sleep "$INTERVAL"
done
